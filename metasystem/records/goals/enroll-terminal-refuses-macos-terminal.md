...
- Concluded: Landed at 1475dc51 through chain enroll-login-build1 (two implementer rounds, critic chain enroll-login-crit1 with two rounds, fresh final critic enroll-login-crit2 with zero material findings). The enrollment walk admits a process with withheld arguments only when it runs with effective uid 0 and its executable is /usr/bin/login on both reads; the darwin parent and executable readers now read the kernel process table and the path lookup, which answer for every live process; the owner is a compared fact; a refusal names the process it saw and the tmux shape that works. Proof: package tests pin the Terminal.app chain (login as session leader), the tmux chain and every refusal; a live identity test walked to this Mac's real root-owned login; a live enrollment through the real Terminal.app chain with the kernel reader succeeded from shell 61288 with login 61287 as session leader. Wido enrolls from Terminal.app with a rebuilt engine. The two critic chains stay unclosed under Ruling O, which goal:merge-stage-critic-close owns.
...
- 2026-09-06T09:08:42Z JJS1ZAG64S03WFAGTXVPWS65N7-m1b-c6925449 done actor=m1b+main-1788680071-18713-e76d5d targets=enroll-terminal-refuses-macos-terminal
Integrity: sha256=0b3cc32a30168f0612c7a19e0759995bb7019236d29167687cdb97750daeb8ae
