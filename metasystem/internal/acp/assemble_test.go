...
func TestWatermarkDefeatsReplay(t *testing.T) {
	...
	assembler.SetFence(3)
	assembler.Consume(1, replayUser)   // replay, at or below the fence
	assembler.Consume(2, replayAnswer) // replay
	assembler.Consume(4, liveAnswer)   // live, inside the window (3, 5)
	candidate, err := assembler.Candidate(5)
...
func TestAssemblyRules(t *testing.T) {
	...
	if candidate, _ = assembler.Candidate(10); candidate != nil {
	...
	assembler = NewAssembler("s")
	assembler.SetFence(0)
	if candidate, err = assembler.Candidate(10); err != nil || candidate != nil {
...
func TestOversizeDisqualifies(t *testing.T) {
	...
	if _, err := assembler.Candidate(10); err != ErrCandidateTooLarge {
	...
	if _, err := assembler.Candidate(2); err != ErrCandidateTooLarge {
