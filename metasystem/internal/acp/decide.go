...
func Decide(effects []Effect, envelope Envelope) Verdict {
	...
	for _, effect := range effects {
		switch one := decideOne(effect, envelope); one {
		case VerdictUnclassifiable:
			return VerdictUnclassifiable
		case VerdictDeny:
			verdict = VerdictDeny
		}
...
func decideOne(effect Effect, envelope Envelope) Verdict {
	...
	case EffectExecute:
		...
		// recorded as evidence by the caller.
		if envelope.Tools == "read-only" {
			return VerdictDeny
		}
		return VerdictAllow
	case EffectNetwork:
		// network=ask cannot occur here — preflight refused it.
...
// PermissionAnswer is the wire-shaped response payload.
