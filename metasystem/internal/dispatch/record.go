...
var immutableFields = map[string]bool{
	...
	"operationId": true, "goalId": true, "goalRevision": true, "goalTier": true, "gateWidth": true, "machineId": true,
...
// chain-closure flags, and the aggregated chain usage. Everything else about
...
func RecordSetup(root, job, sourcePath string) error {
	return withRecordSessionLock(root, job, func(recordPath string, transaction *SessionIndexTransaction) error {
		...
		if asString(current["status"]) != "pending-setup" ||
			...
			!sameValue(record["gateWidth"], current["gateWidth"]) ||
