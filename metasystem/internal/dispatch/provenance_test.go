...
func TestSTR3TierSnapshotPlumbing02FreshFollowUpAndSetupMismatch(t *testing.T) {
	...
	mismatchedTier := follow
	mismatchedTier.Output = filepath.Join(stage, "follow-wrong-tier.json")
	mismatchedTier.GoalTier = 3
	if err := BuildFollowRecord(mismatchedTier); err == nil || !strings.Contains(err.Error(), "follow-up must inherit the parent goalTier") {
		t.Fatalf("follow-up replaced the parent goal tier: %v", err)
	}
