...
func TestingRequirement(goalID, gateWidth string) (string, error) {
	...
	return fmt.Sprintf("\n# Required testing contract\n\nUse the committed shared testing contract for goal %s (recorded gate width %s). Compute the actual whole-project candidate, run the risk-selected public `metasystem test` plan through its admitted proof owner, collect every independent result in each entered stage, and require `test verify` to report sufficient delivery evidence before returning. A failed canary blocks every later stage; do not substitute a fixed full battery or erase the recorded risk. Retained proof is reused across rounds and attempts by execution identity, so this run is the smallest proving run for your change: report its attempt id (the proof-run id in its result's log path). The build cache is provided for the whole chain (GOCACHE, GOTMPDIR and STATICCHECK_CACHE are set): never set, unset or strip them, and never run a gate under env -u or env -i.\n", goalID, gateWidth), nil
}
