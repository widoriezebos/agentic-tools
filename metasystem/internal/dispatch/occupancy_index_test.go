...
func TestSessionOccupancyRecordVerbsKeepOnePublicationOrder(t *testing.T) {
	...
	if err := BuildSetup(setup, "record-verbs", "implementer", "", "main-1", "5", "", sessionKey, "nonce1"); err != nil {
		t.Fatal(err)
	}
	...
	pending := writeJSON(t, filepath.Join(t.TempDir(), "pending.json"), map[string]any{
		"jobId": "record-verbs", "role": "implementer", "runtime": "fake", "round": 1,
		"parentJob": nil, "status": "pending", "phase": "handshake", "error": nil,
		"mainId": "main-1", "claimEpoch": 5, "goalId": nil, "sessionId": nil,
