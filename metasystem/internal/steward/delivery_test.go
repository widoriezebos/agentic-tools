...
func fileOnlyDeliveryBed(t *testing.T, now time.Time, file *goal.GoalFile) (string, func(string, time.Time) (claimedDeliverySnapshot, error)) {
	t.Helper()
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "metasystem.conf"), nil, 0o644); err != nil {
		t.Fatal(err)
	}
	snapshot := claimedDeliverySnapshot{
		converted: true,
		machine:   "bed-m1",
		projection: goal.Projection{Tree: &goal.TreeGoals{
			Live: map[string]*goal.GoalFile{file.Id: file},
		}},
	}
	calls := 0
	t.Cleanup(func() {
		if calls != 1 {
			t.Errorf("delivery snapshot reader was called %d times, want exactly once", calls)
		}
	})
	read := func(gotRoot string, gotNow time.Time) (claimedDeliverySnapshot, error) {
		t.Helper()
		calls++
		if calls != 1 || gotRoot != root || gotNow != now {
			t.Fatalf("delivery snapshot read %d: root=%q time=%v, want root=%q time=%v and one call", calls, gotRoot, gotNow, root, now)
		}
		return snapshot, nil
	}
	return root, read
...
func TestClaimedGoalDeliveryVerdicts(t *testing.T) {
	...
	t.Run("failed job after claim raises dead", func(t *testing.T) {
		...
		root, read := fileOnlyDeliveryBed(t, now, deliveryGoal(claimAt))
		...
		role := checkClaimedGoalDeliveryWithReader(root, now, read)
	...
	t.Run("claim backstop uses a tighter goal elapsed limit", func(t *testing.T) {
		...
		root, read := fileOnlyDeliveryBed(t, now, file)
		writeDeliveryReceipts(t, root, "1|1970-01-01T00:00:01Z|RECEIPT|goal=unrelated")
