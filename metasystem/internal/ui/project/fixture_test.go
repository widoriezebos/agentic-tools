...
// entriesOf flattens one subsection's groups to the ids it listed, in order.
func entriesOf(section Subsection) []string {
	ids := []string{}
...
func sectionOf(thread Thread, id string) Subsection {
...
func groupOf(section Subsection, id string) Group {
	for _, group := range section.Groups {
		if group.ID == id {
			return group
		}
...
func entryOf(section Subsection, id string) Entry {
	for _, group := range section.Groups {
		for _, entry := range group.Documents {
			if entry.ID == id {
				return entry
...
func groupTitles(section Subsection) []string {
	titles := []string{}
	for _, group := range section.Groups {
