...
const shipped = `{"hooks":{"SessionStart":[],"Stop":[]}}`

func TestCheckOwnHooksAcceptsCompliant(t *testing.T) {
	dir := t.TempDir()
	live := write(t, dir, "live.json", `{"hooks":{"SessionStart":[{"command":"$CLAUDE_PROJECT_DIR/metasystem/scripts/agents/supervision-hook.sh"}],"Stop":[]}}`)
...
func TestCheckOwnHooksReportsMissingHook(t *testing.T) {
	dir := t.TempDir()
	live := write(t, dir, "live.json", `{"hooks":{"SessionStart":[{"command":"$CLAUDE_PROJECT_DIR/metasystem/supervision-hook.sh"}]}}`)
	s := write(t, dir, "shipped.json", shipped)
...
func TestCheckOwnHooksRequiresSupervisionHook(t *testing.T) {
	dir := t.TempDir()
	live := write(t, dir, "live.json", `{"hooks":{"SessionStart":[{"command":"$CLAUDE_PROJECT_DIR/metasystem/other.sh"}],"Stop":[]}}`)
	s := write(t, dir, "shipped.json", shipped)
...
func TestCheckOwnHooksRequiresVendoredEntry(t *testing.T) {
	dir := t.TempDir()
	live := write(t, dir, "live.json", `{"hooks":{"SessionStart":[{"command":"scripts/agents/supervision-hook.sh"}],"Stop":[]}}`)
	s := write(t, dir, "shipped.json", shipped)
