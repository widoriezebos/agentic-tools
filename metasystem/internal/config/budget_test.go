...
func clearBudgetLawEnvironment(t *testing.T) {
	t.Helper()
	for _, key := range []string{ElapsedGracePercentKey, SliceNormHoursKey, ReviewRoundMaxKey, Tier1BudgetKey, Tier2BudgetKey, Tier3BudgetKey, GoalNormJobMinutesKey, "metasystem.budget.risk-gate", CarryOpenMaxKey} {
		name := EnvName(key)
...
func TestRiskGateMarkEnforceAndRefusal(t *testing.T) {
	...
	problems := validateRepo(t, validConf+"metasystem.budget.risk-gate=mark\n")
