...
func (r *hostCycleGoals) Capture(string) (string, error) {
	r.captures++
	return "", fmt.Errorf("canonical goal branch is absent")
}
func (r *hostCycleGoals) Accepted() (string, bool, error) { r.accepted++; return "", false, nil }
...
func (r *hostCycleGoals) Release(string) error { return r.unexpected("Release") }
...
func buildGitFreeHostCycle(t *testing.T, behavior string) *Engine {
	...
	t.Cleanup(func() {
		if goalRepository.captures == 0 || goalRepository.accepted == 0 {
			t.Errorf("serving goal source was unused: captures=%d accepted=%d", goalRepository.captures, goalRepository.accepted)
		}
	})
