...
func recoveryCanonicalTip(t *testing.T, endpoint Endpoint) string {
	t.Helper()
	client, ok := endpoint.Repository.(*fakeGoalRepository)
	if !ok {
		t.Fatal("recovery fixture needs the fake repository")
	}
	client.store.mu.Lock()
	defer client.store.mu.Unlock()
	if _, ok := client.store.commits[client.store.canonical]; !ok {
		t.Fatal("canonical recovery commit is missing")
	}
...
func TestRecoveryRebuildsAnswer(t *testing.T) {
	...
	root := endpoint.Root
	if result, err := Open(verbReqFor(endpoint, "01J5X0000000000000000000K0", "mac-a"), "answer-recovery", "Recover an answer.", "main", "Wait."); err != nil || result.Outcome != OutcomeConfirmed {
	...
	rebuilt, err := requestForEntry(endpoint, entry)
	...
	if err := validateCommitFor(endpoint, projection.Tip); err != nil {
...
func TestRecoveryRefusesJournaledSetBudgetAuthority(t *testing.T) {
	...
	endpoint, _ := fakeGoalEndpoint(t)
	root := endpoint.Root
	request := verbReqFor(endpoint, "01J5X00000000000000000QE00", "mac-a")
	...
	tipBefore := acceptedTipForEndpoint(t, endpoint)
	canonicalBefore := recoveryCanonicalTip(t, endpoint)
	treeBefore, err := loadTreeFor(endpoint, tipBefore)
	if err != nil {
		t.Fatal(err)
	}
	...
	if acceptedTipForEndpoint(t, endpoint) != tipBefore {
		t.Fatalf("refused recovery moved the accepted goal tip: before=%s after=%s", tipBefore, acceptedTipForEndpoint(t, endpoint))
	}
	if after := recoveryCanonicalTip(t, endpoint); after != canonicalBefore {
		t.Fatalf("refused recovery moved the canonical goal tip: before=%s after=%s", canonicalBefore, after)
	}
	treeAfter, err := loadTreeFor(endpoint, tipBefore)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRecoveryUnblocksAStrandedPush(t *testing.T) {
	...
	if res, err := Open(verbReqFor(aEndpoint, "01J5X00000000000000000Q010", "mac-a"), "claimable", "Work.", "main", "Go."); err != nil || res.Outcome != OutcomeConfirmed {
	...
	if res, err := Open(verbReqFor(aEndpoint, "01J5X00000000000000000Q040", "mac-a"), "unblocked", "Flows.", "main", "Go."); err != nil || res.Outcome != OutcomeConfirmed {
...
func TestRecoveryRefusesToReplayAbandonEngineFloorAndAbandonedReopen(t *testing.T) {
	...
	reopenEndpoint, _ := fakeGoalEndpoint(t)
	root := reopenEndpoint.Root
	if result, err := Open(verbReqFor(reopenEndpoint, "01J5X00000000000000001Q100", "mac-a"), "normal-reopen", "intent", "main", "next"); err != nil || result.Outcome != OutcomeConfirmed {
		t.Fatalf("open: %+v %v", result, err)
	}
	if result, err := Done(verbReqFor(reopenEndpoint, "01J5X00000000000000001Q110", "mac-a"), "normal-reopen", "done"); err != nil || result.Outcome != OutcomeConfirmed {
		t.Fatalf("done: %+v %v", result, err)
	}
	reopen := verbReqFor(reopenEndpoint, "01J5X00000000000000001Q120", "mac-a")
	...
	rebuilt, err := requestForEntry(reopenEndpoint, entry)
	if err != nil {
...
func TestRecoveryRebuildsParkAndEdit(t *testing.T) {
	...
	aEndpoint, _ := fakeGoalEndpoint(t)
	a := aEndpoint.Root
	if res, err := Open(verbReqFor(aEndpoint, "01J5X00000000000000000Q100", "mac-a"), "target", "Work.", "main", "Go."); err != nil || res.Outcome != OutcomeConfirmed {
		t.Fatalf("open: %+v %v", res, err)
	}
...
func claimedParkRecoveryFixture(t *testing.T, id string) (Endpoint, string) {
	t.Helper()
	endpoint, _ := fakeGoalEndpoint(t)
	root := endpoint.Root
	if result, err := Open(verbReqFor(endpoint, "01J5X00000000000000000PR00", "mac-a"), id, "Branch-backed work.", OriginMain, "Build u1."); err != nil || result.Outcome != OutcomeConfirmed {
		t.Fatalf("open: %+v %v", result, err)
	}
	if result, err := claimApprovedForTest(t, verbReqFor(endpoint, "01J5X00000000000000000PR01", "mac-a"), id, testBudget()); err != nil || result.Outcome != OutcomeConfirmed {
	...
	return endpoint, opid
}

func TestRecoveryRefusesParkWhenGoalBranchIsUnpushed(t *testing.T) {
	t.Parallel()
	endpoint, opid := claimedParkRecoveryFixture(t, "unpushed-park")
	root := endpoint.Root
	reports, err := RecoverWithPolicy(endpoint, fixtureParkRecoveryPolicy{check: func(goalID, next string) (string, error) {
...
func TestRecoveryCompletesParkWhenGoalBranchIsPushed(t *testing.T) {
	t.Parallel()
	endpoint, opid := claimedParkRecoveryFixture(t, "pushed-park")
	root := endpoint.Root
	summary := "goal/pushed-park last unit u1 commit " + strings.Repeat("a", 40) + " is read clean"
	if _, err := RecoverWithPolicy(endpoint, fixtureParkRecoveryPolicy{check: func(goalID, next string) (string, error) {
		return summary, nil
	}}); err != nil {
		t.Fatal(err)
	}
...
func TestParkFailsClosedWithoutBranchChecker(t *testing.T) {
	t.Parallel()
	endpoint, _ := fakeGoalEndpoint(t)
	if result, err := Open(verbReqFor(endpoint, "01J5X00000000000000000PN00", "mac-a"), "nil-check", "Work.", OriginMain, "Build."); err != nil || result.Outcome != OutcomeConfirmed {
		t.Fatalf("open: %+v %v", result, err)
	}
	request := verbReqFor(endpoint, "01J5X00000000000000000PN01", "mac-a")
...
func TestRecoveryReplaysOwnReleaseAndRefusesHumanRequiredStoppingActs(t *testing.T) {
	...
	if res, err := Open(verbReqFor(endpoint, "01J5X00000000000000000Q140", "mac-a"), "own-release", "Release after a crash.", OriginMain, "Continue."); err != nil || res.Outcome != OutcomeConfirmed {
		t.Fatalf("open own release: %+v %v", res, err)
	}
	claimReq := verbReqFor(endpoint, "01J5X00000000000000000Q141", "mac-a")
	...
	if res, err := Open(verbReqFor(endpoint, "01J5X00000000000000000Q151", "mac-a"), "recovered-human-park", "Recover the admitted human park.", OriginMain, "Wait."); err != nil || res.Outcome != OutcomeConfirmed {
		t.Fatalf("open recovered human park: %+v %v", res, err)
	}
	recoveredHumanParkTip := recoveryCanonicalTip(t, endpoint)
	acceptedBefore := acceptedTipForEndpoint(t, endpoint)
	...
	if afterTip := recoveryCanonicalTip(t, endpoint); afterTip != recoveredHumanParkTip {
	...
	assertHumanBoundaryRefusal := func(opid, verb, row, grade, beforeTip string) {
		t.Helper()
		acceptedBefore := acceptedTipForEndpoint(t, endpoint)
		reports, recoverErr := Recover(endpoint)
		if recoverErr != nil {
			t.Fatal(recoverErr)
		}
		...
		if afterTip := recoveryCanonicalTip(t, endpoint); afterTip != beforeTip {
	...
	if res, err := Open(verbReqFor(endpoint, "01J5X00000000000000000Q143", "mac-a"), "foreign-release", "Keep the foreign claim.", OriginMain, "Continue."); err != nil || res.Outcome != OutcomeConfirmed {
		t.Fatalf("open foreign release: %+v %v", res, err)
	}
	foreignClaim := verbReqFor(endpoint, "01J5X00000000000000000Q144", "mac-b")
	...
	foreignTip := recoveryCanonicalTip(t, endpoint)
	...
	if res, err := Open(verbReqFor(endpoint, "01J5X00000000000000000Q146", "mac-a"), "human-origin", "Human-reserved work.", OriginHuman, "Wait."); err != nil || res.Outcome != OutcomeConfirmed {
		t.Fatalf("open human-origin goal: %+v %v", res, err)
	}
	humanOriginTip := recoveryCanonicalTip(t, endpoint)
	...
	if res, err := Open(verbReqFor(endpoint, "01J5X00000000000000000Q148", "mac-a"), "human-park", "Keep the human pause.", OriginMain, "Wait."); err != nil || res.Outcome != OutcomeConfirmed {
		t.Fatalf("open human-park goal: %+v %v", res, err)
	}
	humanPark := verbReqFor(endpoint, "01J5X00000000000000000Q149", "mac-a")
	...
	humanParkTip := recoveryCanonicalTip(t, endpoint)
...
func TestRecoveryNamedStoppingIntentsPreserveTypedOutcomes(t *testing.T) {
	...
	t.Run("already applied park confirms", func(t *testing.T) {
		...
		endpoint, _ := fakeGoalEndpoint(t)
		root := endpoint.Root
		if res, err := Open(verbReqFor(endpoint, "01J5X00000000000000000Q154", "mac-a"), "landed-human-park", "The delayed human park.", OriginMain, "Wait."); err != nil || res.Outcome != OutcomeConfirmed {
			t.Fatalf("open: %+v %v", res, err)
		}
		park := verbReqFor(endpoint, "01J5X00000000000000000Q155", "mac-a")
		...
		beforeTip := recoveryCanonicalTip(t, endpoint)
		acceptedBefore := acceptedTipForEndpoint(t, endpoint)
		detail, err := completeFromIntent(endpoint, entry, nil)
	...
	t.Run("release lost to competitor stays lost", func(t *testing.T) {
		...
		req, err := requestForEntry(endpoint, entry)
		if err != nil {
			t.Fatal(err)
		}
		...
		beforeTip := recoveryCanonicalTip(t, endpoint)
		acceptedBefore := acceptedTipForEndpoint(t, endpoint)
		result, err := runTransaction(endpoint, req)
...
func TestRecoveryConfirmsASightedPushWithoutRebuilding(t *testing.T) {
	...
	res, err := Open(verbReqFor(aEndpoint, "01J5X00000000000000000Q200", "mac-a"), "landed", "Landed work.", "main", "Go.")
	...
	tipBefore := recoveryCanonicalTip(t, aEndpoint)
	acceptedBefore := acceptedTipForEndpoint(t, aEndpoint)
	reports, err := Recover(aEndpoint)
	if err != nil {
		t.Fatal(err)
	}
	...
	if tipAfter := recoveryCanonicalTip(t, aEndpoint); tipAfter != tipBefore {
...
func TestRecoveryHandlesOwnEntriesAndDoneRebuild(t *testing.T) {
	...
	if res, err := Open(verbReqFor(aEndpoint, "01J5X00000000000000000Q320", "mac-a"), "finish-me", "Work.", "main", "Go."); err != nil || res.Outcome != OutcomeConfirmed {
		t.Fatalf("open: %+v %v", res, err)
	}
...
func TestRecoveryRunsTheRealVerbSemanticsAcrossAnArc(t *testing.T) {
	...
	a, b := aEndpoint.Root, bEndpoint.Root
	...
	for i, id := range []string{"rv-one", "rv-two"} {
		...
		if res, err := Open(verbReqFor(aEndpoint, ulid, "mac-a"), id, "Arc "+id, "main", "Go."); err != nil || res.Outcome != OutcomeConfirmed {
			t.Fatalf("open %s: %+v %v", id, res, err)
		}
		if res, err := SetArc(verbReqFor(aEndpoint, ulid[:len(ulid)-1]+"5", "mac-a"), id, "rv-arc"); err != nil || res.Outcome != OutcomeConfirmed {
	...
	canonicalTip := recoveryCanonicalTip(t, aEndpoint)
	if aAccepted, bAccepted := acceptedTipForEndpoint(t, aEndpoint), acceptedTipForEndpoint(t, bEndpoint); aAccepted != canonicalTip || bAccepted == canonicalTip {
		t.Fatalf("the two clients lost their distinct accepted tips: canonical=%s a=%s b=%s", canonicalTip, aAccepted, bAccepted)
	}
	...
	if after := acceptedTipForEndpoint(t, bEndpoint); after != canonicalTip {
		t.Fatalf("the second client's fresh projection did not advance its accepted tip: canonical=%s accepted=%s", canonicalTip, after)
	}
...
func TestRecoveryRefusesAStrandedOriginRewrite(t *testing.T) {
	...
	aEndpoint, _ := fakeGoalEndpoint(t)
	a := aEndpoint.Root
	if res, err := Open(verbReqFor(aEndpoint, "01J5X00000000000000000Q200", "mac-a"), "prov", "Work.", "main", "Go."); err != nil || res.Outcome != OutcomeConfirmed {
		t.Fatalf("open: %+v %v", res, err)
	}
...
func TestRecoveryRefusesJournaledHumanDone(t *testing.T) {
	...
	aEndpoint, _ := fakeGoalEndpoint(t)
	a := aEndpoint.Root
	if res, err := Open(verbReqFor(aEndpoint, "01J5X00000000000000000Q300", "mac-a"), "hand-held", "Work.", OriginMain, "Go."); err != nil || res.Outcome != OutcomeConfirmed {
		t.Fatalf("open: %+v %v", res, err)
	}
	parkReq := verbReqFor(aEndpoint, "01J5X00000000000000000Q310", "mac-a")
	if res, err := Park(parkReq, "hand-held", "waiting on review"); err != nil || res.Outcome != OutcomeConfirmed {
		t.Fatalf("park: %+v %v", res, err)
	}
	doneReq := verbReqFor(aEndpoint, "01J5X00000000000000000Q320", "mac-a")
	doneReq.Actor.Human = "Wido"
	storedDone := doneRequest(doneReq, "hand-held", "Journal text must not conclude this goal.")
	beforeTip := recoveryCanonicalTip(t, aEndpoint)
	acceptedBefore := acceptedTipForEndpoint(t, aEndpoint)
	...
	if afterTip := recoveryCanonicalTip(t, aEndpoint); afterTip != beforeTip {
...
func TestRecoveryCompletesMainSplitAndRejectsHumanOrDoctoredDrafts(t *testing.T) {
	...
	t.Run("created main split completes", func(t *testing.T) {
		...
		endpoint, _ := fakeGoalEndpoint(t)
		root := endpoint.Root
		if res, err := Open(verbReqFor(endpoint, "01J5X00000000000000000RM00", "mac-a"), "recover-main-split", "Recover the split.", OriginMain, "Split it."); err != nil || res.Outcome != OutcomeConfirmed {
			t.Fatalf("open: %+v %v", res, err)
		}
	...
	t.Run("human ratification remains fresh authority", func(t *testing.T) {
		...
		endpoint, _ := fakeGoalEndpoint(t)
		root := endpoint.Root
		if res, err := Open(verbReqFor(endpoint, "01J5X00000000000000000RH00", "mac-a"), "recover-human-split", "Human-ratified split.", OriginMain, "Split it."); err != nil || res.Outcome != OutcomeConfirmed {
			t.Fatalf("open: %+v %v", res, err)
		}
		members := testMembers("recover-human-split")
		request := verbReqFor(endpoint, "01J5X00000000000000000RH10", "mac-a")
	...
	t.Run("doctored stored members fail their digest", func(t *testing.T) {
		...
		endpoint, _ := fakeGoalEndpoint(t)
		root := endpoint.Root
		if res, err := Open(verbReqFor(endpoint, "01J5X00000000000000000RD00", "mac-a"), "recover-doctored", "Digest-bound split.", OriginMain, "Split it."); err != nil || res.Outcome != OutcomeConfirmed {
			t.Fatalf("open: %+v %v", res, err)
		}
		members := testMembers("recover-doctored")
		request := verbReqFor(endpoint, "01J5X00000000000000000RD10", "mac-a")
...
func TestRecoveryClassifiesOldArcDebtAfterDecomposedParentWasPruned(t *testing.T) {
	...
	endpoint, _ := fakeGoalEndpoint(t)
	root := endpoint.Root
	if res, err := Open(verbReqFor(endpoint, "01J5X00000000000000000RP00", "mac-a"), "pruned-split", "Prune after split.", OriginMain, "Split it."); err != nil || res.Outcome != OutcomeConfirmed {
		t.Fatalf("open: %+v %v", res, err)
	}
	if res, err := SetArc(verbReqFor(endpoint, "01J5X00000000000000000RP10", "mac-a"), "pruned-split", "old-pruned-arc"); err != nil || res.Outcome != OutcomeConfirmed {
...
func TestRecoveryCompletesADeadOwnersBlockerOpen(t *testing.T) {
	...
	aEndpoint, _ := fakeGoalEndpoint(t)
	a := aEndpoint.Root
	if res, err := Open(verbReqFor(aEndpoint, "01J5X00000000000000000Q100", "mac-a"), "held", "Work the blocker holds.", OriginHuman, "Wait."); err != nil || res.Outcome != OutcomeConfirmed {
		t.Fatalf("open: %+v %v", res, err)
	}
	if res, err := claimApprovedForTest(t, verbReqFor(aEndpoint, "01J5X00000000000000000Q101", "mac-a"), "held", testBudget()); err != nil || res.Outcome != OutcomeConfirmed {
