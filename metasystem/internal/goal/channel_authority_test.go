...
func TestAskedAppendsTheAskedMarkerOnce(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if result, err := Open(verbReqFor(endpoint, "01J5X0000000000000000000D0", "mac-a"), "asked", "Ask something.", "main", test.initial); err != nil || result.Outcome != OutcomeConfirmed {
				t.Fatalf("open: %+v %v", result, err)
			}
			req := verbReqFor(endpoint, "01J5X0000000000000000000D1", "mac-a")
			...
			if result, err := Asked(verbReqFor(endpoint, "01J5X0000000000000000000D2", "mac-a"), "missing", "question-b", "other", "Fact."); err != nil || result.Outcome != OutcomeRejected || !strings.Contains(result.Detail, "not live") {
...
func TestAnswerRecordsAuthenticatedChannelWordOnce(t *testing.T) {
	t.Parallel()
	endpoint, _ := fakeGoalEndpoint(t)
	if result, err := Open(verbReqFor(endpoint, "01J5X0000000000000000000E0", "mac-a"), "answered", "Record the answer.", "main", "Wait."); err != nil || result.Outcome != OutcomeConfirmed {
		t.Fatalf("open: %+v %v", result, err)
	}
	req := verbReqFor(endpoint, "01J5X0000000000000000000E1", "mac-a")
...
func TestAuthenticatedChannelApprovalRequiresTheTokenOnce(t *testing.T) {
	...
	t.Run("lookup and strict token", func(t *testing.T) {
		t.Parallel()
		endpoint, _ := fakeGoalEndpoint(t)
		if result, err := Open(verbReqFor(endpoint, "01J5X0000000000000000000F0", "mac-a"), "authority", "Authorize an act.", "main", "Wait."); err != nil || result.Outcome != OutcomeConfirmed {
			t.Fatalf("open: %+v %v", result, err)
		}
		now := verbReqFor(endpoint, "01J5X0000000000000000000F1", "mac-a").Now
		if _, err := authenticatedChannelApprovalForEndpoint(endpoint, "missing", "op", "token", now); err == nil || !strings.Contains(err.Error(), "not live") {
			t.Fatalf("missing goal was not refused by name: %v", err)
		}
		if _, err := authenticatedChannelApprovalForEndpoint(endpoint, "authority", "not-an-answer", "token", now); err == nil || !strings.Contains(err.Error(), "not an authenticated channel word") {
			t.Fatalf("non-answer was not refused: %v", err)
		}
		proof := AnswerProof{Provider: "slack", User: "UWIDO", Ref: "1/2", Step: 42}
		answerReq := verbReqFor(endpoint, "01J5X0000000000000000000F2", "mac-a")
		...
		if _, err := authenticatedChannelApprovalForEndpoint(endpoint, "authority", opid, token, now); err == nil || !strings.Contains(err.Error(), "required token") {
			t.Fatalf("missing token was accepted: %v", err)
		}
		carryingReq := verbReqFor(endpoint, "01J5X0000000000000000000F3", "mac-a")
		...
		got, err := authenticatedChannelApprovalForEndpoint(endpoint, "authority", carryingReq.opid(), token, now)
	...
	t.Run("resume uses standing approval without consuming an answer", func(t *testing.T) {
		...
		if result, err := Open(verbReqFor(endpoint, "01J5X0000000000000000000G0", "mac-a"), "resume-goal", "Resume once.", "main", "Wait."); err != nil || result.Outcome != OutcomeConfirmed {
			t.Fatalf("open: %+v %v", result, err)
		}
		approveGoalForTest(t, verbReqFor(endpoint, "01J5X0000000000000000000G5", "mac-a"), "resume-goal", budget)
		answerReq := verbReqFor(endpoint, "01J5X0000000000000000000G1", "mac-a")
		...
		claim := verbReqFor(endpoint, "01J5X0000000000000000000G2", "mac-a")
		claim.ClaimEpoch = 9
		...
		resumeReq := verbReqFor(endpoint, "01J5X0000000000000000000G4", "mac-a")
		...
		if _, err := authenticatedChannelApprovalForEndpoint(endpoint, "resume-goal", answerReq.opid(), token, resumeReq.Now); err != nil {
	...
	t.Run("set-obligation consumes the answer", func(t *testing.T) {
		t.Parallel()
		endpoint := channelObligationEndpoint(t, "obligation-goal")
		root := endpoint.Root
		token := SetObligationApprovalToken("obligation-goal", ObligationDraft, "Wido")
		answerReq := verbReqFor(endpoint, "01J5X0000000000000000000H0", "mac-a")
		...
		request := verbReqFor(endpoint, "01J5X0000000000000000000H1", "mac-a")
		...
		if _, err := authenticatedChannelApprovalForEndpoint(endpoint, "obligation-goal", answerReq.opid(), token, request.Now); err == nil || !strings.Contains(err.Error(), "already consumed by set-obligation") {
			t.Fatalf("consumed set-obligation approval was reusable: %v", err)
		}
	})
}

func channelObligationEndpoint(t *testing.T, id string) Endpoint {
	t.Helper()
	endpoint, client := fakeGoalEndpoint(t)
	endpoint.Remote, endpoint.Branch = SyncLocal, LocalLedgerBranch
	if err := os.WriteFile(endpoint.Root+"/metasystem.conf", []byte("metasystem.governance.correlation-policy=\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	openedAt, claimAt := "2026-08-30T08:00:00Z", "2026-08-30T08:05:00Z"
	...
	seed.files = vTree(root, []*GoalFile{file}, nil)
