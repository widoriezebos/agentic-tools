...
func TestSTR4R1RaiseTransaction(t *testing.T) {
	...
	if result, err := OpenRisked(opened, "risk-raise", "Raise rigor without erasing control state.", OriginHuman, "Exercise the raise.", nil, nil, low, 0, "", &budget, nil); err != nil || result.Outcome != OutcomeConfirmed {
	...
	if opened, err := OpenRisked(preserveOpen, "preserve-override", "Preserve an existing override.", OriginHuman, "Raise derivation.", nil, nil, preserveLow, 3, "standing full review", &budget, nil); err != nil || opened.Outcome != OutcomeConfirmed {
...
func TestSTR4R1FourDowngradesRefused(t *testing.T) {
	...
	for index, test := range cases {
		...
		if result, err := OpenRisked(req, test.id, "Exercise "+test.id+".", OriginHuman, "Edit it.", nil, nil, test.initial, test.tier, test.why, &budget, nil); err != nil || result.Outcome != OutcomeConfirmed {
...
func TestRiskOverridesAboveAndBelow(t *testing.T) {
	...
	result, err := OpenRisked(pair, "override-above", "Record a conservative override.", OriginHuman, "Review it.", nil, nil, low, 2, "independent review requested", &budget, nil)
	if err != nil || result.Outcome != OutcomeConfirmed {
	...
	if _, err := OpenRisked(obligationAuthorityVerbReq(root, "01J5X00000000000000000RB10", "mac-a"), "override-below-pair", "Refuse an unsafe override.", OriginHuman, "Refuse it.", nil, nil, high, 2, "pair asks lower", &budget, nil); err == nil || !strings.Contains(err.Error(), "human act") {
...
func TestSTR4R1FiveMemberExceptions(t *testing.T) {
	...
	if result, err := OpenRisked(opened, "budget-exceptions", "Count every over-box member.", OriginHuman, "Raise two members.", nil, nil, low, 0, "", &box, nil); err != nil || result.Outcome != OutcomeConfirmed {
...
func TestRestatedAnswersNeverLowerARecordedTier(t *testing.T) {
	...
	if res, err := OpenRisked(seat, "wide", "Wide but routine.", OriginHuman, "Edit it.", nil, nil, exposed, 3, "the earlier formula", &budget, nil); err != nil || res.Outcome != OutcomeConfirmed {
