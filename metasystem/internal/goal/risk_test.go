...
func TestRiskOverridesAboveAndBelow(t *testing.T) {
	...
	if _, err := OpenRisked(verbReqFor(endpoint, "01J5X00000000000000000RB10", "mac-a"), "override-below-pair", "Refuse an unsafe override.", OriginHuman, "Refuse it.", "", high, 2, "pair asks lower", &budget, nil); err == nil || !strings.Contains(err.Error(), "human act") {
