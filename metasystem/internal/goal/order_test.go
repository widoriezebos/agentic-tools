...
func TestPriorityRace(t *testing.T) {
	...
	t.Run("same-position", func(t *testing.T) {
		...
		held.BeforePush = func(attempt int) error {
			...
			injected = true
			result, err := Publish(b, setPriorityRequest(requestB, "e", 1, sequencePointer(1)))
			if err != nil || result.Outcome != OutcomeConfirmed {
	...
	t.Run("same-target", func(t *testing.T) {
		...
		held.BeforePush = func(int) error {
			...
			injected = true
			result, err := Publish(b, setPriorityRequest(requestB, "target", 1, sequencePointer(1)))
			if err != nil || result.Outcome != OutcomeConfirmed {
	...
	t.Run("priority-first", func(t *testing.T) {
		...
		held.BeforePush = func(attempt int) error {
			...
			injected = true
			result, err := Publish(a, setPriorityRequest(rankRequest, "target", 1, sequencePointer(1)))
			if err != nil || result.Outcome != OutcomeConfirmed {
