...
func TestUniverseNamesAndCounts(t *testing.T) {
	if ProcessUniverseForeign.String() != "foreign" ||
		ProcessUniverseExcludedByAge.String() != "excluded-by-age" ||
		ProcessUniverseSignalable.String() != "signalable" {
		t.Fatal("universe names drifted")
	}
	census := TaggedProcessCensus{Indeterminate: []IndeterminateProcess{
		{PID: 1, Universe: ProcessUniverseForeign},
		{PID: 2, Universe: ProcessUniverseForeign},
		{PID: 3, Universe: ProcessUniverseExcludedByAge},
	}}
	if census.ForeignCount() != 2 || census.ExcludedByAgeCount() != 1 {
