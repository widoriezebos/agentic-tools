...
// four steps it names and no others. Arming mints an identity and spawns a
...
// bytes with evidence.root rewritten through the engine's own writer, and
// that the nickname reaches git configuration.
...
// mixedRunner runs git for real and answers for the engine, which is not
...
func (r *mixedRunner) Run(command Command) (string, error) {
	...
	return r.said[commandKey(command)], nil
}
...
func fixtureRepository(t *testing.T, bare, source string) {
	...
	run(t, source, "git", "config", "goal.sync-branch", "main")
