...
// The direct-invoker check (verification rounds 2–4): the classifier's
...
// whose refusal must cover "an agent invoked me itself" (KI-23's
// acknowledge) call these for the caller node, and every unprovable
...
// trustworthy silence (round 4, finding 1).
...
// an unconfigured runtime's binary is still an agent (round 3's
// residual: the configured-only universe let an unconfigured invoker
