...
func vendoredShape(t *testing.T, template bool) (repo, install string, resolver Resolver, top *topRecorder) {
	t.Helper()
	repo = t.TempDir()
...
func TestOwnerFourAnswersInVendoredAdoptedShape(t *testing.T) {
	t.Parallel()
	repo, install, resolver, top := vendoredShape(t, false)
	cases := []struct {
		path string
		want Ownership
	}{
	...
	for _, c := range cases {
		top.expectCanonical(install, repo, nil)
		got, mode, err := resolver.Owner(c.path)
		if err != nil || got != c.want || mode != "adopted" {
			t.Fatalf("Owner(%q) = %v %q %v; want %v adopted", c.path, got, mode, err, c.want)
		}
	}
	top.expectCanonical(install, repo, nil)
	if got, mode, err := resolver.Owner(filepath.Join(repo, "docs", "guide.md")); err != nil || got != OwnerApp || mode != "adopted" {
		t.Fatalf("absolute app path: %v %q %v", got, mode, err)
	}
	top.expectCanonical(install, repo, nil)
	if got, _, err := resolver.Owner("/no/such/outside"); err == nil || got != OwnerOutside {
		t.Fatalf("outside path accepted: %v %v", got, err)
	}
	top.expectCanonical(install, repo, nil)
	if got, _, err := resolver.Owner("../escape.txt"); err == nil || got != OwnerOutside {
...
func TestOwnerTemplateModeRidesAlong(t *testing.T) {
	t.Parallel()
	repo, install, resolver, top := vendoredShape(t, true)
	top.expectCanonical(install, repo, nil)
	got, mode, err := resolver.Owner("metasystem/memory/rulings.md")
...
func TestOwnerSymlinkJudgedByEntryPath(t *testing.T) {
	...
	top.expectCanonical(install, repo, nil)
	got, _, err := resolver.Owner("docs-link")
...
func TestOwnerUsesShippedInventoryInUnvendoredAdoptedShape(t *testing.T) {
	...
	resolver, top := resolverFixture(t, repo)
	...
	for _, test := range []struct {
		...
		got, mode, err := resolver.Owner(test.path)
