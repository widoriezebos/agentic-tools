...
# Review brief: goal records get an owning test surface (chain dispatch-cap-surface2-20260910)

FINDING IDS: chain-unique, DCS-04, DCS-05, ... never F-n. Report `round`
...
surface's static groups and a selection test. The same bounded policy
correction landed for internal/obligationstate as c0d5b136.

Scope: the computed diff of job dispatch-cap-surface2-20260910
(metasystem/testing.json and at most one test file in
internal/testpolicy). Contract:
...
1. The surface owns exactly the four path patterns, with the
   ...
   validator allows.
2. A records-only change now resolves as delivery with the static
   groups, proven by a selection test.
