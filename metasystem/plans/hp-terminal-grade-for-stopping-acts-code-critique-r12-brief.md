...
# Review brief: the terminal grade for stopping acts after the fourteenth round (chain hp-terminal-build1-20260909, reviewing round hp-terminal-build1-20260909-r14)

FINDING IDS: chain-unique, continue the chain's sequence at HPT-38,
never F-n and never a reused id. Report `round` as 1 in your return:
it is this job's own round. You are the twelfth critic on this chain.
...
Round twelve, from the ninth critic (hp-terminal-crit9-20260909):
HPT-28 (high) recovery replayed a human's park as a machine park (the
actor rebuilt without the journaled "by" name), after which any agent
could unpark it; HPT-29 the recovery refusal named no condition; HPT-30
the chain was behind main. Round twelve rebuilds the replayed actor
exactly as journaled (the write-ahead record of an admitted act),
appends the typed condition to the cannot-replay sentence, and is
rebased onto main with the register citations re-checked. Attack: a
journal entry whose "by" name was never admitted live (can an agent
write such an entry by any route?); the replayed human act writing
anything beyond what the live act would have; the rebase moving
anything the chain did not touch. Closing review: unless something
material remains, say so plainly so the chain lands.

Round thirteen, from the tenth critic (hp-terminal-crit10-20260909):
HPT-31 (high) round twelve had made journal text an authority
credential (a stored "by" name replayed into a human-attributed
record, and done's name-only gates widened). Round thirteen reverses
it: recovery never carries a human name; any entry whose intent stores
a "by" is refused for every verb with the cannot-replay sentence,
writing nothing; nameless entries replay as agent acts under round
eleven's conditions; the doctrine comment and the code agree (HPT-33).
HPT-32 (three command-package tests failing on unmodified main) is
recorded, not this chain's. Attack: any verb whose intent stores the
name under another key; a refusal that still writes a history line; a
nameless entry that the live verb would have required a human for;
the entry closed in a state that blocks later recovery. Closing
review: unless something material remains, say so plainly so the
chain lands.

Round fourteen, from the eleventh critic (hp-terminal-crit11-20260909):
HPT-34 (medium) round thirteen's recovery wrapper replaced the verb's
typed outcomes (already applied, lost to a competitor, nothing to do)
with the cannot-replay refusal, so a human-run act whose push landed
mid-recovery was recorded rejected while in fact applied; HPT-35 the
intentArgs comment still described name-based authority. Round
fourteen lets typed outcomes through and refuses a named entry only
where the mutation would have succeeded; the comment now states the
doctrine. HPT-36 (the blanket refusal reaching proof-free verbs) is
recorded on member goal hp-recovery-grants-no-grade-to-unstamped-intents;
HPT-37 (fixture branch chosen by lease classify) recorded. Attack: a
typed outcome that also wrote something; the pass-through admitting a
named entry whose mutation succeeded. Closing review: unless
...
Scope: the computed diff of round hp-terminal-build1-20260909-r14 (the
...
hp-terminal-build1-20260909-r14; if your sandbox cannot run it, read the
