...
| TD-2 | CRITICAL | Phase 5 bullet 3 | The golden corpus is captured from the CURRENT writers before any conversion, per family, and every conversion checkpoint diffs bytes.Equal against it | `internal/dispatch`, `internal/missionrunner`, `internal/host` | `internal/dispatch/testdata/corpus/jobrecord/`, `internal/missionrunner/testdata/corpus/turnstate/`, `internal/host/testdata/corpus/hostresult/` | `corpus_capture_test.go` in each of the three packages, re-diffed every run | Not applicable: the corpus is committed evidence | DONE | all three families captured: dispatch (7, unescaped), missionrunner (4, escaped), host (3, unescaped) |
| TD-3 | CRITICAL | Phase 5 bullet 2 | The frozen grammar fixtures pin what current readers ACCEPT — duplicate keys, ill-typed known fields, null-vs-absent, trailing bytes — and new readers match verdict-for-verdict before writers convert | `internal/dispatch`, `internal/missionrunner`, `internal/wiredoc` | `internal/dispatch/grammar_freeze_test.go`, `internal/missionrunner/wiredoc_equivalence_test.go`, `internal/wiredoc/wiredoc_test.go` | `TestJobRecordGrammarFrozen`, `TestTurnStateGrammarFrozen`, and `TestDecodeGrammar` — verdict-for-verdict against old readers and the envelope decoder | Not applicable: fixture verdict equality | DONE | dispatch (10 verdicts) and missionrunner (5, incl. errNotJSONObject) frozen; host reads through the same owners |
...
| TD-5 | HIGH | Phase 5 staging | Leaves that no decision dereferences by field stay maps, dispositioned per family at its checkpoint in this file | `plans/typed-documents-design.md` | the Leaf dispositions section of `plans/typed-documents-design.md` | `internal/host` has no lens file and `git grep "field-read"` finds the recorded reasons — the dispositions are themselves the checkable artifact | Not applicable: same | DONE | None |
