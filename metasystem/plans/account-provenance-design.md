...
| codex 0.148.0 | `codex login status` prints "Logged in using ChatGPT" and nothing else; `--help` shows no output-format flag. The identity lives in `${CODEX_HOME:-~/.codex}/auth.json`: top-level keys `OPENAI_API_KEY`, `auth_mode` (observed `chatgpt`), `last_refresh`, `tokens`; `tokens` keys `access_token`, `account_id`, `id_token`, `refresh_token`. The `id_token` payload decodes to claims including `iss` (`https://auth.openai.com`), `aud`, `exp`, `iat`, `email`, `email_verified`, `sub`, and the object `https://api.openai.com/auth` with `chatgpt_account_id`, `chatgpt_plan_type`, `chatgpt_user_id`, `organizations`, `user_id`. | `credential-claims` as defined below, `surface` = `codex-login-status+auth-file`, `accountId` = `chatgpt_account_id`, `accountLabel` = `email`. Login status nonzero: `not-logged-in`. File unreadable: `credential-unreadable`. `auth_mode` other than `chatgpt`: `api-key-mode`. Identity payload not decodable, or `iss` not `https://auth.openai.com`: `credential-invalid`. Expiry is read from the ACCESS token's `exp` claim, decoded the same way, never from the identity token's: the identity token is issued with a one-hour lifetime and is not reissued when the CLI refreshes its access token (observed live on this Mac 2026-09-06 by the closing build critic: an identity token 7.3 days past its `exp` beside an access token still in date and a CLI reporting a valid login), so its age says nothing about the login. Access token not decodable or without `exp`: `credential-invalid`. Access token `exp` before the engine's now: `credential-expired`. Claims present but either identity claim empty: `identity-incomplete`. |
...
- **Codex claims fixture, Go, `internal/adapter/codexaccount_test.go`:**
  synthetic `auth.json` files with unsigned identity and access tokens: chatgpt mode with valid claims gives `credential-claims`; `auth_mode` of `apikey` gives `api-key-mode`; an access token past its `exp` gives `credential-expired`; an identity token days past its own `exp` beside an access token in date gives `credential-claims`; an access token without a decodable `exp` gives `credential-invalid`; wrong issuer gives `credential-invalid`; missing email gives `identity-incomplete`;
...
| APC-01 (closing build critique, 2026-09-06) | The codex expiry check reads the access token's `exp`, never the identity token's, whose one-hour lifetime and non-reissue would have made every codex capture `credential-expired`; the claims fixture gains the stale-identity-token and no-expiry cases. |
