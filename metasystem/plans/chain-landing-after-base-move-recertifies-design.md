...
| `metasystem/internal/goal/verbs.go:1244` and `:1256`, `metasystem/scripts/agents/land.sh:204` | Goal parking requires human authority for human-origin goals and clears claim binding. The landing driver's failed-step exit persists no park. The new landing-attempt record below is therefore necessary. |
| `metasystem/internal/landing/receipt.go:55` and `:298`, `metasystem/internal/landing/observe.go:139` | Receipts admit any nonempty command; only full-width chains currently enforce command identity. Area-width recertification must bind an explicitly supplied command. |
...
For an area-width chain this invocation additionally requires `--test-command <command>`, whose selection and binding are specified under the two gates below. It has no inferred default.
...
- `internal/landing` owns candidate binding and the durable landing-attempt park described below, implemented in its new `park.go`. The CLI and the two landing scripts carry the explicit record reference and stop transport from changing a proved result. They do not implement overlap or park-record policy in Bash.
...
For a literal path changed on only one side, copy that side's entire resulting entry, including deletion, mode, and blob. For a path changed on both sides, proof is available only when all three entries exist, have the same regular-file mode (`100644` or `100755`), and all three blobs pass `gittree.IsRecertifiableText`. This single predicate, owned by `disjoint_merge.go`, requires `utf8.Valid(blob)` and forbids code points U+0000–U+001F except TAB (U+0009), LF (U+000A), and CR (U+000D), and all of U+007F–U+009F. Empty content passes. No decoding replacement, normalization, MIME detection, extension, attribute, or Git binary heuristic participates. “Supported text” means exactly this predicate; “non-text” or “binary” refusal means failing it. The producer, proof recomputation, and content canary use this same function before invoking Git. In particular, newline-separated FF/FE bytes without NUL fail; `--text` cannot admit them. A both-sided mode change, addition, deletion, symlink, gitlink, non-text blob, or rename endpoint is unsupported. Identical both-sided edits are not deduplicated into an exemption.
...
Run blob comparisons outside attribute-bearing worktrees, with system/global Git configuration disabled and no repository-local diff configuration or attributes loaded, and with `LC_ALL=C` and `LANG=C` so marker bytes are fixed. In particular, local `diff.algorithm`, indent heuristics, whitespace-ignore settings, external drivers, and text conversion must not select different hunks. Record the Git version and proof version. The canonical invocation defines the hunks; implementations must not substitute another diff library or enlarge them by context. Consumption recomputes them. A different Git version may consume a proof only if recomputation reproduces its canonical hunk manifest and merged tree; otherwise refuse as unproven, without silently reissuing evidence.

Accept hunk headers with byte grammar `@@ -u[,n] +v[,m] @@[ section]\n`: coordinates are ASCII decimal integers, omitted counts default to one, and the optional section suffix is one ASCII space followed by any bytes up to the terminating LF. Ignore that suffix entirely when extracting coordinates or building the manifest. Express each edit using **base line-boundary coordinates**: a nonempty old range consumes `[u-1, u-1+n)` and produces the indicated new-side lines; a zero-count old range inserts at boundary `u`.

Git terminates every displayed hunk payload line with LF even when the blob's final line lacks one. The exact marker line `\ No newline at end of file\n` immediately following a `-`, `+`, or context-space payload line removes that display LF from that payload on the old side, new side, or both sides respectively. The marker is not payload and consumes neither old nor new count. It is valid only once after an eligible payload line and only if that line is the final line of its affected blob side; a misplaced, repeated, or unknown backslash marker is malformed output. All other payload LFs remain literal bytes; CR is never stripped. Validate counts and ranges and require reconstruction of each side from its base and parsed edits to reproduce that side's blob exactly, including final-newline changes.

Authoring probe, run in an empty temporary directory with the listed options and configuration/locale isolation: Git `2.50.1 (Apple Git-155)` printed `@@ -1,0 +2 @@ alpha` for insertion after `alpha`, and printed `@@ -3 +3 @@ beta`, `-last`, a no-final-newline marker, `+LAST`, and another marker for an unterminated last-line replacement. These are legal forms the parser must accept, not malformed-output refusals.
...
- `gateWidth` and `testCommand`, selected by the rule below. Store the exact command bytes, including whitespace.
- `inputDigest` and `recordDigest`. Use the repository's canonical `wiredoc` encoding and SHA-256. `inputDigest` covers the immutable source references/hashes, `H`, `A`, `B`, `R`, `T`, `Ttree`, proof kind, `gateWidth`, and `testCommand`. `recordDigest` covers all record fields except itself. A hash detects inconsistency; it does not replace the recomputed proof.
...
The read-only verifier in `internal/validate` loads the referenced original evidence, rechecks its closure and hashes, verifies `B + P = R`, recomputes the no-overlap merge and manifest, and verifies `Ttree + Q = M`. A recomputed merged tree differing from recorded `M` yields `chain-recertification-unproven` with structured `detail=merged-tree-mismatch`, even if every recorded hash and the stored patch equation are consistent. It recomputes the conformance boundary as well; a JSON `pass` field is never authority. It needs retained objects and source evidence, not a still-present delegate worktree. Both merge conformance and landing consume this one verifier.
...
A recertified landing requires a fresh existing-format test receipt for the **actual `C`**, after permitted carriage is added. The old receipt for `R`, or one for bare `M` when `C` differs, is invalid. Resolve gate width from the root chain using landing's existing rule: absent/null means `area`; only `area` and `full` are legal. For `area`, the authorized coordinator must supply a non-whitespace-only `--test-command` at preparation, selected from that chain's accepted verification requirements. The command is an explicit coordinator input, not inferred from a prior receipt, transcript, filename, or successful exit. There is no default such as `true`; this mechanism binds the coordinator's selection and does not attempt to judge a shell command's adequacy. For `full`, select the existing `landing.fullBatteryCommand` value; an optional `--test-command` is accepted only when byte-identical to that value. The flag is illegal on other conformance stages. Missing/invalid selection refuses preparation as `chain-recertification-test-command-refused` before materialization.

Freeze the selected width and command in the record and both digests. On consumption, require the current root's width to equal the record's width and recheck the full-width constant when applicable; mismatch is `chain-recertification-test-command-refused`. Create the receipt with `metasystem landing test-receipt --root <integration-project-root> --tree <C> --command <record.testCommand>`. Its existing execution directory, tree-posture checks, and receipt ceiling are unchanged. Landing additionally requires `receipt.Command == record.testCommand` byte for byte. Area-width missing, unsuccessful, stale, or command-mismatched receipts use `chain-recertification-test-command-refused`; full-width receipt failures retain `chain-full-gate-refused`. These refusals are hard even in observation mode. A different area command requires a new preparation and a fresh receipt, not an edit to a published proof. This design does not make textual disjointness a substitute for execution evidence.
...
An unattended node makes one preparation attempt for one frozen target. It does not automatically pull and loop after a refusal. Its one durable move is the new **landing-attempt park**, owned by `internal/landing/park.go` and exposed as:

```text
metasystem landing park --root <integration-project-root> --chain <root-chain> --target <T> --reason <refusal-code> --detail <explanation>
```

The caller also passes `--recertification <record path>` when one was published, `--candidate-commit <commit>` when its local landing commit exists, and repeatable `--recovery-ref <refname>` for every source/result anchor named by the failed operation. The target is the frozen `T`, not a newly fetched tip. The unattended coordinator invokes this verb once for a refusal from preparation or merge conformance; `land.sh` invokes the same verb once for an explicit recertified-path refusal through commit or non-fast-forward push. Network-only transport failure retains the existing failed-transport behavior. Bash passes values; Go validates and writes the record.

This is a coordinator evidence write with the existing `holder-only` authority check, using real caller classification from `internal/lease`, never caller-supplied classification JSON. An unattended caller must be the authenticated integration-checkout lease holder. For an unattended goal-bound chain, its machine and lineage must also match the current claim of the root job's `goalId`; read that goal through the existing accepted-ledger resolver. Goal-free chains record a null goal. A proven human uses the existing human authority path without the agent claim-match restriction. Delegates cannot issue this record. Recheck authority under the publication lock. This records a stopped landing attempt only: it neither parks the goal nor clears its claim, stop capability, fences, approval, or obligations. Clearing the claim is wrong here because recovery still belongs to that claimant and a landing refusal grants no authority to retire a human reservation. Do not call `goal park`, mission parking, or a scheduler-state transition as a substitute. Existing mission-state handling remains the mission runner's responsibility.

Persist immutable schema-version-1 JSON at `metasystem/artifacts/agents/landing/parks/<chain>/<recordDigest>.json` (installation-relative in adopted repositories). Its fields are `schemaVersion` (integer 1), `state="parked"`, `chain`, nullable `goal`, authenticated `actor` (string `machine+lineage` for a holder, literal `human` for a classified human), `targetCommit`, `reason`, `detail`, `parkedAt` (UTC RFC3339Nano), nullable `recertification`, nullable `candidateCommit`, `recoveryRefs` (array of `{name, oid}` strings sorted by ref name, duplicates removed), and `recordDigest` (SHA-256 over canonical `wiredoc` bytes of all other fields). Fields other than the integer and array are strings or the stated nulls. Retain an invalid explicitly supplied proof's canonical repository-relative path as diagnostic data without requiring that proof to verify. If that path itself is unsafe or noncanonical, record it only in `detail` and set `recertification` to null. Validate the chain identifier, target object, optional commit, and recovery refs; no ref is moved. Reject symlinks or traversal in the park's output path. Use an owned temporary file, flush, and atomic publication under the existing per-chain lock; never overwrite different bytes. Retain these records in the same durable-mirror boundary as recertification evidence.

Only after publication and read-back verification does the verb return exit zero and `state=parked`, `reason=<code>`, `parkRecord=<path>`; the caller prints `PARKED` with that path and ends the failed landing nonzero. The persisted `state`, reason, target, time, and recovery references distinguish a deliberate park from a stalled or vanished process. Absence of a verified record never means parked. A park write has a ten-second ceiling including its lock wait, shortened by any remaining enclosing ceiling. Authority, validation, write, read-back, or timeout failure returns nonzero with `chain-recertification-park-failed`, the original refusal as `cause`, and the recording error; the caller prints `PARK-FAILED`, never `PARKED`, preserves the worktree/commit/anchors and any possibly published record path, and stops without a second park or landing attempt. It must report the recording failure instead of silently treating the diagnostic exit as a durable park.

The node may work another item only through existing claim rules; this park grants no claim transition. A later externally initiated retry must refresh the target, reconstruct the original admitted source posture if necessary, issue a new proof, create a receipt for the new candidate, and pass the evaluator again. Old park records remain historical evidence and authorize no retry. Neither `--no-verify`, observation-only acceptance of a failed proof, force pushes, nor a manually asserted merged tree is a recovery path.
...
| `chain-recertification-unproven` | Unsupported file shape, malformed/missing proof, unreadable Git objects, ambiguous source selection, or an operation that could not run. Use structured `detail=non-text-blob` for failure of `IsRecertifiableText`, `detail=unsupported-mode` for a both-sided mode refusal, and `detail=merged-tree-mismatch` when recomputation disagrees with recorded `M`; no inferred clean merge. |
...
| `chain-recertification-test-command-refused` | The required command selection, bound width, or area-width candidate receipt fails the explicit command contract. Correct the selection or produce the required candidate receipt; no guessed command. |
| `chain-recertification-park-failed` | Recording the terminal landing-attempt park failed. Carry the original refusal in `cause`; report an unconfirmed park and preserve recovery evidence without retry. |
...
Conformance, receipt failures other than the new area-width rule, warden, authorization, class, and goal-ownership failures retain their existing reasons. The unattended caller attempts the named landing park on them too; failure of that write follows `chain-recertification-park-failed` and does not recurse. A human-present coordinator can use existing lawful correction paths; this design supplies no special override.
...
Every refusal observation below starts from an otherwise passing state and changes only the named prerequisite. Assert the exact refusal code and, where specified, structured detail; assert bar c for landing refusals and `Mode=refuse` for the new hard refusals even with a valid observation-only promotion policy. Neither a generic nonzero exit nor “not bar a” suffices. Keep authority, source evidence, class declarations, index/worktree posture, and a freshly executed receipt for the actual candidate valid except where that particular prerequisite is under test. Restore the passing state between mutations; never accumulate defects from one subtest into another.

| Fixture and smallest run, from `metasystem/` | Required observation | Ceiling |
| --- | --- | --- |
| New `TestChainLandingRecertifiesAfterBaseMove` in `metasystem/cmd/metasystem/landing_verbs_test.go`; `go test ./cmd/metasystem -run '^TestChainLandingRecertifiesAfterBaseMove$' -count=1 -timeout=120s` | A real temporary Git repository and recorded implementation/code-critic chain exercise review creation, the new recertify command with an explicit area command, merge conformance, a candidate receipt, and `landing observe`. Main and chain change well-separated lines in one source file. Before recertification, select the ordinary chain path with all other admission checks satisfied: the same merged candidate yields `chain-output-mismatch`; afterward it contains both edits and passes bar a. The paired subtest edits a third line that the tiny source test does not inspect, executes the bound command to create a valid receipt for that tampered tree, and gets `chain-output-mismatch` with all proof inputs unchanged. The command, uncarried-path, target, and forged-proof subtests below isolate their own refusal reasons. | 120 seconds for the whole test; all subprocesses inherit the remaining bound. |
| New `TestDisjointMergeProof` in `metasystem/internal/gittree/disjoint_merge_test.go`; `go test ./internal/gittree -run '^TestDisjointMergeProof$' -count=1 -timeout=60s` | One compact table pins disjoint edits, a middle-of-file insertion whose real Git header has a section suffix, disjoint unterminated final-line changes with Git's marker, and repeated-line alignment under hostile diff configuration. Successful results equal explicit expected bytes, not the implementation's own patch output. Separate same-line, same-gap insertion, deletion-boundary insertion, and adjacent-consuming-hunk cases each require the typed overlap refusal mapped to `chain-recertification-overlap`, with both offending base ranges. Separate non-text and mode cases follow the exact observations below. No refusal case combines unsupported content or mode with overlapping ranges. | 60 seconds total, no polling or provider calls. |
| New `TestRecertifiedLandingParksOnOriginMove` in `metasystem/cmd/metasystem/landing_verbs_test.go`; `go test ./cmd/metasystem -run '^TestRecertifiedLandingParksOnOriginMove$' -count=1 -timeout=120s` | Drive the real landing script against owned local bare remotes with valid holder/goal-claim authority, proof, candidate receipt, and a passing pre-race evaluator result. Follow the existing landing fixture's reduced commit-wrapper technique solely for driver ordering. Advance origin only at the controlled push boundary. Require `chain-recertification-target-moved`, one verified landing park record with `state=parked`, that reason, frozen `T`, the retained candidate commit and proof path, and `PARKED` output naming it. Compare goal/claim bytes before/after: unchanged. Require unchanged retained local candidate, no rebase command, and no second push. The recording-failure subtest below drives the real park verb without another transport race. This proves transport handling, not a mocked certification verdict. | 120 seconds total, one induced race and one push attempt. |

The main canary uses a nested installation and an **area-width fixture chain**, so its candidate receipt runs only a tiny source test that checks both independently specified values. Pass that test's exact command via `--test-command`; use the production selection, hashing, receipt execution, and comparison path. It does not claim to exercise full-width execution. Give the closed critic a limit of three rounds and three consumed rounds. In the successful and refusal runs, compare those fields, original review/patch bytes, critic return bytes, and goal budget before/after. They must be unchanged. Include a main-only file outside the project to verify nested grafting. The following are bounded subtests of the three runs above, not separate scenario families:

- **Command selection and binding (CLB-03):** before successful preparation, omit only `--test-command` on the area chain and require `chain-recertification-test-command-refused` before any materialization. After the passing landing observation, execute a different successful command (`true`) to obtain an otherwise valid receipt for the identical candidate; keep the proof and bound command unchanged. Require the hard `chain-recertification-test-command-refused`, then restore the receipt by running the bound command. This tests command identity, not a failed test or stale tree.
- **Uncarried path:** add one ordinary source file outside `S` whose existing path class allows chain behavior, without a register-carriage declaration. Keep all certified entries exactly `M`, execute the bound command for this actual candidate, and require `chain-has-uncarried-paths`. An unclassified path, missing receipt, or goal-ownership failure is not this observation.
- **Changed target (CLB-05):** advance the integration HEAD from `T` to a new commit `T'` with exactly the same whole-repository tree, keeping the original record explicitly selected. Stage/materialize the same candidate `C` and run the bound receipt command again after the move. Source objects, proof bytes, hashes, candidate entries, path policy, and goal claim remain valid; only the target commit identity differs. Require the hard `chain-recertification-target-moved`. An old-path `chain-output-mismatch`, a receipt refusal, or an observation-only would-refuse does not satisfy the test.
- **Forged proof (CLB-05):** restore `T` and use an isolated copy of the prepared fixture. Fabricate `M*` by changing only that untested third line of a certified file in genuine `M`. Set recorded `mergedTree=M*`, construct a real patch `Q*` with `Apply(Ttree,Q*)=M*`, and update both the recorded merged whole-repository snapshot and its `/merged` anchor ref to the corresponding grafted `M*`. Preserve genuine `H`, `A`, `B`, `R`, `T`, all original review/critic evidence and hashes, and the canonical hunk manifest. Recompute `mergedPatchDigest`, `inputDigest`, and `recordDigest` with production encoding, and place all forged bytes at the canonical paths for those recomputed identifiers. Stage/materialize `C=M*` and execute the bound test to produce a fresh valid receipt. Assert the stored digests, paths, anchors, patch equation, and candidate binding are internally consistent before consumption. Require the hard `chain-recertification-unproven` with structured `detail=merged-tree-mismatch`: the independent merge from genuine inputs yields `M`, not `M*`. A self-hash, malformed-path, stale-receipt, or old-path rejection is a test failure. This simulates a record author recomputing hashes, not a corrupt byte flip.
- **Content and mode (CLB-02):** the non-text table row uses newline-delimited FF/FE bytes with no NUL and changes distant records on the two sides. All entries exist with mode `100644`; there is no overlap under the raw line coordinates. Assert the shared production `IsRecertifiableText` predicate rejects these literal bytes and the real merge operation returns the typed unsupported-content refusal mapped to `chain-recertification-unproven`, `detail=non-text-blob`. A separate mode row uses valid supported text and disjoint hunks, but changes one side's mode to `100755`; require the unsupported-mode refusal mapped to `chain-recertification-unproven`, `detail=unsupported-mode`. These must not pass by failing an unrelated shape or overlap check.
- **Park recording failure (CLB-01):** after the transport observation, invoke the real park verb once more with the same valid authority and captured refusal inputs in the owned fixture. Replace only its park-directory location with a regular-file obstruction, retaining the successful record separately for comparison. Require `chain-recertification-park-failed`, `cause=chain-recertification-target-moved`, and the directory-write error, with no confirmed-park output or new verified park record. Candidate commit, proof, recovery refs, and goal/claim bytes remain unchanged. No further push, race, or retry is introduced; this subtest shares the transport test's 120-second ceiling.
...
metasystem/internal/landing/park.go
...
There is no unresolved choice between review and proof in this revision. The historical-base limitation, conservative overlap boundary, and no-automatic-retry policy remain deliberate decisions. The five accepted findings are folded into the named production owners and canary observations above. This page closes at revision 2; build behind those fixtures and perform the required implementation critique. If implementation reveals a specification gap, stop and report it to Wido through the orchestrator; do not silently choose behavior or open a third design revision.
