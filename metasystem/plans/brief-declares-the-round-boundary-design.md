...
- Next step: revision 3, amended after unit 1b's read (seat rulings A1 to A5), is the implementation spec. Landed: units 1a (e461dfd1), 1b-i (fad3ace8), 1b-ii (314fb806), 2 (9c030bcb), 3a-i (300ecfc6, row R1) and 3a-ii (this landing, rows R2 and R3). Next, in order, each briefed to Codex with its own Boundary, Ceiling and proof rule, read by Opus, and landed after every section the test plan selects: unit 3b, then 3c, 4a (its backslash end-to-end test re-derived under rulings A1 and A4), 4b, 4c (first decide the count's handling of attributes and settings, section 'Carried to unit 4c'), 6a, 6b. builder-proves-each-rule-by-mutation U0 follows 3c.
