...
| CC-2 | Stop treating local green as sufficient: verify against the pushed state, and keep CI green as the gate of record | SUPERSEDED by the human's ruling 2026-08-05: the local suite is the gate. A green local run is sufficient to commit and push, and a red GitHub run never blocks work. CI stays as a second opinion worth reading, because a different machine has twice caught real portability defects, but it decides nothing |
