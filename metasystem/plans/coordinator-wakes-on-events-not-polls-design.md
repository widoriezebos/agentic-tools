...
the first rostered design read is folded here. The member goals of section 6 are then opened and built on the roster lane.
...
| Channel wait | `metasystem/cmd/metasystem/main.go:457`; `metasystem/cmd/metasystem/channel_verbs.go:199` | The channel family's wait verb reads a durable human-question record and polls the provider. A recorded answer ends it. Its deadline is unbounded by default. Under this design it registers a human-act wait on the question's goal with verb answer, takes the 24-hour default deadline and follows row 2 of the stop table. |
...
The `delegate --wait` wrapper in `metasystem/scripts/agents/dispatch.sh` keeps completed 0, failed 3, timeout 4,
vanished 5, unknown status 6, malformed 7 and cancelled 8. Job watch and run watch keep their existing exit mappings in the same way.
...
The seat runs one blocking wait command. Nothing is spawned to hold it. Extend the adapter contract in `metasystem/scripts/agents/adapters/runtime-common.sh:3` with wait-delivery, taking waitId, nonce, deadline and session reference.
At registration, Go asks once and records the answer in the row before publishing pending. Exit 0 must return one word: blocking or resume.
Exit 2 means the runtime declines both. Registration refuses with a named reason, grants no exemption and leaves today's watch commands available.
There is no third route. With blocking, the runtime keeps one foreground command open until its result or deadline.
Claude Code, Codex and Devin all run a shell command to completion today, so all three report blocking.
With resume, the runtime cannot keep that command open. The waiter still blocks in its own process.
If the seat is gone at the event, the waiter persists its result and asks the existing steward continuation launch to start the next turn. This is the launch used by the idle-with-backlog handoff in `metasystem/internal/steward/revive.go:288`.
The next turn's first command is `metasystem wait --resume WAIT-ID`. No turn is launched merely to check whether work finished.
The engine acknowledges delivery only by writing the successor session's announcement record naming waitId when that turn starts.
Persist the delivery request and that acknowledgement separately from the result, keyed by waitId and result identity. Submission is not acknowledgement.
Existing launch serialization prevents duplicate concurrent turns. Recheck lease ownership and stop fences before continuation.
For a replaced or human-stopped seat, retain the result for explicit recovery and launch no automatic turn. Retry delivery without redoing target work.
...
A native wake never chooses the delivery route. Without it, the same blocking command uses bounded record reads and records-based resume.
Post hoc event projection in `metasystem/internal/adapter/events.go:3` is not native wake support.
The steward's operator queue remains an optional notice channel, not the wait record or proof of model delivery.
...
| --- | --- | --- |
| 1 | A valid job, run, attempt or landing wait joins the seat's claimed goal. | It is work in flight. It satisfies the unwatched-work join for its target, suppresses open work whose signature equals the saved signature, and exempts idle-with-backlog exactly as a live delegate job does today. |
| 2 | A valid human-act wait covers its goal. | It suppresses only open work with the saved signature and that goal's waiting-on-human condition. It never exempts idle-with-backlog. Channel wait follows this row. |
...
Freeze refusal counters and signatures only while suppression applies. Unrelated unwatched work keeps its existing decision.
...
After a missed hint, the timer or immediate reread finds the durable event. The rows are the queue; no daemon or event journal is needed.
At session start, the supervision hook's `start` event calls the engine's `session start` verb. Without hooks, plain `report turn-verdict` does this scan.
It enumerates this checkout's rows whose owner lineage the new session succeeds under the lease records.
For each pending row, it prints one WAITING line with the exact `metasystem wait --resume WAIT-ID` command. The model issues it as its first act.
Only the resume delivery route above launches a model turn automatically. Enumeration itself launches nothing.
...
| --- | --- | --- |
| 1: one bounded wait | TestWaitJobTerminals in `metasystem/internal/dispatch/watch_test.go`; TestWaitRunTerminalsAndDeadline in `metasystem/internal/run/waiter_test.go` | Exercise every terminal, already-terminal entry, missing record and reused identifier. One command returns the typed result, without model polls. Assert schemaVersion 2, the pointer and refusal of a live same-key registration. Fake bed leg wait-job-run exercises the public verb. |
...
| 1: ledger events | TestWaitGoalLandingAndHumanAct in `metasystem/internal/goal/txn_test.go`; TestWaitGoalCursorHistory in `metasystem/internal/goal/attention_test.go`; TestWaitChannelAnswer in `metasystem/cmd/metasystem/channel_verbs_test.go` | A two-clone bed publishes a landing, approval, denial and unrelated commits. Match goal, verb and optional chain; reject local-only commits, wrong authority, lost history and rewinds. Bed leg wait-ledger covers local and remote endpoints, plus a durable question answer, its goal's human-act registration and the 24-hour default deadline. |
| 2: accelerator and fallback | TestWaitHintsOnlyTriggerReads in `metasystem/internal/run/waiter_test.go`; TestWaitDeliveryContract in `metasystem/internal/adapter/runtime_test.go` | Hint tests check each publication owner writes durably before hinting. Early, duplicate, false, wrong-nonce and dropped hints leave results unchanged; the timer remains bounded. Delivery tests check exact inputs, the two exit-0 words, exit-2 refusal and blocking for all three roster runtimes. Fake bed legs wait-native-hint and wait-no-native prove no intermediate model turn. The resume case accepts only the successor announcement naming waitId as acknowledgement. |
| 3: Stop honours pending | TestPendingWaitTurnVerdict in `metasystem/internal/goal/turnverdict_test.go`; TestPendingWaitIdleBacklog in `metasystem/internal/goal/turnverdict_idle_test.go` | Assert every stop-table row. Valid job, run, attempt and landing waits satisfy their joins and idle exemption. Human-act and channel waits never exempt idle backlog. Test unchanged signatures and counters, changed work returning 6, every failed eligibility check, unrelated work and unchanged fences. Bed legs wait-stop-fake and wait-stop-claude exercise hook and plain-command verdicts; Claude must reach an actual provider turn end. |
| 4: recovery replay | TestWaitRestartRecoveryReplay in `metasystem/internal/run/waiter_pair_test.go` | Crash before subscription, after registration, after source publication and after result persistence. Remove pipes and notifications. A successor sees WAITING and issues resume first. Assert pointer lookup and its missing-pointer error, resumedBy, duplicate resume 64, original cursor and deadline, one result and no stolen live wait or stale cleanup. Bed leg wait-restart repeats real process death and, for resume delivery, proves the successor announcement acknowledges the wait. |
...
The injected ports in section 5 let each member prove its own mechanism before later delivery integration.

| Member goal | Its DONE | Its fixtures from section 5 |
| --- | --- | --- |
| wait-verb-returns-on-recorded-events | The commands `metasystem wait --job`, `--run`, `--attempt` and `--goal` (landing and human-act) each return on the recorded event or bounded deadline with typed exits. The version-2 row and its pointer exist. Restart re-registers from rows alone through --resume. This owns clauses 1 and 4's record recovery. | The first three clause-1 rows, TestWaitRestartRecoveryReplay and the failure-bounds row; bed legs wait-job-run, wait-proof, wait-ledger and wait-bounds. |
| publication-owners-hint-the-waiter | The job transition owner, attempt terminal commit and confirmed ledger publication each hint after their durable write. Dropped or false hints change no result. Wrappers keep the public exit mappings of job watch, run watch and delegate --wait. This owns clause 2's engine half and compatibility. | The hint portions of fixture row 4, TestWaitHintsOnlyTriggerReads and TestWaitCompatibilityMappings; bed legs wait-native-hint, wait-no-native and wait-compatibility for those assertions. |
| stop-gate-honours-a-registered-wait | The stop table holds on the fake adapter and Claude. The adapter operation records blocking or resume for every roster runtime. A successor announcement acknowledges resume delivery. This owns clause 2's adapter half, clause 3 and clause 4's delivery half. | Both stop tests, TestWaitDeliveryContract and wait-restart with real process death and announcement acknowledgement; bed legs wait-stop-fake and wait-stop-claude, plus the delivery cases of wait-native-hint and wait-no-native. |
...
The third member changes `metasystem/scripts/agents/adapters/runtime-common.sh`, `metasystem/scripts/agents/adapters/claude.sh`, `metasystem/scripts/agents/adapters/codex.sh`, `metasystem/scripts/agents/adapters/devin.sh`,
...
`metasystem/internal/report`, `metasystem/internal/steward/revive.go` and `metasystem/scripts/agents/supervision-hook.sh`.
Each member includes its named test owners, bed scenarios and matching contract changes in `metasystem/testing.json`,
