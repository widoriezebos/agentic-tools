...
If the authoritative decision cannot be persisted, the observed work decision still stands: an observed idle-with-backlog condition still refuses the stop, uncounted, with its reason and command, and the refusal says the count could not be spent. Infrastructure alone never manufactures a refusal from a lost record.
...
A Codex delegate round has no native Stop event; its adapter (`metasystem/scripts/agents/adapters/codex.sh`) calls the gate at its turn boundary with the round's job coordinates and relays a committed refusal through the round's existing follow-up transport, and completes on an allowance. That is the second production boundary this goal proves.
...
The second, independent channel is the hook log: the gate appends one line per infrastructure condition to the log the hook already writes at `metasystem/scripts/agents/supervision-hook.sh:878` (canonical checkout, holder session and lease epoch, hook turn generation, deadline end, cause code, component, count: the same identity the version-2 record keys on), from the shell, with no engine and no record lock in the way, and `DrainStopIncidents` also reads that log's unacknowledged lines when the version-2 record is unreadable, so the steward hears of a condition whenever either channel was writable.
...
| stop-hosts-enforce-the-common-gate | DONE: the Claude native Stop hook of a seat and the adapter turn boundary of a Codex delegate round both call the one Go gate and enforce its committed decision, proven live on both. Carries clauses 1 to 4 across the two production boundaries. Fixtures: TestEveryRuntimeUsesStopDecision, the five fake replay legs, RealClaudeNativeStopReplay, RealCodexAdapterStopReplay. The managed-seat lifecycle for hosts without a native Stop is the follow-on goal managed-seats-run-under-the-common-gate, opened when a runtime needs it. |
| stop-refusals-under-ten-a-day | DONE: seven days of stop-verdict files and incident records on every seat show fewer than ten refusals per seat per day, and the count with its residual unconfirmed deliveries is recorded on this page for Wido's acceptance. Carries the umbrella's operational clause. |
