...
Six files are standing ledgers, not task-local evidence, and are exempt from delete-when-shipped: `receipts.log` (task receipts feeding the retro), `instruction-ledger.md` (instruction changes with expected effects and verdicts), `known-issues.md` (recorded-but-unscheduled defects, capability ceilings, and do-not-retry dead ends), `refactor-baseline` (last gate-accepted state), `frontier` (best-known improvement state), and `goals.md` (the goal ledger: the thread of intent, mutated only through the supported goal actions (`metasystem help human`), with `goals-accepted.json` as its accepted baseline beside it). They stay committed and current for the life of the project. The goal ledger is NOT an audit log — pruned goals leave no git history; the decisions documents remain the audit surface. A handoff plan's "Next step" stays the per-stream in-flight authority; the turn verdict is the one point reconciling it with the goal.
