...
| HIW-O15 | CRITICAL | slice-7 round-15 review | HEAD movement during a turn is accounted: only certified integrations may advance committed HEAD, and staged-only bytes cannot ship through a commit the snapshot never sees (the projection covers the worktree, not the index or HEAD motion) | wall turn lifecycle + `internal/gittree` | none yet | none yet | host commits staged bytes mid-turn on a VM target | MISSING | design the HEAD-accounting obligation; predates this slice |
| HIW-O16 | HIGH | slice-7 round-15 review | The HOST is fenced at the whole repository in nested checkouts exactly as delegates are: a sibling-path change on the host path surfaces as a wall violation or refusal, never invisibly | wall snapshot scope + admission | none yet (delegates: conformance.go refuseOutsideProjectChanges) | none yet (delegates: nested_conformance_test.go sibling witnesses) | host edits a sibling path on a nested VM target | MISSING | design the host-side repository fence with the O15 work |
