...
# Re-review brief: the system-login node, round 2 (chain enroll-login-build1; supersedes the round-2 brief landed at d515ebe8)
...
Re-report EACH of them under its own id: ELN-01 with `material` false
and evidence that the corrected test now passes and pins a real
assertion (that is how the register resolves it), or still material
with the evidence that it is not fixed. A finding you do not re-report
stays open in the register and blocks the chain's close.
