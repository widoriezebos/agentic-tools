...
- Goal and current status: revision 3 (narrowed to the buildable rules, section 11) with build spec 9b. Slice 0 landed 4fbd271c; slice 1 part 1a-i landed 20eecaf0; parts 1a-ii and 1b are not built.
...
- Next step: waiting on the Fable usage limit (2026-09-13 13:54Z): part 1a-i landed 20eecaf0. The Fable delegate that writes the build briefs for part 1a-ii (hazard close, merge gate and landing reading the closure, requiring the closure round to equal the last critic round; types to a stdlib-only internal/readsubject) and part 1b (REDUNDANT_READ and CONCURRENT_READ admission, durable events, three follow-up legs), carrying N3-1 to N3-7 from records/misc/critique-closes-slice1a-i-code-read.md, stopped on the Fable usage limit; design stays in the Fable lane, so the seat relaunches it when the limit resets. Slice 0 landed 4fbd271c.
