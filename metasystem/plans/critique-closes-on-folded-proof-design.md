...
- Next step: in flight, by design (2026-09-13 13:20Z): Codex fold round 3 of part 1a-i (the closure written in the one record write that sets chainClosed, after CloseCheck), then the packages, the fast gate and the dispatch bed through the engine, one Opus closing read, and landing; then part 1a-ii (the hazard close, merge gate and landing reading the closure, with the types moved to a stdlib-only internal/readsubject) and part 1b (REDUNDANT_READ and CONCURRENT_READ admission, durable events, the three follow-up legs) from Fable briefs. Slice 0 landed 4fbd271c.
