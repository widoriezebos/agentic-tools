...
Claims and lineage. Open, a required decision for U3c's design (SSTB-303): how claims and lineage authority move to the continuation. A claim keys on machine plus lineage (internal/goal/verbs.go:502-506, :968-969), so a second lineage on the same machine is refused; `HandoffBinding` and the stage.go intent carry no claimant; dispatch.sh clears the claim epoch and main id. Until that decision is built, the successor may not claim or land; `handoffGoalIsStillOwned` (handoff.go:54-66) is a check, not a transfer. The intent's `JobId` is the successor's job record; its custody names the handoff nonce and the binding's predecessor session, main id and pid; the successor's receipts carry the same seat. The chain closes at the reap (internal/steward/reap.go:33-57). The authority rules for cancelling a handoff (C1a, C1b of plans/coordinator-context-stays-under-budget-design.md) are unchanged.

The live predecessor after the confirmation. It is handed off: the gate marks the session (a `handedOff` nonce on the session state at the first Stop that sees the running signal, never before) and every later Stop of that session is allowed silently, whatever the backlog. That silent Stop must be an output the hook and the adapter accept: today the hook treats an empty display as unreadable (scripts/agents/supervision-hook.sh:2368) and internal/adapter/stopoutput.go:91-97 writes `systemMessage` on every allow, so U3c's design settles the output contract (SSTB-305). The idle-backlog, open-work, unwatched, uncertainty and goal-revision branches are skipped for a handed-off session; it is never refused on idle-backlog again. The `HandoffRecorded` closure reads live intents and consumed intents whose binding names the session, so the allowance survives consumption and the reap. It does no work. Its background waiters were ended by the handoff (below); their exit wakes it once, and that turn makes one call and stops. It stays in the pane as the seat's human console: a human message costs one turn; the predecessor answers in one line naming the successor's job id and relays the text through the inbox of U7. Whether a headless session receives a `SendMessage` is unverified; the inbox does not depend on it, which is one reason open question 3 recommends U7.
...
| each seat, full working day, subagent files included (a forecast, not a proof threshold, SSTB-307) | at most 150M, derived per seat before the proof day | m1e 174M; 149M | m1e 389.2M to 12:51 |
...
| stop-hook refusals per seat-day, `context-over-trigger` excluded (a forecast, not a proof threshold, SSTB-307) | at most 10, derived per seat before the proof day | two per idle digest, a digest per landing or claim: at most 10 at 4 idle conditions | 41, 7, 55 |
...
|---|---|---|---|---|---|
| U6a | seats-spend-tokens-in-bounded-sessions | docs/orchestration.md carries S1 to S6 in their today forms, with a document-reading witness | about 60 | first; none | none; this page settles the rules |
| U3a | coordinator-context-stays-under-budget, amendment 8d | the two config keys; the hook passes `transcript_path` and the runtime; the sample in the verdict; the `context-over-trigger` block ahead of idle and open-work blocks; the gate's own staging at the second block | about 400, split | after U6a; none | how the block shares the health cursor; the staging seam's shape beside `PrepareIdleContinuation`; the sentence text |
| U3e | coordinator-context-stays-under-budget, 8d (open question 4, answer b only) | a tool-use hook that reads the last call's usage and denies any tool call but the handoff, a revival or a delegate launch over the trigger | about 120 | after U3a | the hook's per-call cost; the allowlist of tool calls over the trigger; the deny text |
| U3b | coordinator-context-stays-under-budget, 8d | the handoff ends in-flight waits and records `openWork` with each row's complete selector, deadline left and child identity | about 300 | after U3a | the signal seam; the 32 KB state bound with full selectors and the manifest overflow; a `registering` row's grace |
| U3c | coordinator-context-stays-under-budget, 8d | the automatic successor: the two holds and the `others` exclusion in handoff.go, the verb's immediate revival and 30 second confirmation, `steward status` and the verb's confirmation lines, the brief and the role sentence, `context resume` (verify, then fresh registration of every `openWork` row), the handed-off allowance that survives consumption, the SessionStart notice | about 500, split | after U3b and U1: fresh registration needs U1's authenticated-session registration (SSTB-308) | what `LiveHandoffForSession` returns after consumption; the session marker; the relay line for a human message; SSTB-301: the hook contract for a headless continuation, which must receive a Stop trigger and record its own next handoff (scripts/agents/supervision-hook.sh:1451-1475, :1581-1586; internal/adapter/claude.go:122-131); SSTB-305: the predecessor's silent Stop as an output the hook and the adapter accept (scripts/agents/supervision-hook.sh:2368; internal/adapter/stopoutput.go:91-97); SSTB-310: the pid and job exclusions with identities, not counts (internal/steward/census.go `workersFromVerdict`; internal/steward/verdict.go:47-64; internal/steward/revive.go:280-285); SSTB-303, a required decision: how claims and lineage authority move to the continuation (internal/goal/verbs.go:502-506, :968-969; `HandoffBinding` and the stage.go intent carry no claimant; dispatch.sh clears the claim epoch and main id), and the successor may not claim or land until it is built |
| U1 | registered-wait-matches-the-runtime-session | members A, B1, B2, B3, C of plans/registered-wait-matches-the-runtime-session-design.md, unchanged and in order | that page's | before U3c, which depends on it (SSTB-308); no dependency of its own | moved SSTB-209: a changed-line allocation at or under 300 per member and a direct focused Go witness for the installed-binary path without the fixture bed's `METASYSTEM_WAIT_BINARY`; moved SSTB-117: focused witnesses instead of bed legs |
| U2a | stop-gate-sees-harness-tracked-work | local waits: `wait --local --label L -- <command>` and `--pid P`, a kind `local` row with the child's pid and birth, child liveness in eligibility | about 400, split | after U1 | the child exit result; the attached pid's poll; the deadline that never signals the child |
...
| U2b | stop-gate-sees-harness-tracked-work | the human question wait proved on the landed goal wait | about 80 | after U2a | none |
...
| U5a | spend-fence-reports-tokens-per-model-and-cause | attribution in internal/spend: cause per call, kind with continuations as main, delegate kind from the `Kind:` line and the job role, the Claude-only engine filter, recursive subagent discovery | about 480, split | after U4 | moved SSTB-207: the `Kind:` line's grammar, the role map, the missing-line count; moved SSTB-116: the cursor cache schema and invalidation; SSTB-204: the runtime field name and the `startedAt` window |
| U5b | spend-fence-reports-tokens-per-model-and-cause | the health reason line, per-model ceilings, landed units by landing commit | about 280 | after U5a | moved SSTB-206 and 114: the commit resolution through git, the receipt field that marks a revival, the `seat` field and the claimant fallback |
| U5c | spend-fence-reports-tokens-per-model-and-cause | the verbs `spend window`, `spend compare`, `spend reads --last 5` and the router line | about 300 | after U5b | moved SSTB-205: the endpoints and roots the verb owns and the script's prototype status; moved SSTB-115 and 111: the provider comparison artifact and tolerance, the zone and the fleet sum |
| U7 | seats-spend-tokens-in-bounded-sessions (open question 3, answer b) | a seat inbox: `message send`, `message read`, the unread count in the Stop display, the handed-off predecessor's relay | about 200 | after U3c | moved SSTB-208: the inbox path, schema, receiver identity, cursor, lock and crash rule; the router line |
| proof | seats-spend-tokens-in-bounded-sessions | records/misc/tokens-day-<date>.md: one working day of three seats through `spend window`, `spend compare` and `spend reads`, against P5 | record | last | SSTB-307: the per-seat lines (the 150M line and the ten-refusal line) are forecasts, not proof thresholds; before the proof day the thresholds are derived per seat from `spend window`'s own cause counts, with a named landing source for the per-unit denominator; the three-seat total and the main-share lines stay as stated |

## 3. Open questions for Wido (four)

1. Which automatic launcher. Recommended (a): the steward continuation with the holds changed (P2); the pane session becomes the seat's human console. The former alternative (b), a steward-driven pane relaunch, is deleted: it costs the predecessor's death before every successor, which SSTB-302 rules out. Pane relaunch by Wido is not an option.
...
4. The in-turn bound, and what DONE's "stays under a stated context size by construction" promises (SSTB-304: the 4K per call and the 162K and 198K bounds come from averages, not maxima). Recommended (b): build U3e; DONE stands, and the number is the ceiling, 200K: over the trigger the hook allows three calls, the 50K margin covers them at four times the observed average growth, and the proof's maximum rule reports any call over 200K. Alternative (a): no hook; DONE's wording changes to a Stop-sampled trigger with a stated observed overshoot: every Stop sample under 200K, and every call under 200K only while turns hold at 12 calls (S2), 48K at 4K per call; a longer turn or a larger call breaks it, and the proof's maximum rule is the only check.
...
| SSTB-208 | folded in P7 as one line: every verb-adding unit names main.go; U7's storage contract moved to U7, this goal |
...
Round 3 (artifacts/reports/tokens-design-critique-r3.md, 11 material findings, verdict rework), folded by Wido's decision (option a) on 2026-09-15 with no further critique round. Every finding is folded here, moved to U3c's design questions, or recorded open with its owner.

| finding | disposition |
|---|---|
| SSTB-301 | moved to U3c: a headless continuation must receive a Stop trigger and can record its own next handoff; "under the same hooks" retracted in P2 |
| SSTB-302 | folded in P2, P3 and question 1: every over-trigger and idle Stop blocks until the running signal; option 1(b) deleted; a capture fault is `context-handoff-capture-fault` and does not block |
| SSTB-303 | open, owner U3c's design (coordinator-context-stays-under-budget): how claims and lineage authority move to the continuation; the successor may not claim or land until built |
| SSTB-304 | open, owner Wido, in open question 4: what DONE's "by construction" promises, with the number |
| SSTB-305 | moved to U3c: the predecessor's silent Stop as an output the hook and the adapter accept |
| SSTB-306 | folded in P4: the baseline ends 12:51 CEST; ruling R11's 12:45 was the seat's error |
| SSTB-307 | open, owner this goal's proof step: the per-seat lines are forecasts; thresholds derived per seat from the verb's cause counts before the proof day |
| SSTB-308 | folded in P7: U1 lands before U3c, named in the order and in U3c's cell |
| SSTB-309 | folded in P6: S3 and its check distinguish a fresh revision from a critique follow-up |
| SSTB-310 | moved to U3c: the pid and job exclusions need identities, not counts |
| SSTB-311 | folded in P7: one line, every owning goal's design gives each rule a focused witness |
| SSTB-312 | folded, non-material: goals by id only |
