...
Orchestrator Identity: m1d (lineage main-1788941004-20871-6e7a43, coordinator under goal delivery-candidate-is-the-workspace-not-the-ledger, closing read of chain dcwl-build3-20260911, final work round 3)
Date: 2026-09-11

# Review brief: closing read of the workspace-candidate implementation

FINDING IDS: chain-unique, continue the register: DCW-25, DCW-26, ... never F-n.

## What you are reading

The implementation chain dcwl-build3-20260911 for goal
...
final work round 3 (job dcwl-build3-20260911-r3; whole-project tree 3eeab385ca4d182a41a1b3c2d59071d0d26f0713), reviewed tree 71862805a8598eaf41ef46e119cd737d7f4d534f (the installation subtree the conformance review names as reviewedTree), rebased onto
...
This chain's one round is the reviewed candidate of chain
dcwl-build2-20260910 (read twice: `metasystem/artifacts/agents/dcwl-context/code-read-r1.md`,
`metasystem/artifacts/agents/dcwl-context/code-read-r2.md`; one material
finding, DCW-19, and ten low ones) re-applied on `main` at 609071da2 plus
the six fixes the second read's dispositions ordered (DCW-19 to DCW-24),
a seventh (the new section declared in
metasystem/scripts/agents/validate-section-selector.sh, which `test
check` demanded) and an eighth: the DCW-19 fix as first written passed
the reused result's tree to `PublishCommittedReceipt` and broke exact
reuse after a records move; round 3 resolves the accepted index tree once
before the run (the `--tree` flag, else the staged index) and passes that
(metasystem/cmd/metasystem/landing_verbs.go, the `--mode` branch). The
briefs of this chain are metasystem/artifacts/agents/dcwl-context/build3-brief.md,
build3-brief-r2.md and build3-brief-r3.md in the same directory (unlanded; they land with the chain).
Report your reviewed tree as 71862805a8598eaf41ef46e119cd737d7f4d534f. Read the whole diff, but
spend your time where it changed: confirm each of the eight fixes is
present and complete (one line each), say whether any of them reaches
beyond its finding, and add anything the two reads missed. Do not
re-derive what they settled.

Write your register as this new file: `metasystem/records/misc/delivery-candidate-is-the-workspace-code-read-r3.md`
...
Findings by id (DCW-25 onward) with the file and line in the reviewed
