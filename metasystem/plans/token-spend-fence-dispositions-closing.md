...
| Round 3 (answered by m3 at the takeover, 2026-09-03): the design never says what happens when a present transcript path cannot be listed, stat'd or read (the `~/.claude/projects` root, a slug directory, or one `.jsonl` file). Returning the error makes the role unknown, which contradicts section 6's exhaustive unknown conditions; skipping it contradicts section 3's "every present file enters the lifetime goal scope". | AUTHORIZED, section 2.1's rule for an unreadable job record applied to the seat meter: every present transcript path whose listing, stat or read fails is one explicit `unmeasured` ledger entry {path, error} with reason `seat unreadable`, counted, never skipped; a home directory that cannot be resolved is one such entry naming the reason. `SeatSummary` gains `UnreadableFiles` (json `unreadableFiles`), the count of those entries (both scopes, one count: the age of a path that cannot be stat'd is unknown). The seat segment of the section 6 line gains ` unreadable=<n>` after `aged=<n>`: `seat tokens=… lifetime=… files=<n> aged=<n> unreadable=<n> unmeasured requests=<n>`, where `files` keeps counting the readable files that entered a row. The role stays ALIVE; section 6's unknown conditions are unchanged. Nothing else moves. Named test: `TestSeatUnreadableTranscriptIsCountedNotSkipped` (an unreadable transcript path in the fixture bed — a directory named `<id>.jsonl` fails ReadFile in every sandbox without chmod; `unreadable=1` with its path and error in the ledger, Measure returns no error, the other files' tokens unchanged). |
