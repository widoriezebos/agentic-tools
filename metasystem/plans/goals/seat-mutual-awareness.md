...
- Origin: main
- Next step: RULED A BY WIDO (2026-09-06 ~23:25 CEST, his words 'Ok, option a' after the seat explained the three options in plain English): the rollout is operational. Every machine pulls, rebuilds and re-arms before any seat writer runs (the engine-rearm law makes a pull a rebuild); the validator refuses a spoiled tip; a spoiled tip is repaired by an existing human act at the terminal (a hand commit with the guard acknowledged, or goal repair); the enable marker and the human-only repair verb are REMOVED from the design. Residual accepted by Wido: an old checkout can only spoil the directory by a deliberate hand landing. NEXT (m1d, once claimed again): revision 4 of plans/seat-mutual-awareness-design.md by a Fable design round (implementer role, Working Mode: design) that removes the marker and the repair verb, folds SMA-C-24 (deadline-second boundary) and SMA-C-25 (slice order: targets can hear before a seat asks) from records/misc/seat-mutual-awareness-critique-r3.md, and states the operational rule in one paragraph; one Sol design critique; then the build box for the four slices, sized smaller than the design's seventeen reservations. Box used since re-approval: three reservations, 360 of 720 job-minutes; the revision and its review fit.
...
Integrity: sha256=5c795f452de6a86ab3434a8599da1e9b583915fe5f740bc9fca95123449c602f
