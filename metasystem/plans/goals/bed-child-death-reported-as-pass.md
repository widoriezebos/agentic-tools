...
- Origin: main
- Next step: CORRECTED MECHANISM (m1c, 2026-09-06 12:4xZ, verified on stock bash 3.2.57): an expansion death under set -u enters the EXIT trap with $? already 0, so capturing the status first and exiting with it cannot tell a dead child from a finished one; the builder proved it with a probe and gap-stopped round one without editing. The build now follows plans/bed-child-death-reported-as-pass-fold-r2-brief.md (51a018c1): the child sets fixture_child_completed=1 as its last statement; cleanup captures $?, treats an unset flag with status 0 as a death and exits 70 with 'the scenario child died before completing'; on_signal records 128+signal into fixture_signal_status and cleanup exits with it (probe: death 70, exit 1 kept, success 0, TERM 143); both same-statement declarations split (install_rearm_engine line 662 and make_repo line 613); a first scenario bed-death-self-test is expected to die and passes only when reported nonzero. Chain bcd-build1 round two (bcd-build1-r2) is running. Then: one Fable critique, close, land, run the bed seat-side under stock bash and cite the three genuinely green re-arm scenarios on goal rearm-remedies-and-adoption-notes as the real-runner proof.
...
Integrity: sha256=33722e2204921c323054e12ba21dffe3acf28f06a342db2b3361938fac5e8eca
