...
- Origin: main
- Next step: BUILD DECISION D-RESUME (m0, 2026-09-03, resolving a design-vs-fixture contradiction the build flagged): resume passes the SAME approved-and-valid gate as claim and steal - an unapproved or expired-approval goal refuses resume with APPROVAL_REQUIRED. The design revision-2 exception (a proof-bearing resume MINTS approval) is WITHDRAWN under the feature's core invariant 'nothing executes without a standing human approval'; a human resuming an unapproved goal runs goal approve first (or approve-and-resume as two recorded human acts). Rationale: resume is execution; the invariant admits no execution-path exception, matching HAE-CLOSE-02 (steal must not mint from expired approval). Recorded as a build directive, verified by the code review; no new design-review round (R-60-m1). --- IN PROGRESS: design done (revision 2, plans/human-approval-for-execution-design.md), closing review folded to build inputs; build re-dispatched with this decision.
...
Integrity: sha256=690203d35b031a3aa52ffce6d8aeb5a3348951095e3c2ba48d36c2e55a3b7a9e
