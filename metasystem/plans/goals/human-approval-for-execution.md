...
- Origin: main
- Next step: IN PROGRESS (tier 3, R-54-m1): design landed 071446e1, its ONE review running now. CONVENTION FIX pending (m1's note): the design landed under records/human-approval/hae-design.md but in-flight designs live in plans/; at the FOLD landing, git mv it to plans/human-approval-for-execution-design.md and land there via seat carriage --allow-new-plan (delegate worktrees can't touch plans/, but the seat lands it there like every other in-flight design). Then: one fold, one closing review, build, one code review. FEATURE: fifth ledger state 'approved' only the human sets (goal approve verb, resume/set-obligation proof pattern), budget carried at approval, APPROVAL_REQUIRED on every claim path, grandfather sweep as one recorded human act.
...
Integrity: sha256=79d75ac1c97b1d8eccd421ecb962d024f7c58f9a90d47f4a938bee34a20e5a77
