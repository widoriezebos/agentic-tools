...
- Origin: main
- Next step: 2026-09-14 m1b: build round 29 in flight, and it covers every hook bed failure left. Round 28 verified on the trunk tip, carried onto M1e's 0995650b: deadline and unreadable-worker routes now complete their hook attempt. A keep-going copy of the hook bed on round 28's tree fails exactly three groups: a repeated deadline overrun that records the cause as a code and counts 1 per uncertain episode instead of 2 per session; and two fixtures written against the retired shell parent, which revision 4 of the design page (landed b0b9f37e, designed on Claude claude-fable-5-1) replaces with StopDeadlineRecordWriteRefused and StopDeadlineEndsOnlyItsDirectWorker, adding one parent obligation: at the freeze cutoff the parent ends only its direct worker and prints one fixed stderr line. Round 29 builds all three. The supervision and landing beds are running on round 28's tree now; stop-hook-monitor and nested-root's override check stay red on the tip until M1c's grep-pipe slice lands. Budget in Wido's name: elapsed 3d, active jobs 2, attempts 42. Then the rostered Opus read (brief refreshed), the delivery proof and the landing.
...
Integrity: sha256=8a0db54ad43ea74ec45248fc249ae14588f8fd408bdd41fd24f9b51e73cd461c
