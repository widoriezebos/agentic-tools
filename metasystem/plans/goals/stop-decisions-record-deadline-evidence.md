...
- Origin: main
- Next step: 2026-09-14 m1b: build round 25 in flight. Round 24 verified outside the delegate sandbox on a clean worktree of the tip: the landing bed passes end to end again (file-based seeding replaced the archive pipes that got SIGPIPE), the killed-worker route logs its outcome line, but the trail-line rework regressed two lines green since rounds 8 and 19 (the ordinary 'stop response decision=... elapsed=' line and the 'stop-condition infrastructure supervision-arming-failed' line); round 25 restores both and pins the exact set of log lines per delivered route in a Go test. A control of the supervision bed on the unmodified tip (73103ce7) fails two scenarios of its own, stop-hook-monitor (the newest Stop wording no longer matches the bed) and rearm-provenance (an archive pipe M1c is fixing); both are excluded from this chain and reported to M1e and M1c. Thirty-five build defects found by the beds and fixed since round 6. Then the rostered Opus read, the delivery proof and the landing.
...
Integrity: sha256=abb2c48700baf8de9e751e4b598e4e75b1e494f56befc77ab653c5b513ce2ae1
