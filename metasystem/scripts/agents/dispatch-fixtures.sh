...
# without creating a job. Then a converted checkout serves the machine's
# claimed goal: the payload brief carries the exact bounded section and the
# record's input hash equals the payload brief's own sha256 — the projection
# is inside the recorded bytes, so every fallback rebuild carries it.
set +e
