...
approve_fixture_goal() { # goal id, optional complete tuple flags
  ...
  "$ms" goal approve --root "$clone" --id "$goal_id" --by Wido \
    --temporary-human-word "Wido approves fixture goal $goal_id" --review-by 2026-09-06 "$@" >/dev/null
}
...
"$ms" goal set-budget --root "$clone" --id budget-check \
	...
	--by Wido --temporary-human-word "Wido approves fixture rebudget" --review-by 2026-09-06 >/dev/null
...
if norm_refusal=$("$ms" goal set-budget --root "$clone" --id norm-parent \
	...
	--temporary-human-word "Wido proposes an over-norm fixture budget" --review-by 2026-09-06 2>&1); then
...
"$ms" goal set-budget --root "$clone" --id ship-widget \
  ...
  --active-job-limit 1 --review-round-limit 3 --by Wido \
  --temporary-human-word "Wido approves the migrated fixture claim" --review-by 2026-09-06 >/dev/null
