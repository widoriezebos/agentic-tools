...
for ((iteration = 1; iteration <= iterations; iteration++)); do
  ...
  "$census" alive --pid "$old_pid" --start-time "$old_start" >/dev/null \
    || { echo "fingerprint harness watcher is not live before kill" >&2; exit 1; }
  command=$(ps -p "$old_pid" -o command= 2>/dev/null || true)
  [[ "$command" == *"$old_tag"* || "$command" == *"$repo"* ]] \
    || { echo "fingerprint harness could not prove watcher ownership before kill: $command" >&2; exit 1; }
  kill -TERM "$old_pid"
