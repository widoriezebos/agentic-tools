...
- Every claim about code carries file:line. Every rule gets a witness that fails without it and a named mutation. Units at most 300 changed lines each, in landing order, each with its witness. Builds will be Codex gpt-5.6-sol in a worktree, reads Opus, critique by another model; say which unit is first.
