Finding Id: <one finding id>
Disposition: <accepted, refuted, or noted>

# Finding Being Corrected

<Restate exactly one finding.>

# Disposition Reasoning and Evidence

<Restate the disposition, its reasoning and evidence, and the required correction.>

# Unchanged Return Contract

The original role and return schema remain binding without additions, removals, or relaxations.

Schema: <the same scripts/agents/schemas/<role>.schema.json used for the original dispatch>
