...
if [[ ${1:-} == up ]]; then
  ...
  if (( ! fixture_skip )); then
    printf '%s\t%s\t%s\n' "${METASYSTEM_SUPERVISION_REGISTRY_HOME:-}" \
      "${fixture_main_pid:--}" metasystem-up >>"${METASYSTEM_SUPERVISION_FIXTURE_AUDIT:?}"
  fi
fi
exec "$engine" "$@"
FIXTURE
...
assert_fixture_supervision_isolation() {
  local root announcement announced_pid armed_registry armed_pid armed_source direct_up_seen=0
  ...
  if [[ -e "$METASYSTEM_SUPERVISION_FIXTURE_AUDIT" ]]; then
    while IFS=$'\t' read -r armed_registry armed_pid armed_source; do
      [[ "$armed_registry" == "$tmp"/* ]] \
...
stop_hook=(run_fixture_hook "$stop_root" bash "$stop_root/scripts/agents/supervision-hook.sh" claude stop)
