...
| missionrunner/TestTerminateGroup | adopt-fixtures frozen gate (full race+coverage go gate) | 1 | 2026-09-06 | "--- FAIL: TestTerminateGroup (5.01s)" inside the adopt bed's witness gate on m1 at load average 7 (three seats' work plus a Fable critic and a Sol builder); the package took 586s; solo rerun of the two TerminateGroup tests on main green in 5.8s; wall-clock-patience-under-load class, sibling of the row below; evidence artifacts/agents/suite-failures/20260906T181005Z-adopt-40115 in the rra-build1 worktree | first sighting |
| missionrunner/TestTerminateGroupLeaksNoGroupsUnderCompression | adopt-fixtures frozen gate (full race+coverage go gate) | 3 | 2026-09-06 | second and third sightings the same evening (first 2026-08-31 in the governed discharge): "(10.48s)" at load 7 and "(10.51s)" at load 4-6 on m1 (evidence artifacts/agents/suite-failures/20260906T183143Z-adopt-11310 in the rra-build1 worktree); solo rerun green both times; THREE inside thirty days, the fix goal missionrunner-terminate-flake is already open (approved, parked cold-resumable) and now carries the three-strike promotion | fix goal open: missionrunner-terminate-flake |
| channel/fake/TestTelegramTokenRoutes | adopt-fixtures frozen gate (full race+coverage go gate) | 1 | 2026-09-06 | "fake_test.go:47: fake did not stop" after 5.04s under the same load; solo rerun on main green in 0.3s; a fixed five-second stop bound in the fake channel's test | first sighting |
...
| proofrun watchdog evidence-copy 1s window | battery go gate | 2-DETERMINISTIC-IN-BATTERY, FIXED | 2026-08-29 | TestDoneFileWinsBeforeAndDuringEvidencePreservation/during_preservation: "evidence copy exceeded its 1s timeout" under battery load; passes 4x unloaded. Root shape: a FIXED 1s wall-clock window in watchdog_test.go:98 - violates the scaled-cap fixture law (fixed windows died with the 86-minute hang lessons). Owner hint: m2's proof-run arc (in flight); the fix is a harness_fixture_cap-scaled window, and belongs with timing-tests-synthetic-clock. Reclassified after a second consecutive battery envelope: deterministic in the battery clone, not a load flake. Round two CLOSED it (fix-forward, c-next): the failure was subprocess scheduling latency under the concurrent race+coverage gate; the test now injects preservation directly - no real-time race remains, subtests 0.00s, proven under cold-cache race conditions; no production defect (production evidence timeout is a configurable 60s) |
