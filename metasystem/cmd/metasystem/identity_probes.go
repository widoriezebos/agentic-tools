...
func runIdentityExists(args []string) int {
	flags := flag.NewFlagSet("identity exists", flag.ContinueOnError)
	pid := flags.Int64("pid", 0, "process id")
	if flags.Parse(args) != nil {
		return 2
	}
	if *pid < 1 {
		fmt.Fprintln(os.Stderr, "identity exists: --pid must be positive")
		return 2
	}
...
func runIdentityGroupExists(args []string) int {
	flags := flag.NewFlagSet("identity group-exists", flag.ContinueOnError)
	pgid := flags.Int64("pgid", 0, "process group id")
	if flags.Parse(args) != nil {
		return 2
	}
	if *pgid < 1 {
		fmt.Fprintln(os.Stderr, "identity group-exists: --pgid must be positive")
		return 2
	}
