...
func families() []family {
	return []family{
		...
		},
		{
			...
			verbs: []verb{
				{"claim-launch", "claim or resolve one fingerprinted process launch", runDispatchClaimLaunch},
				{"claim-occupancy-prepare", "prepare a launch occupancy witness off the cap lock", runDispatchClaimOccupancyPrepare},
				...
				{"ownership-patch", "write a platform-exact launch ownership patch", runDispatchOwnershipPatch},
				{"custody-add", "append a custody process to a job record under its lock", runDispatchCustodyAdd},
				{"prefork-mark", "write advance evidence before a job supervisor forks", runDispatchPreforkMark},
				{"custody-groups", "list every recorded custody group for wind-down", runDispatchCustodyGroups},
				{"reconcile-reservation", "reconcile an identityless launch reservation", runDispatchReconcileReservation},
		...
		},
		{
			...
			verbs: []verb{
				...
				{"estimate", "record the claimant's current remaining-work estimate", runGoalEstimate},
				...
				{"banners", "print current appetite checkpoint banners; empty is silent", runGoalBanners},
