...
// TestIntentCompatibilityGitAdapterRecordsTheFloorThroughTheEngineFloorOwner:
// settings compatibility --minimum-engine SHA --by NAME runs the real
// goal engine-floor owner as its own process with exactly the public
// adapter's argv. Enrolled-terminal proof is not the terminal
// classification: the owner's kernel reader never consumes the fixture
// table. So the test-only process seam records the adapter's original argv
// and only then appends the existing --fixture-human-authority flag and a
// lineage, which the owner accepts solely for this exact fake-runtime root
// carrying a fixture enrollment. The owner's real transition is observed:
// the accepted ledger's root record gains the human engine-floor line, and
// the bare public read reports it. Git is the claim because the owner
// process publishes to the Git ledger.
func TestIntentCompatibilityGitAdapterRecordsTheFloorThroughTheEngineFloorOwner(t *testing.T) {
	engine := intentTestEngine(t)
	root := migratedHumanTerminalRoot(t, "compatibility-machine", os.Getppid())
	...
	delivery := &intentDeliveryOwners{
		process: func(process intentProcess) intentProcessResult {
			original = append(original, append([]string(nil), process.argv...))
			if fixture {
	...
	before := project()
	if code, result := run("compatibility"); code != 0 || !strings.Contains(result.Summary, "no minimum engine") || len(original) != 0 {
		t.Fatalf("unrecorded read: %d %+v", code, result)
	}

	// Control: the adapter's own argv, with no fixture proof, is refused by
	// the owner's enrolled-ancestry check and records nothing.
	fixture = false
	code, result := run("compatibility", "--minimum-engine", commit, "--by", "Wido")
	if result.Outcome != intentRefused || code == 0 || !strings.Contains(result.Summary, "could not prove enrolled human ancestry") ||
		project().Tip != before.Tip {
		t.Fatalf("floor without enrolled proof: %d %+v", code, result)
	}
	fixture = true

	code, result = run("compatibility", "--minimum-engine", commit, "--by", "Wido")
	if len(original) != 2 {
		t.Fatalf("owner runs = %v", original)
	}
	want := []string{engine, "goal", "engine-floor", "--root", root, "--commit", commit, "--by", "Wido"}
	for _, argv := range original {
		if !slicesEqual(argv, want) {
			t.Fatalf("public adapter argv = %v, want %v", argv, want)
		}
	}
	expectOutcome(t, "engine floor", code, result, intentConfirmed)
	after := project()
	line := after.Tree.Root.History[len(after.Tree.Root.History)-1]
	if after.Tip == before.Tip || after.Tree.Root.Revision != before.Tree.Root.Revision+1 ||
		line.Verb != "engine-floor" || line.Actor != "human:Wido" || line.Reason != commit+" every enrolled seat runs this engine or newer" ||
		goal.EngineFloorOf(after.Tree.Root) != commit {
		t.Fatalf("the owner's actual floor: tip %s -> %s, revision %d -> %d, line %+v", before.Tip, after.Tip,
			before.Tree.Root.Revision, after.Tree.Root.Revision, line)
	}
	if code, result := run("compatibility"); code != 0 || result.Summary != "the recorded minimum engine is "+commit+" (a person's assertion, not a probe of the machines)" {
		t.Fatalf("recorded read: %d %+v", code, result)
	}
}
