...
func workerAuthorizedFileAttemptFixture(t *testing.T) (string, string, proofrun.Attempt) {
	t.Helper()
	...
	proofIdentity, err := proofrun.BuildProofIdentity(controlRoot, conf, "full", "worker-root", nil, 2)
	if err != nil {
		t.Fatal(err)
	}
