...
func processIntentCommands() []intentCommand {
	return []intentCommand{
		{
			name: "start", audience: "both", summary: "let this checkout's work run again, or start an agent session",
			usage: []string{"metasystem start [checkout]", "metasystem start session"},
			details: []string{
				...
				"start session is how an agent session starts; it is the same as metasystem up.",
			},
			flags:    []intentFlag{intentInstallationFlag, intentTemporaryArmFlag, intentReviewByFlag, {name: "lineage", value: "LINEAGE", advanced: true, usage: "start session: the session's owner lineage"}},
			maxArgs:  1,
			examples: []string{"metasystem start", "metasystem start session"},
			run:      runIntentStart,
		},
		{
			name: "stop", audience: "both", summary: "stop this checkout, one job, or the current session",
			usage: []string{"metasystem stop [checkout]", "metasystem stop job J", "metasystem stop session --by NAME"},
			details: []string{
				...
				"stop job J cancels exactly the launch or dispatch job J; nothing else stops.",
		...
		{
			name: "restart", audience: "human", summary: "stop then start this checkout, or restart the interface",
		...
		{
			name: "status", audience: "both", summary: "what is running: this checkout, one job, or one unit run",
			usage: []string{"metasystem status [checkout]", "metasystem status job J", "metasystem status unit U"},
			details: []string{
				"Unknown and stale readings are reported as such; a read failure is never shown as stopped or healthy.",
				"Launch and unit runs belong to the current user; a job id is matched exactly, never by prefix.",
			},
			flags:    []intentFlag{intentInstallationFlag},
			maxArgs:  2,
			examples: []string{"metasystem status", "metasystem status job design-r2-4f1c", "metasystem status unit build-3a"},
			run:      runIntentStatus,
			legacy:   legacyProcessCall,
		},
		{
			name: "enroll", audience: "human", summary: "enroll this terminal as the person's",
		...
		{
			name: "ask", audience: "agent", summary: "ask the person a question through the channel",
		...
		{
			name: "answer", audience: "human", summary: "answer a mission's question",
			usage: []string{"metasystem answer mission M Q TEXT", "metasystem answer mission M Q --answer-file FILE", "metasystem answer question Q"},
			details: []string{
			...
			maxArgs:  4,
			examples: []string{"metasystem answer mission demo host-failure 'retry: the host is back'", "metasystem answer mission demo host-failure --answer-file answer.md", "metasystem answer question q-20260925-1"},
			run:      runIntentAnswer,
		},
		{
			name: "fleet", audience: "both", summary: "every machine's presence, this one first",
		...
		{
			name: "doctor", audience: "both", summary: "diagnose this checkout's machinery, changing nothing",
			usage:    []string{"metasystem doctor"},
			details:  []string{"Checks every part of the checkout's machinery once and repairs nothing. Each problem names the command that fixes it, where there is one."},
			flags:    []intentFlag{intentInstallationFlag},
			maxArgs:  0,
		...
		{
			name: "ui", audience: "human", summary: "the browser interface: start, stop or status",
...
func legacyProcessCall(args []string) bool {
	for _, arg := range args {
		switch arg {
		case "checkout", "job", "session", "unit", "--json", "-json", "--help", "-help", "-h":
			return false
		}
	}
	return true
...
func (inv *intentInvocation) resolveJob(id string) (intentJob, *intentResult) {
	...
	switch {
	case err == nil:
		found = append(found, intentJob{kind: "launch", launch: record})
	...
	if dispatchJobIDPattern.MatchString(id) {
		if problem := inv.selectLayoutRoot(); problem == nil {
			...
			case readErr == nil:
				found = append(found, intentJob{kind: "dispatch", dispatch: object, recordPath: path})
			default:
	...
	case 0:
		return intentJob{}, &intentResult{Outcome: intentRefused, code: 1, Targets: targets,
			Summary:  fmt.Sprintf("no job %s: no launch record of this user and no dispatch job in %s; nothing was done", shellCommand([]string{id}), repository),
			Decision: "name the exact job id; launch ids are listed by metasystem internal launch list"}
	case 2:
		return intentJob{}, &intentResult{Outcome: intentRefused, code: 1, Targets: targets,
			Summary:  fmt.Sprintf("%s names both a launch record and the dispatch job %s; nothing was done", shellCommand([]string{id}), found[1].recordPath),
			Decision: "use the owner directly: metasystem internal launch cancel --id J, or metasystem delegate --cancel J",
			Data:     map[string]any{"launch": found[0].launch, "dispatch": found[1].dispatch}}
	}
	return found[0], nil
}

func (inv *intentInvocation) stopJob(id string) int {
	job, problem := inv.resolveJob(id)
	if problem != nil {
		return inv.render(*problem)
	}
	targets := []intentTarget{{Kind: "job", ID: id}}
...
func runIntentStatus(inv *intentInvocation) int {
	...
	switch kind {
	case "job":
		job, problem := inv.resolveJob(target)
		if problem != nil {
			return inv.render(*problem)
		}
		targets := []intentTarget{{Kind: "job", ID: target}}
		if job.kind == "launch" {
...
func runIntentAsk(inv *intentInvocation) int {
	...
	wait := []string{"metasystem", "internal", "channel", "wait", "--root", inv.stateRoot, "--question", q.ID}
	poll := []string{"metasystem", "internal", "channel", "poll", "--root", inv.stateRoot}
	if err != nil {
	...
	if pending {
		...
		if q.Undelivered > 0 {
			result.next, result.nextReason = poll, "the channel poll posts undelivered questions again"
		} else {
...
func runIntentAnswer(inv *intentInvocation) int {
	...
	missionID, askID, answer := args[1], args[2], args[3]
	...
	if !missionIDRe.MatchString(missionID) || !missionIDRe.MatchString(askID) || strings.TrimSpace(answer) == "" || strings.ContainsRune(answer, 0) {
		return inv.render(intentResult{Outcome: intentRefused, code: 2, Targets: targets, Summary: "a mission and question id are lowercase words with dashes, and the answer is non-empty text; nothing was answered"})
	}
	engine, err := inv.owners.processes.mission(inv.stateRoot, missionID)
	if err != nil {
		return inv.render(intentResult{Outcome: intentFailed, code: 1, Targets: targets, Summary: "mission answer: " + err.Error()})
	}
	...
	resume := []string{"metasystem", "internal", "mission", "resume", "--root", inv.stateRoot, "--mission", missionID}
	switch {
	case code == 0:
		return inv.render(intentResult{Outcome: intentConfirmed, Targets: targets, Summary: "answered " + askID + " of mission " + missionID, text: lines, Data: data})
	case effects.ResetRecorded && !effects.AskAnswered:
		...
		return inv.render(intentResult{Outcome: intentPartial, code: code, Targets: targets, text: lines, Data: data,
	...
	case effects.AskAnswered || answered:
		...
		return inv.render(result)
	case code == 3:
		return inv.render(intentResult{Outcome: intentRefused, code: code, Targets: targets, text: lines, Data: data,
			Summary: "the mission did not take the answer; nothing changed"})
	}
	return inv.render(intentResult{Outcome: intentFailed, code: code, Targets: targets, text: lines, Data: data, Summary: "the mission's state could not be read; nothing changed"})
}
...
func runIntentDoctor(inv *intentInvocation) int {
	...
	for _, role := range verdict.Roles {
		...
		if role.Remedy != "" {
			line += "; remedy: " + role.Remedy
		} else if role.NoAutomaticRemedy {
			line += "; no command repairs this"
		}
		lines = append(lines, line)
	}
	return inv.render(intentResult{Outcome: intentConfirmed, code: verdict.ExitCode(), Targets: inv.checkoutTarget(scope),
		Summary: verdict.Line(), text: lines, Data: steward.NewHookHealthPreview(verdict)})
}
...
func (inv *intentInvocation) askAfterFailure(q channel.Question, failure error, code int, targets []intentTarget, warnings []string) intentResult {
	...
	if readErr != nil {
		...
		result.Decision = "repair the channel question storage at " + shellCommand([]string{questionFile}) + ", then read the question with metasystem internal channel show --root " + shellCommand([]string{inv.stateRoot}) + " --question " + q.ID
		return result
	}
	...
	default:
		...
		result.next = []string{"metasystem", "internal", "channel", "poll", "--root", inv.stateRoot}
		result.nextReason = "the channel poll posts saved questions that have no thread"
	}
	return result
}
