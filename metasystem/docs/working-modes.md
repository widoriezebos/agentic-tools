...
The metasystem organizes work into modes. A mode is a promise about what will be true when you are done. Each mode has its own rules because each one fails in its own way. This guide explains every mode in plain language: why it exists, when you are in it, how it works, and how to use it.
...
- *The frontier* is your best version so far: exact commit, score, and the run that proved it. `metasystem report frontier` guards it. `challenge` refuses to call a run an improvement unless it beats the frontier by more than the noise floor, and `record` refuses to overwrite a better frontier with a worse one. When you beat the frontier, preserve that exact state before touching anything else.
...
**How it works.** Freeze the frame first: exact symptom, exact state, what has been tried, and a budget. Keep two to four theories that evidence could kill, and hunt the evidence that decides between them instead of the evidence that comforts you. Before every attempt, write a short contract: what question this answers, what it costs, and when you will stop. Classify every result honestly: did the contract improve, did a theory die usefully, or did nothing change? The stop-loss triggers (two no-progress results, one expensive run that taught nothing, a dead end) are mandatory. The machine-checkable ones are enforced by `metasystem validate stop-loss`, which reads the ledger and blocks new cycles once a dead end, two no-progress results, or the cycle budget is on record. When a trigger fires, stop, preserve what was learned, revert what failed, and take the decision up a level.

**Why the stop-loss works this way.** The name comes from trading: a stop-loss is an exit you commit to before you enter, because you know your judgment will be at its worst at exactly the moment the exit is needed. Investigations have the same problem. Sunk cost makes the next attempt feel cheaper than it is, and every failed try produces a new idea that feels one fix away. So the exit is decided up front: the ledger states a cycle budget before the work starts, and the honest classification you write after each cycle is the tripwire. `metasystem validate stop-loss` reads those classifications and refuses another cycle once a trigger is on record. Arguing with it means arguing with the version of you that was still thinking clearly.

A fired stop-loss is the mechanism working, and it is a good outcome. Three things happen: the learning is preserved in the ledger, behavior changes that did not work are reverted, and the decision moves up a level with the evidence attached. It converts a doomed afternoon into a documented dead end. The same pattern runs at smaller scale in the other modes: refactor mode allows three fix attempts before the unit is reverted, and improve mode stops after three experiments that fail to beat the frontier. Everywhere the shape is identical: the exit is committed before the work starts, the results are classified honestly, and the escalation carries the evidence.
...
One workflow sits above all the modes: the retro (`skills/retro/SKILL.md`). It is how the metasystem improves itself. Every repo-changing task leaves a one-line receipt. When enough accumulate, a retro runs. It treats the metasystem's own rules the way improve mode treats code: every rule change was adopted with a written, testable expectation ("correction X never repeats"), and the next retro checks whether that expectation came true, then keeps, amends, or reverts the change. A rule that cannot show its value after two reviews is removed by default. Humans veto every change; nothing rewrites the rules automatically. Over time this should make the metasystem smaller and more precise.
