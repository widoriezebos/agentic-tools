...
The hazards above, together with the configuration-and-dispatch hazard derived by the anti-mimicry test below, give the delivery system seven working roles. Here is what each may do and may not do; everything that follows relies on these names.
...
One configuration deserves its own warning, because most adoptions build it first. Economy pushes toward a single coordinating actor that starts work, keeps the records, performs acceptance and writes the reports. Each function is harmless alone. Together they let one actor start its own work, accept it and describe the result, and no prohibited combination appears on paper because each role was assigned one at a time. The separations that matter must therefore hold at the actions rather than in the actor's conduct: acceptance refuses work no independent examination covers, and release refuses any candidate that has not passed acceptance, whoever asks. A combination policed only by good conduct is not a configuration. It is the hazard back in one actor.
