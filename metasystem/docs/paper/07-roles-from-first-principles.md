...
In Chapter 6 we established that a fresh perspective needs contextual independence: an examiner not already exposed to the builder's path. Here that becomes an authority question. A new machine worker can take a temporary role without a reporting line or a permanent title, but prior exposure can still disqualify it from serving as the independent examiner.
...
Two further hazards arise around the work rather than inside the candidate. A stopped worker may look slow, and only another actor who observes whether it is alive and making progress can make the stop clear. A technically complete record may be unreadable to the person who must rule on an exception. Silent death requires a liveness watcher. A misleading retelling requires a narrator.

## The roles

The hazards above, together with the configuration-and-dispatch hazard derived by the anti-mimicry test below, give the delivery system seven working roles. Here is what each is allowed to do and what it is not; everything that follows relies on these names.
