...
A proposal supplies one trigger for the test, but some roles persist across changes without ever being proposed. They grew out of practice before the rules around them, and the actor holding such a role may control the process that would question it. In that case, nobody has a clear duty to start the test. These roles therefore need a second trigger. Chapter 12 supplies the machinery for it: each gets a governance record with an owner, a review date and an appeal route. The record also names the hazard the role controls, the permissions it needs and must not hold, and the responsible authority allowed to change or withdraw it. The watch over dated records flags an overdue review to that authority. At review, the role must answer the same questions as a proposed role. If it cannot, that authority changes the role or withdraws its permissions within its recorded scope.
