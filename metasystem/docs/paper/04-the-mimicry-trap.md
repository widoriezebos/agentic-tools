...
Consider an inherited approval meeting. First ask what 'need' it was, why was the meeting requested? Perhaps information was scattered and people needed one place to see all of it, two groups held conflicting goals and needed a way to settle them, or a consequential action needed a named person to approve it. Next ask whether that need still exists, and whether something else now meets it. A shared record can meet the first need; it cannot settle a conflict of values or accept accountability. Then ask what observable evidence would show that a replacement works. If work is meant to be visible, can a reader see current state, last progress, deadline and failure without asking the worker? If the meeting is meant to find faults, does the replacement catch a change you already know is broken?
