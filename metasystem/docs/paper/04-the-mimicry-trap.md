...
Code review adds a second perspective while spreading knowledge that would otherwise remain with one person. It serves scrutiny and continuity beyond the original author. It also develops the reviewer: trying to understand another person's design can change what they know to question in their own work.
...
Consider an inherited approval meeting. First ask why the meeting was requested and which needs it met. It may have served several at once: information was scattered and people needed one place to see all of it, two groups held conflicting goals and needed a way to settle them, and a consequential action needed a responsible authority to approve it. Next ask, for each need, whether it still exists and whether something else now meets it. A shared record can meet the first need; it cannot settle a conflict of values or accept accountability. Then ask what evidence would show that each remaining need is still met. If work is meant to be visible, can a reader see current state, last progress, deadline and failure without asking the worker? If the meeting is meant to find faults, does the replacement catch a change you already know is broken and let through a change you know is sound?

Only after those questions does a responsible authority decide what happens to the form, and the decision is recorded. When no need remains, discard the form and record why. When meeting the need is mechanical and its success observable, machinery can replace the form. When a need remains and the old form still serves it, retain the form. When the need remains but the old form fits it poorly, adapt the form. When the act contains a value ruling or accountability that machinery cannot hold, keep it as an explicitly human act, held by a responsible authority. One ceremony often serves several needs, and each can lead to a different outcome for the form. The approval meeting served three, and each gets its own answer: a shared record takes over the scattered information, the conflict between the two groups still has to be argued out between people and ruled on by a responsible authority, and approval of the consequential action stays with the responsible authority who answers for it. What remains of the meeting is the part only people can do.
...
Code review survives in adapted form. The builder finishes a candidate and supplies the exact result and its evidence. An independent examiner receives the authorized intent, relevant constraints and finished work, but not the builder's path of reasoning. The independent examiner tries to find faults and ties each conclusion to a result that others can reproduce. To find out whether the examination finds faults at all, give it a version you already know is broken. This adaptation preserves scrutiny and continuity without requiring line-by-line approval as a social ritual. Machinery may gather information and perform much of the scrutiny. The learning that human review supplied still needs an answer: records preserve knowledge, while a person needs practice to acquire the judgment to use it. In Chapter 14 we will develop that part of the replacement.
