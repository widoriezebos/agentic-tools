...
This is the mimicry trap. A familiar process is copied onto a workforce with different limits, so the result looks controlled while the need that once justified the process goes unexamined. Avoiding the trap does not require rejecting history. It requires reading each inherited practice as evidence about the problem that it was built to solve.

## The need behind each ceremony

What follows are things I have seen in software teams, but not every team uses each practice.
...
Code review adds a second perspective while spreading knowledge that would otherwise remain with one person. It serves both scrutiny and continuity beyond the original author. It also develops the reviewer: an attempt to understand another person's design can change what they know to question in their own work.
...
Consider an inherited approval meeting. First ask why the meeting was requested and which needs it met. It may have served several at once: information was scattered and people needed one place to see all of it, two groups held conflicting goals and needed a way to settle them, and a consequential action needed a responsible authority to approve it. Next, for each need, ask whether it still exists and whether something else now meets it. A shared record can meet the first need, but it cannot settle a conflict of values or accept accountability. Then ask what evidence would show that each remaining need is still met. If work is meant to be visible, can a reader see current state, last progress, deadline and failure without asking the worker? If the meeting is meant to find faults, does the replacement catch a change you already know is broken, and does it let through a change you know is sound?
