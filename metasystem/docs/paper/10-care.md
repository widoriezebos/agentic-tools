...
The releaser does the watching here, and it does not decide for itself what to look at. The conditions worth watching were recorded with the intent, and the enforced rule from Chapter 1 refused release until live observation could tell an expected rise in sign-ins from a broken loop. So the releaser compares what production reports against the bounds it was given: expiry after inactivity, refresh that does not extend, uploads that finish, places restored. It watches the harm signals too: sign-in loops, abandoned forms, sessions that outlive the authorized limit. The comparison does not stop when expansion completes: it stands for as long as the intent's conditions do, and the drift the next section describes lands on that standing watch.

Not everything arrives as a measure. Counts of expiry events and failed sign-ins come from the service itself. Patterns of use can show people abandoning a task after expiry without saying why. A support report can describe lost work that no counter names. Direct feedback from an affected person can reveal what the service cannot see at all: a warning that confused people, a shared device that now behaves unsafely. The measures record themselves under the enforced comparison; a person records a support report or direct feedback against the release it concerns. Either way the observation lands in the same custody that holds the rest of the work: bound to the exact released version, with its source, time, affected group and uncertainty, beside the checks and rulings that authorized that version. The limits travel with it. A fall in active sessions may be successful expiry or frustrated departure. A support spike may be a severe problem for a few people or an easy-to-report nuisance. Silence from users proves nothing.

Recorded observations then move the actors with authority to act. Whether a bound was crossed is mechanical: the comparison is an enforced rule, and inside the bounds the release keeps expanding under the authority it already has. A crossed bound makes the releaser contain or restore, within its own permission, and that action is recorded too. Observations that need interpretation, the ambiguous fall in sessions, the support spike, reach people in the narrator's report, and judgment there decides what they are: a disproved claim becomes repair work for a builder, through the same construction and examination as any change, and a question about the intent itself, such as whether the chosen signal reflects how people actually work, goes to the responsible authority. Production reaches the system only as observations, observations become records, and records move the actors. Observation serves the outcome; it is not a dashboard of convenient activity.
...
Before release, the session change proves that the previous behavior can be restored while old and new session records coexist. When lost-work reports appear, the releaser uses that path. Restoration is not improvised during the incident, and it does not depend on the failed candidate remaining able to repair itself.
