...
None of this is invented after release: the conditions being watched were recorded with the intent, and the enforced rule from Chapter 1 refused release until live observation could tell an expected rise in sign-ins from a broken loop.

The interaction with production runs through the same roles and records as everything else. Authorized release is the actor that touches the live environment: it moved the candidate into traffic, it holds the authorized bounds, and it compares what the measures report against those bounds, expanding, pausing or restoring within its own permission. Every measure it reads is an observation record, bound to the exact released version, its source and its time, stored beside the checks and rulings that authorized that version. Signals no measure can capture, a support report, direct feedback from an affected person, enter the same way: someone records them against the release they concern. From there the ordinary roles act. An observation within bounds moves no one. One that crosses a bound makes authorized release contain or restore, and that action is recorded too. One that disproves a claim becomes repair work for a builder, through the same construction and examination route as any change. One that questions the intent goes to the responsible authority in the narrator's report. Production reaches the system only as observations; observations become records; records move the actors that have authority to act.
