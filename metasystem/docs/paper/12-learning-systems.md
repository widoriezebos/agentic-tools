...
Start with what a rule physically is. A rule is one checked-in file in the application's rule set: a check, which is code; any threshold the check reads, which is data; the known-bad case it must flag and the known-good cases it must pass; and a record, described below. The generic half of the metasystem supplies the boundaries; each application accumulates rules like this one. The check fires with power in one place: each time a candidate is submitted for acceptance, it runs against that exact candidate, and the submission is refused on the spot if session-limit code reads local wall time. A builder can run the same check at any time while working; that run is advice. Changing the threshold is an act of authority, not an edit. And application code is never the home of a rule; it is what the rules judge.

Before the clock rule receives its power, its sponsor, the builder proposing it, must show more than the original failure. An intentionally broken candidate using local time must fail the check. Correct candidates using an elapsed-time source must pass. Code that only formats a local time for display must also pass. These cases establish both sides of the line: what must be stopped and what must remain possible. This is Chapter 6's discriminating test, pointed at the rule itself, and the cases are not an argument: they are files that run.

The record is data too, and a rule about rules reads it at the same acceptance boundary. That is what gives governance teeth, because a written expectation is only advice. A rule cannot be adopted while its record is incomplete: no named owner, no known-bad fixture, no review date, no adoption. The owner is Chapter 5's legislator, the person or group with authority to maintain or withdraw the rule. The record also states what the rule may judge, the evidence a refusal must attach, the signals the owner watches for side effects, and the appeal route by which a builder or affected person can challenge a refusal; the gate fills the owner and the appeal route into every refusal it issues. A rule cannot keep power in bad standing either: when the review date passes without renewal, or the known-bad fixture stops failing, the rule drops by itself from refusing to marking, and its owner is told. An unowned rule loses its teeth automatically, so the pile of unowned refusals cannot form.
...
The enforced rule is then revised and examined independently of its author. The independent examiner supplies changes that must be blocked and changes that must be permitted. It also runs the whole rule set against known-good work, because two individually sensible refusals can combine to block all valid work. Gradual activation follows: warning, refusal in an isolated setting, refusal for a limited class of live changes and wider authority only while the observations remain inside stated bounds.
