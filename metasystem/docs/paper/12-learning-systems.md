...
The loop closes only when later production behavior tests both outcomes. If the clock failure recurs, the enforced rule has not lived up to its authority. If legitimate work is blocked, its scope is wrong. If the lockout burden changes or affected people overturn the earlier balance, the ruling must change. A system learns not when it accumulates prohibitions, but when observed experience produces a revisable difference in future behavior. The session-expiry incident has then taught a lesson: one part becomes an enforced rule against an evidenced failure, and the part that requires judgment stays with the responsible authority who can answer for it.
