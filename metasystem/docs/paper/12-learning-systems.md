...
Concretely, a rule is three things. The check is code, wired into the path of the action it controls, so the refusal happens whether or not anyone remembers it. Its threshold is often data, a recorded value the check reads, and changing that value is an act of authority, not an edit. Its government is a record. Documentation can describe a rule; it cannot be one. Where the code lives follows Chapter 1's split of the metasystem: the generic half supplies the boundaries where checks run and the rules that apply to any application's delivery, while the application-specific half accumulates rules like the clock rule, which mean nothing outside this application. Application code is never the home of a rule; it is what the rules judge.
