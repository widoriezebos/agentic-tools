...
The loop closes only when later production behavior tests both outcomes. If the clock failure recurs, the evidence no longer supports the enforced rule's authority. If legitimate work is blocked, its scope is wrong. If the lockout burden changes or affected people overturn the earlier balance, the ruling must change. A system learns not when it accumulates prohibitions, but when observed experience produces a revisable difference in future behavior.
