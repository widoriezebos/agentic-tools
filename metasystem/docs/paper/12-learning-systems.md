...
The expiry release also raises a different question. Support data shows that more people must sign in again, and accessibility representatives explain that repeated authentication places unequal burdens on some users. No clock check can decide how much lockout burden is acceptable in exchange for a shorter unattended session. The disagreement concerns the product's values and the distribution of harm; it is not a defect with one mechanically correct answer.
...
The loop closes only when later production behavior tests both outcomes. If the clock failure recurs, the evidence no longer supports the enforced rule's authority. If legitimate work is blocked, its scope is wrong. If the lockout burden changes or affected people overturn the earlier balance, the ruling must change. A system learns not when it accumulates prohibitions, but when observed experience produces a revisable difference in future behavior. The session-expiry incident has then taught a lesson: one part becomes an enforced rule against an evidenced failure, and the part that requires judgment stays with the responsible authority who can answer for it.
