...
Concretely, a rule is three things. The check is code, wired into the path of the action it controls, so the refusal happens whether or not anyone remembers it. Its threshold is often data, a recorded value the check reads, and changing that value is an act of authority, not an edit. Its government is a record. Documentation can describe a rule; it cannot be one.
