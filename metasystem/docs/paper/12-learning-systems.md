...
Suppose the proposed clock rule refuses any session-limit calculation that reads local wall time. Before it receives that power, its sponsor must show more than the original failure. A deliberately broken candidate using local time should fail the check. Correct candidates using an elapsed-time source should pass. Code that merely formats a local time for display should also pass. These cases establish both sides of the boundary: what must be stopped and what must remain possible.
...
Adoption can still be proportionate. A narrowly understood recurrence with severe consequences may justify a firm rule after strong reproduction and independent examination. A weak association may justify observation only. The evidence threshold is part of the decision, not a universal number. The responsible authority decides what uncertainty is acceptable for the consequence at hand and leaves that reason in the record.
...
The record calls this a near-miss and preserves its uncertainty. It does not claim that nobody was harmed merely because no report arrived, nor that harm certainly occurred because it was possible. It records the exposure, the intervention, and the missing evidence. That wider base can strengthen the clock rule, expose a weak production signal, or justify a narrower rollout next time.
...
The expiry release also raises a different question. Support data shows that more people must sign in again, and accessibility representatives explain that repeated authentication places unequal burdens on some users. No clock check can decide how much lockout burden is acceptable in exchange for a shorter unattended session. The disagreement concerns the product’s values and the distribution of harm, not a defect with one mechanically correct answer.

Machinery can gather the sign-in rate, identify affected paths, compare alternatives, and show where the evidence is weak. It cannot make the value ruling or take accountability for it. The lesson therefore remains a question routed to the responsible authority. The ruling records the chosen balance, its reasons, its scope, the evidence considered, and a date for reconsideration. Later builders receive it as prior guidance, while affected people retain a route to challenge it.
...
After containment, the builder corrects the elapsed-time comparison, an independent examiner reproduces the old failure and challenges the repair, and an enforced rule binds the passing evidence to the exact candidate. The candidate lesson then follows its own path through evidence, ownership, testing, limited activation, and appeal. One result becomes the clock rule. The question of acceptable lockout burden remains a human ruling with an owner and review date.

The loop closes only when later production behavior tests both outcomes. If the clock failure recurs, the enforced rule has not earned its authority. If legitimate work is blocked, its scope is wrong. If the lockout burden changes or affected people overturn the earlier balance, the ruling must change. A system learns not when it accumulates prohibitions, but when observed experience produces a revisable difference in future behavior. The session-expiry incident has then taught a lesson: one part becomes an enforced rule against an evidenced failure, and the part that requires judgment stays with the responsible authority who can answer for it.
