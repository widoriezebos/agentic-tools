...
Routine intervention is limited. Humans should not review every machine action, relay routine state by hand or serve as an unrecorded recovery mechanism. Their attention is needed for the decisions a person must answer for and for examination where shared machine blind spots or possible harm require an independent human view. Time must also be available to develop that judgment. For a decision, the system presents a bounded question, relevant evidence, options and the consequence of delay.
