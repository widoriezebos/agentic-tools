...
Access limits have a hard boundary. They can prevent a mind from reading something next; they cannot remove what that mind has already learned. A person who built the session change cannot become independent just by receiving a new label and losing access to the earlier notes. A machine worker that has already absorbed the builder's sequence of ideas and failed attempts cannot supply a fresh view just because later permissions hide that sequence. Role-scoped access provides least authority. Only a fresh mind provides a fresh perspective.
