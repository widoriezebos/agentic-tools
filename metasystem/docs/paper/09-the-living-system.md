...
Such a crossing is a trust boundary, and each one is handled by a mechanism, not by how convincing the material sounds. The package's urging is text, not a command: the tools that read dependencies hold no secrets to reveal, and the dependency itself enters pinned to an exact version with a checksum and a change history, so a substituted copy shows. Generated code is only a candidate: it takes the same examination route as any other change and runs with no more permission than its builder had. The checking tool cannot simply declare success: every result is recorded with the tool, the exact candidate and the conditions of the run, and a tool that passes everything is exposed by the known-broken candidate it must fail on. Where the consequence requires independence, one component's claim is verified by another that would have to be compromised separately.

Least authority limits what a compromised part can reach. Isolation limits how far the harm can spread. The preserved history makes tampering with the record visible. None of these measures proves that the world is safe. Together they replace blanket trust with named boundaries that can refuse, contain and expose hostile action.
