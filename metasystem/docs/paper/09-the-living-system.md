...
Such a crossing is a trust boundary, and defending it takes no new machinery: the roles, records, verification and access rules of the earlier chapters do the work, applied at the boundary. Each defense answers exactly one of these attacks; an attack not named here gets nothing from them and needs the same design question asked again. The urging package fails on access: dependency text is data, not a command any tool obeys, and the builder holds no live secrets, so there is nothing to reveal. The swapped dependency fails on records: the exact version and checksum of the examined copy are recorded, and the build refuses anything that does not match, so the copy that runs is the copy that was examined. The forged-marker path fails on roles: generated code is only a candidate, and it reaches users through independent examination and custodian acceptance like any other change. The lying check fails on verification: its results are recorded against the exact candidate, tool and conditions, and a check that cannot fail on the preserved known-bad candidate proves nothing. The record editor fails on access and custody: the records are owned by the custodian and writable by no other actor, so the edit needs a second compromise, and even an edited record fails at release, where an enforced rule checks the evidence chain against the exact candidate.
