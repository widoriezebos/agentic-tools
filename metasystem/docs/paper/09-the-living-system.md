...
Such a crossing is a trust boundary. This paper does not offer a security design; defending a delivery system against attackers like these needs a treatment of its own. What the structure built so far offers is the ground such a design can stand on, in the paper's own terms. Access decides what an attack can reach: a builder that holds no live secrets has none to reveal, however persuasive the text that asks. Roles decide who must be convinced: generated code, whatever produced it, is only a candidate, and nothing in it reaches users until independent examination and custodian acceptance let it through. Records decide what an attacker must falsify: results bind to the exact candidate, tool and conditions, and the history is owned by the custodian, so making a refused candidate look accepted takes more than one compromise. Verification decides what a lie must survive: an examination can include runs that are required to fail, and a tool that reports success without running passes a case that must fail, which exposes it. These are attachment points for security work, not a completed defense. How far they hold against a determined attacker is a question this paper leaves open.
