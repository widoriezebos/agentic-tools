...
Repair then tests a revised classification against this case and against cases that should remain on the path governed by automatic checks. A rule that sends every harmless wording change to a person may prevent one miss by making the system unusable. Some lessons can become an automatic refusal; others remain guidance for an independent examiner because their meaning depends on context. The aim is a better dividing line rather than an ever-growing collection of permanent enforced rules.
