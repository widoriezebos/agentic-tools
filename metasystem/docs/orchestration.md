...
A brief asks the delegate only for verification it can actually perform. Initial and follow-up implementer briefs carry the same shared-testing requirement: run focused tests and the fast gate while developing and leave what is returned in the worktree; the proof of the round is the orchestrator's, made by its enrolled engine on the worktree's snapshot (`metasystem job prove-round`) and recorded where the next round and the landing reuse it, because a job worktree has no enrolled engine and an attempt recorded inside a sandbox is evidence nobody else can reach. The validation suite is not an automatic delegate requirement: process-owning selections need real process visibility that delegate sandboxes may deny, so the orchestrator runs the named runtime selection outside the sandbox and the brief says so (KI-15). Demanding the impossible turns correct delegate behavior into a gap-stop.
...
The foreground `metasystem wait` verb blocks on durable job, run, proof-attempt, and accepted-ledger records. Its adapter seam is `wait-delivery --wait-id ID --nonce NONCE --deadline RFC3339-UTC --session SESSION-ID`: an eligible runtime prints exactly `blocking` and exits zero, while exit two declines registration. The adapter carries no event evidence; Go rereads the durable source and owns the version-2 waiter row, deadline, and records-only resume.
