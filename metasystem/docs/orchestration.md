...
**The backlog never waits for the human (standing order, Wido 2026-08-27).** While the backlog holds claimable work, the coordinator works it — nights, absences, and the hour change nothing; a deferred start (a morning timer, a park until the human returns) is a violation even with a wakeup armed. A parked STREAM never means a parked COORDINATOR: when one item blocks on a reserved decision or a structured budget refusal, raise it and claim the next item; go quiet only when the next tracked step is already in flight. The ONLY exception is the human explicitly saying stop, or explicitly directing the work elsewhere.
