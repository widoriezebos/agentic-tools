...
8. Run `scripts/validate-metasystem.sh`, then a focused project build and test. The validator invokes setup's check mode for the installed runtime set and validates each real event, action, runtime, matcher and timeout. `CONFIG_READY` proves only that files are structurally ready. Provider trust or a provider restart remains a separate provider-owned step, and neither fact proves that a lifecycle event has been observed. Do not edit trust stores or credentials as part of setup. Arm a repository's first supervision explicitly with `bin/metasystem up --repo <path>`; later session hooks are the steady-state path. Claude retains its receipt notice and fail-closed Stop behavior, Codex rejoins after compaction, and Devin's non-tool lifecycle groups have no matcher. Hard checks must not depend on anyone remembering to run them.
