...
- **Write to a human, in plain English.** Spell out identifiers at first use ("KI-4, the slow process scan"), give numbers their meaning, never a status line of ids and jargon alone. A sentence that would not survive being read aloud to a colleague who has not seen the repository gets rewritten — every summary, return, refusal, commit message. "Load-bearing" is banned; name what depends instead. Messages to the human additionally follow `docs/seat-communication.md` (all delegator seats, all machines; Wido 2026-08-31).
