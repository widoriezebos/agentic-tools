...
- **Source comments speak the application's language, in plain English.** A comment states a constraint or reason in terms of the system itself — its components, invariants, and failure modes — never in terms of the process that produced the change. No review-round or finding numbers, no slice or iteration names, no "previously/now" narration of the development history: the next reader was not there and cannot follow any of it. When the why rests on a behavior, name the behavior ("a rejected return also parks and mints an ask"), not the event that surfaced it. Provenance belongs in commit messages and decision records, never in the code.
...
- **Strictness guards invariants, never conveniences.** A check may refuse loudly only when it protects a named invariant whose violation is a real defect. A rule that breaks on benign variation — an arbitrary size cap, a missing lawful path that forces workarounds, a format nit a reader would shrug at — is itself the defect: handle the variation intuitively (normalize it, allow it, or provide the sanctioned verb) or do not encode the rule. If no invariant can be named for a check, it is a convenience masquerading as a rule.
