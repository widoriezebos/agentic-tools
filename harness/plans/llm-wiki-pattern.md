...
- Status: critique round 5 adjudicated; parked for human review until the lifecycle checkpoint below
- Review-by: 2026-09-03 or the next harness retro, whichever occurs first
- Goal: determine what Andrej Karpathy's "LLM Wiki" pattern is, whether it solves a current harness problem, and, only if the evidence supports it, design the smallest robust integration for later implementation approval.

## Decision to Make

Should this harness gain an LLM-oriented wiki/knowledge mechanism? A positive decision requires all of the following:

1. A primary-source description of the pattern and its intended benefits.
2. A named current harness consumer and workflow that existing owners do not already serve.
...
- Trace the current harness's instruction routing, task-local plans, durable documentation, agent mission state, receipts/retrospectives, and validation paths.
...
- Building a general-purpose RAG platform, vector database, or autonomous documentation system without a demonstrated harness consumer.
- Treating generated summaries as authoritative truth without source provenance and human-owned correction paths.
- Importing Karpathy's implementation shape verbatim when the harness has different ownership or lifecycle constraints.
...
| --- | --- | --- |
| `AGENTS.md`, `wow.md`, `plans/README.md`, `docs/project-rules.md`, `docs/collaboration.md`, and `docs/design/design-principles.md` | Checked by reading | The harness already distinguishes canonical policy, task-local evidence, and durable ledgers; any wiki must respect those ownership boundaries. |
| The concurrent worktree and `HEAD` changed repeatedly during research as validation and orchestrator work landed; by the final verification checkpoint only this plan remained untracked | Observed with repeated `git status`, `git diff`, and `git rev-parse` | The peer stream resolved without this task absorbing its files. Any later implementation must still re-baseline and treat unrelated changes as user/peer-owned. |
...
- [Knowledge Compounding](https://arxiv.org/abs/2604.11243) reports large token savings over four sequential queries. Four queries are too few to establish a portable harness contract, so this is supporting context only.
...
- Exact state at the final verification checkpoint: repository `46cbba7` with only this untracked plan. Earlier checkpoints contained concurrent validation and orchestrator changes that landed independently during the research.
...
Uncertainty: The current harness does not yet show repeated query demand. [S:abcdef0123456789abcdef0123456789abcdef0123456789abcdef0123456789:L30-L33]
```
...
Model-backed operations record the model, source ids, affected page ids, and verification result in the task receipt note. Paid raw responses follow the harness's existing replay and durable-evidence rules. No separate telemetry store is introduced.
...
7. Run the exact preregistered number of development candidate repetitions, no fewer than three. Each repetition initializes an empty corpus, independently compiles it from the same raw sources in a fresh session with the frozen procedure, and answers the fixed questions in fresh sessions. Freeze the complete candidate code, prompts, schema, selector, model configuration, and corpus-building procedure after a valid development gain.
...
The existing `skills/improve/SKILL.md` owns this evaluation. The first post-trigger deliverable is the scoring evaluation and frozen prompt packets, not the wiki. Version 1 deliberately has no script that launches models. A named human operator runs each generated packet in a fresh top-level session through the selected runtime's normal surface, exports provider usage from the authoritative source, and records the response. This avoids inventing a dispatcher role or bypassing the harness adapters.
...
| 6 | Independently rebuild the development corpus in every preregistered candidate repetition under improve mode | Exact declared count of fresh compilations; development candidate mean meets the exact target and every aggregate/run guard passes |
...
| LLMW2-003 | ACCEPT | The proposed evaluator named model sessions without an executable owner in the harness. | Assigned model execution to a named human operator using fresh normal-runtime sessions. The planned script only prepares immutable packets, validates exported usage and response joins, and scores results. |
...
| RW-OPTIONAL | HIGH | `harness-architecture.md:14-17`; current-consumer gate | Default adopters pay no corpus, instruction, or dependency cost | `optional-skills/research-wiki/` and `scripts/adopt.sh` | Planned opt-in copy path | Planned paired adoption fixtures | Planned fresh-adoption inspection | PARTIAL | Critique adoption mechanics, then implement |
...
- The design still requires the round-6 closure check and human acceptance before implementation.
...
- File-and-line map of the relevant current harness behavior.
