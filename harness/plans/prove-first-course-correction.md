...
- Next step: finish item 20b's last mile. The implementation exists and is preserved, unverified, on branch `agent/implementer-20260804t205954z-5650` (commit `3681c64`): `mission-prompt.py`, `mission-runner.sh`, the `claude` and `fake` host adapters, the cron example, the `turnId` stamp in `dispatch.sh`, and the runner fixtures. The suite reaches `runner-cycle-start`, which exits 3 because supervision does not arm inside the fixture's mission repository within its 45-second ceiling. Fix that fixture, get the suite green, then review the diff and merge; then Mission Zero
...
| CC-3 | Prove-first: build the minimal runner and run Mission Zero, the smallest real unattended mission (make one failing test pass on a scratch repository), before any further mission-mode design | IN PROGRESS. D-CC1 assumed the 6.2a prompt artifacts were already shipped; they were not, so CC-3 splits. **20a DONE** (`fa2be35`): orchestrator preamble, return schema, turn-instruction body, `assert-turn-prompt.sh`, orchestrator registered in `assert-return-complete.sh`. **20b BUILT, UNVERIFIED** on branch `agent/implementer-20260804t205954z-5650` (`3681c64`), one fixture short of green;: the prompt assembler, one host adapter (`scripts/agents/hosts/<runtime>.sh start-turn`), and the minimal runner loop over lease, one turn, runner-side measurement, one ledger entry, four end states. Then Mission Zero itself. Fences per D-CC3 unchanged: cycles 5, jobs 5, concurrency 1, job cap 10 minutes, wall clock 1 hour, cheap delegate models |
