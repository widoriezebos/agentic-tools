...
- Next step: diagnose KI-10, why the mission runner's fixtures kill the suite on a two-core CI runner while passing locally. The runner itself is finished and proven locally on branch `agent/implementer-20260805t063758z-3648` (`fac4676`, suite green in an unrestricted environment); main was reverted at `bbd89b2` only to keep the gate trustworthy. Fix CI, re-merge, then Mission Zero
