...
A detached launch is not trusted on say-so (C1-18): the adapter writes `running` only on the runtime's session-established signal, the event that carries the new session or thread id, which no runtime emits before successful authentication (C2-14, predicate per C3-15). Output that is not that signal, an authentication error or a login prompt included, leaves the record `pending` until the grace period expires into `failed`. Dispatch waits a short grace period for that flip plus the transcript sidecar's existence; a handshake that does not complete marks the record `failed` with the error named and exits nonzero, so a missing CLI or expired authentication surfaces at dispatch time instead of as an overnight hang.
...
| `selftest` | A scripted sequence exercising the full contract, not one ping (C2-22): dispatch a trivial brief, probe mid-run, verify the return shape, follow-up once and assert the same session was resumed, cancel a second scratch dispatch and assert `cancelled` was recorded (C3-8), prove the permissions mapping for real by attempting a permitted read and a forbidden write in a scratch workspace (3.6), confirm the constructed CLI invocation carried the mapped flags, and extract usage where the runtime reports it. Spends a small number of real calls; run manually on adoption and after CLI upgrades |
...
artifacts/agents/capabilities/<runtime>-<version>-<date>.json   immutable capability snapshots from probe (3.8)
```
...
```json
{
  ...
  "branch": "<job branch>",
  "permissions": {"requested": {"preset": "workspace", "writeRoots": ["<worktree>"], "network": "deny", "approvals": "deny"}, "effective": null},
  "capMin": 180,
  "pid": 12345,
...
Supervision is the existing contract, reused rather than reinvented: before the first dispatch, arm the two standing processes once per session (C2-1): `scripts/watch-background-jobs.sh --dir artifacts/agents/jobs --scope <repo-root>`, which reports, and `scripts/agents/dispatch.sh reap --interval <sec>`, which transitions and mirrors. Liveness never depends on output cadence, honoring the existing rule that absence of output is not absence of progress (C3-12): the adapter supervisor touches a `<job-id>.hb` sidecar on a short timer for as long as the CLI process lives, so the watcher's newest-sidecar-mtime rule sees a heartbeat even through long silent stretches, with the transcript contributing when it does advance. Each record has one lifecycle, so the watcher's report-once state matches (C2-9).

Job payloads are paid artifacts, and `artifacts/` is a wipe-safe directory, so the two are reconciled the way `plans/README.md` already requires (C1-16): when a job reaches a terminal status, `reap` mirrors it in a defined order (C2-20, C3-10): copy the transcript and payload to `<evidence.root>/agents/<job-id>/` (the conf key that gives `plans/README.md`'s durable evidence root a machine-readable home, C3-11), hash every copied file and verify, write the manifest at the destination, set the `mirror` object in the local record atomically, and copy the record file itself last, so the mirrored record is the final local record and the manifest covers everything except the record that carries it. Until that mirror exists, the job's files are the only copy of paid output and are not disposable.
...
21. The hook receiver and the three payload translators (report R7), with a defined event contract (C3-22): each hook drops one JSON file into `artifacts/agents/hooks/incoming/` named by timestamp and dedupe key, fields `runtime`, `event`, `sessionId`, `jobId` where known, `ts`, `dedupeKey`; the translators are per-runtime hook configuration templates that write those files; the runner consumes and deletes them, deduplicating by key; the transport is the filesystem, no socket and no daemon, and the polling path is proven equivalent in a fixture.
