...
- When a record reaches terminal status, `reap` mirrors that record's own files to `<evidence.root>/agents/<root-id>/` (the conf key from C3-11), hashing each file and re-reading the copies to verify, and writes the destination manifest last, covering everything copied so far. The mirrored set is enumerated (C5-10): the record json, the transcript, the round directory including `diff.patch` (written there by `assert-conformance.sh`, so the worktree itself never needs mirroring; the work's substance is the diff plus the base commit), the chain's `brief.md` with the first mirrored record, and the snapshot file the record names (C4-16). The non-null `mirror` object is `{path, manifest, mirroredAt}`, with `manifest` the destination manifest file's own hash.
...
- Disposability is chain-scoped and closure is explicit (C5-9): the orchestrator closes a chain after review with `dispatch.sh close --job <root-id>`, which stamps `chainClosed` in the root record; follow-up refuses on a closed chain. The payload directory and worktree are disposable only when the chain is closed, every record is terminal, and the destination manifest covers them all, so a completed parent awaiting a possible correction round is never disposable by accident. `evidence.root` must be an absolute path outside the repository and outside every build tree, and both the audit and dispatch validate that shape before any job runs (C4-11).
