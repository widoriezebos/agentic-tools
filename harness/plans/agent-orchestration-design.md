...
The record has one writer path and a legal-transition table (C2-4): every mutation goes through a single helper in `dispatch.sh` that writes to a temporary file and renames atomically, with a compare-and-set on status. The complete table (C3-1): `pending` to `running` on the handshake's session-established signal; `pending` to `failed` when the handshake fails; `pending` to `cancelled` when a cancel lands before start; `running` to exactly one of `completed`, `failed`, `cancelled`, `timeout`. No other transition exists, terminal is final, the first terminal write wins, and a later writer (a completion racing a cap, a reap racing an adapter) re-reads and no-ops. The adapter writes `completed` and `failed` for its own run; `reap` owns the rest.

`reap` sweeps job records, proves ownership of each recorded PID before touching it (the shared-machine rules verbatim), and transitions: past the record's own `capMin`, persisted at dispatch so the sweep needs no outside knowledge (C2-2), it winds the job down and writes `timeout` with the reason `budget-cap`; a record still `running` whose process is gone becomes `failed` with the reason `process-lost`. Wind-down is executable and defined (C2-3): TERM to the proven-owned PID, a grace period, then KILL. That is safe here by construction: the job worktree is disposable by design and conformance reviews the computed diff, so a torn workspace write cannot enter certified work, and the no-mid-write rule protects what the harness itself owns, the record and the mirror, which only the atomic helper touches. `reap --interval <sec>` runs as the session's standing reaper, and the arm-once contract becomes two standing processes armed together before the first dispatch: the watcher reports, the reaper transitions and mirrors (C2-1). The two caps are ordered so they cannot disagree (C3-13): `watch.cap-min` is the global backstop, dispatch refuses any job `capMin` at or above it, and the reaper's interval is shorter than the watcher's, so the true terminal transition lands before the backstop would fire. Exit codes (C1-19, C2-15, C3-2): `--wait` maps 0, 3, 4, 5, 8 to `completed`, `failed`, `timeout`, vanished, `cancelled`; `status` prints the record's status and exits 0, or 6 for an unknown job id, or 7 for an unreadable or malformed record; 2 is usage error everywhere.
...
- `permissions.requested` embeds the full expanded envelope, every field, with `preset` recording which shipped preset expanded to it, or `custom` (C4-6). `permissions.effective` is what the adapter actually mapped onto the provider, written at handshake.
- Process ownership is a process group (C4-14, the round 4 blocker): the supervisor starts the runtime CLI in a fresh process group and records both `pid` (the supervisor) and `pgid` (the group). The supervisor writes the terminal status before exiting on every path it survives. When `reap` finds the supervisor dead under a `running` status, the order is fixed: prove ownership of the group by `pgid` plus `instanceTag`, kill the group so no orphan child can keep writing, then write `failed` with `process-lost`, then mirror. A job is never declared dead while its processes can still mutate the worktree.
- `capMin` defaults from the conf's `dispatch.cap-min` (shipped default 120) and dispatch refuses any value at or above `watch.cap-min` (shipped default 180), so the watcher's backstop always sits strictly above every job cap (C4-13, C3-13).
...
- When a record reaches terminal status, `reap` mirrors that record's own files (its record json, its transcript, its round directory, and the snapshot file it names, C4-16) to `<evidence.root>/agents/<root-id>/`, hashing each file and re-reading the copies to verify, and writes the destination manifest last, covering everything copied so far.
...
- Disposability is chain-scoped: the payload directory and worktree are disposable only when every record of the chain is terminal and the destination manifest covers them all. `evidence.root` must be an absolute path outside the repository and outside every build tree, and both the audit and dispatch validate that shape before any job runs (C4-11).
