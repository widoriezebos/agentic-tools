...
- **Host.** The runtime and model that hold the orchestrator for this mission; the runner drives it through the matching host adapter (C3-20). Which agent the human talks to interactively is still never configured; this field only governs unsupervised turns.
...
The loop is advanced by a deterministic controller, never by hope that a session wakes up (report R1): `scripts/agents/mission-runner.sh` is a foreground process the operator starts, owning waiting, timeouts, mission state transitions, and the start of each next orchestrator turn, which it launches headlessly through a per-runtime host adapter with a defined contract (C3-20): `scripts/agents/hosts/<runtime>.sh start-turn --mission <file> [--resume-session <id>]` runs one orchestrator turn on that runtime with the mission file and mission ledger as standing inputs, prints the host session id for continuity, and exits with a mapped outcome (0 turn completed, 3 turn failed, 6 session unresumable); on 6 the runner starts a fresh host session whose first input is the mission file, the mission ledger, and the batched-asks list, records the new session id, and notes the discontinuity in the mission ledger, because losing host context is a fact to record, never a silent restart (C4-20). Hooks may wake the runner early, and one shared hook receiver with three small payload translators turns each runtime's lifecycle events into notifications (report R7), but correctness never depends on a hook firing: polling the job records and the mission ledger is the fallback that always works. Mission lifetime is decoupled from host-session lifetime: a host session ending must never kill mission jobs that are allowed to survive a restart, and the runner resumes a mission from its files alone.
