...
- In flight right now: critique loop pass 2 (mission mode and documentation), round 1 dispatched; pass 1 (mechanism) closed EXECUTABLE at round 7 with both parties concurring
...
scripts/agents/dispatch.sh --role <role> --brief <file>
    ...
    [--workspace <dir> | --worktree] [--permissions <preset|envelope-file>]
    [--wait] [--cap-min N]
...
| Merge conflicts between peer agents go to the human | Neither agent resolves by force | Reduced inside one mission by serial merging and one worktree per delegate, and a conflict the orchestrator cannot resolve as trivially disjoint parks the stream as reserved (corrected at C8-19: concurrency means prevention was never true); across missions, park |
...
Mission mode turns the mechanism of Part 3 into an unsupervised loop. Rewritten in one pass at critique round 8, whose findings shared one generating cause: the first draft let the loop be constrained by artifacts the loop itself could write. The organizing principle of the rewrite is the separation of signed authority from loop-mutated state, with tamper-evidence as the enforceable boundary: the harness cannot make an orchestrator physically unable to touch its own fences without operating-system controls it cannot assume, so it makes every touch detected, parking, and attributable instead, and it moves every measurement that gates continuation out of the orchestrator's hands into the deterministic runner's.

### 6.1 Authority and state, split (C8-2, C8-9)

Two files per mission, with disjoint writers:

- **The contract**, `plans/mission-<mission-id>.contract.md`: human-signed authority, frozen for the mission's duration. Its sections: the intent, stated observably and in plain language; the acceptance gate (one runnable command, its threshold vector, the metric direction, and the certification status of the reference truth); the guard metrics with floors, noise floors, and measurement commands; non-goals; the fences (mission wall clock, total cycles, total jobs, concurrency, per-job timeout, plus any provider-native caps); the pre-authorized envelopes, each bounded; the host declaration (runtime and model for unsupervised turns); and the worst-case exposure line (6.3). Machine-consumed values live in one fenced key=value block with a defined grammar, so `assert-mission.sh` and the runner parse a format, never prose (C8-9). The contract also records, at preflight: the gate-integrity set (the repository paths and content hashes of the gate command's implementation and reference truth) and the baseline failure set (which tests are red on day zero, C8-18).
- **The state**, `plans/mission-<mission-id>.state.json`: written only by the runner through the same atomic single-writer helper jobs use. It holds the stream table (each stream `active`, `parked-reserved` with its batched ask, `parked-stop-loss` with its ledger pointer, or `done`), the cycle ledger, the waiting-on-the-human list, the runner lease, and the current turn. The mission id is the identity everywhere; "stream" names only the work subdivisions inside a mission, never the mission itself (C8-20), and job records carry the mission id.

The contract's hash is recorded in the state at preflight. A contract that changes mid-mission is a human act by definition: the runner detects the hash change before every turn and parks the mission with a `contract-changed` ask, resuming only after a fresh preflight (C8-2).

### 6.2 The loop, the runner, and the host turn

One cycle: the runner starts an orchestrator turn through the host adapter; the orchestrator picks work from active streams, writes briefs, dispatches delegates through the roster, reviews and certifies per Part 3, and ends its turn; the runner then measures and records.

- **Host turns are jobs (C8-5).** The runner dispatches each orchestrator turn through the dispatcher itself, role `host`, so a turn gets a record, a cap, group-scoped kill coverage, typed usage, and cancellation exactly like a delegate. The host role's envelope (C8-1) grants what orchestration genuinely needs (the repository, the mission state's directory read-only, dispatching) and its record makes every turn's cost and duration visible. The residual is stated, not hidden: an orchestrator with repository write access can, in principle, touch anything in the tree; the boundary is tamper-evidence (below), and prevention beyond it would require controls outside the harness's reach.
- **Mission accounting is unforgeable by the accounted (C8-4).** The runner owns a mission context (the lease in the state file plus a control-plane marker under `artifacts/agents/missions/<mission-id>/`); the dispatcher stamps every job dispatched during a leased mission with that mission id from the context, and the brief's mission line is informational only. Fences count what the context stamped, so a host cannot write work out of the accounting.
- **Progress is measured by the runner, never self-reported (C8-8).** After each turn, the runner itself runs the acceptance gate's cheap form or the declared metric command, computes the delta against the previous cycle, and appends the cycle entry to the ledger with that measured value. The orchestrator's own classification is recorded as commentary; the triggers read the runner's measurements. A turn that ends without the runner being able to measure is recorded as no-progress by default, so omission cannot buy another cycle.
- **Stop-loss semantics match the script that enforces them, verbatim (C8-7).** The contract declares `Cycle budget: N` (total cycles, the script's meaning) and `No-gain budget: M` (trailing cycles without a `contract-improved`, the semantics REM-3 shipped); `assert-stop-loss.sh` runs unmodified against the mission ledger, and `contract-improved` is written only when the runner's measured delta clears the declared noise floor.
- **Tamper-evidence before and after every turn (C8-3).** The runner hash-verifies the frozen set: the mission contract, the gate-integrity paths, the reference truth, `harness.conf`, the roster section, and the instruction files. Any drift parks the mission with a `gate-integrity` ask naming the changed file. The conformance guard for delegates already covers `plans/` and the control plane; the gate-integrity paths join that guard, so no delegate diff can touch them either.
- **The runner is supervised like everything else (C8-11, C8-12).** It runs detached with its own job-style record under `artifacts/agents/jobs/` (the existing watcher covers it), a heartbeat, and a singleton lease in the mission state: pid, instance tag, started and renewed timestamps. A second start refuses while the lease holder is alive and takes over only after proving the holder dead, the same ownership proof as every kill. The state records the current turn (id, host session, started); on restart, an open turn whose host session is gone is marked aborted and the cycle re-runs idempotently by turn id. Nothing depends on the runner being lucky: its death leaves jobs governed by the reaper and watcher, and the mission simply does not advance until a runner holds the lease again.
- **Host turn outcomes are mapped (C8-15).** Exit 0 advances; exit 6 (session unresumable) starts a fresh host session seeded with the contract, ledger, and asks, recording the discontinuity; exit 3 (turn failed) retries once, and a second consecutive failure parks the mission with a `host-failure` ask.
- **Mission end states are complete (C8-14).** A mission is `running`, `completed` (the full acceptance gate passed, verified by the runner), or `parked` with a reason: `stop-loss`, `fence` (which fence), `gate-integrity`, `contract-changed`, `host-failure`, or `all-streams-parked` (no stream active, gate unpassed). Mixtures resolve deterministically: `done` streams are inert, and the mission parks when nothing is active and the gate has not passed. Every end state is machine-recognizable and none requires a human awake.

Hooks remain acceleration only (report R7): they may wake the runner early; polling the records and the ledger is the correctness path; mission lifetime is decoupled from host sessions, and a host `SessionEnd` never kills mission jobs.

### 6.3 Enforcement, fences, and evidence

- `scripts/assert-mission.sh --file <contract> [--preflight]`: validates the contract's structure and grammar, every envelope bounded and drawn only from the pre-authorizable categories (C8-17: the reserved-decisions list in `docs/project-rules.md` gains a per-item `pre-authorizable: yes|no` marking, with production data, publishing, and history rewrites marked never); `--preflight` additionally requires the gate runnable with its baseline and integrity hashes recorded, the baseline failure set captured, the watcher AND the reaper armed (C8-16), and the runner lease acquirable. Unsupervised operation without a passing preflight is a refused command.
- **The fences are enforceable bounds, and the money claim is honest (C8-6).** The machine enforces the lifecycle fences (wall clock, cycles, jobs, concurrency, per-job timeout) plus provider-native caps where they exist. Monetary spend across providers is not machine-enforceable without native caps, so the contract must carry the computed worst-case exposure (jobs times per-job cap times the costliest rostered tier, plus native-cap totals) and the human signs that line against the spend fence in `docs/project-rules.md` at preflight; `assert-mission.sh` refuses a contract whose exposure line is missing or stale. Absence of the human never widens spending authority, because the authority was sized and signed before the human left.
- **Mission evidence is a first-class proof bundle (C8-23).** The runner mirrors the contract, state, cycle ledger, gate outputs, baseline set, waiting list, and the rehearsal's documentation-defect log to `<evidence.root>/missions/<mission-id>/`, hash-verified, at every park or completion and on a cadence while running. The delegate-job mirror of 3.4 is unchanged; this bundle is the mission-level counterpart.
- **The runner CLI is a public contract (C8-21)**: `mission-runner.sh start|resume|status --mission <id>`, exit codes 0 running-or-completed handoff per subcommand, 3 refused (lease held, preflight missing, contract hash mismatch), 7 state unreadable; defined here because the docs-only rehearsal must be able to name every command it needs.
- Phase 5 fixtures, all through the fake adapter: contract grammar accepted and each section rejected when missing, unbounded, or drawn from a never-eligible category; preflight refusing an unrunnable gate, an unarmed reaper, and a stale exposure line; the fence set refusing dispatches past each bound with the batched ask written; runner-measured no-progress parking the mission under the script's unchanged semantics; a favorable self-classification failing to override the runner's measurement; a mid-mission contract edit parking with `contract-changed`; a gate-path edit by a delegate failing conformance and by anything else parking with `gate-integrity`; lease takeover refused while the holder lives and succeeding after proven death; an aborted turn re-running idempotently; host exit 3 retry-then-park; a reserved decision outside an envelope parking one stream while another continues; and mission end mapping for each reason.

### 6.4 What stays human, even at 3am

The five behaviors in Part 5 are binding rules of mission mode, and the one-line law stands: the human's absence narrows what the loop may do, never widens it. Reserved decisions outside a bounded envelope park their stream; tests that are red against the recorded baseline park their stream, while the baseline reds the mission exists to fix are the goal, tracked by the gate (C8-18); merge conflicts between concurrent delegates that the orchestrator cannot resolve as trivially disjoint park the stream as reserved, because the peer rule was always that force-resolution is human work (C8-19); instructions, conf, roster, and the mission contract are frozen, with drift detected and parking rather than trusted (C8-2, C8-3); retro proposals queue; and no envelope can authorize what `docs/project-rules.md` marks never pre-authorizable.
