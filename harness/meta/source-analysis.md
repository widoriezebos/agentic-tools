...
| Reference-truth law: truth from direct evidence or human instruction only, never from system output; ambiguity leaves truth unchanged; truth-verified and software-passing are separate facts | The source document-processing repository operates this as hard law after real incidents of goldens being edited toward provider output; the template carried only the narrow "never weaken an assertion" form in the verify skill (gap found 2026-07-31) | Design principles (Core Standard) |
| Obligation-gate proof-cell concreteness: DONE/READY_FOR_RUNTIME on critical/high rows require backticked/pathed/named proof or an explicit "Not applicable" with reason | Ported from the source repository's stricter gate, which guarded three shipped matrices; the template's own worked example contained the vague-runtime-proof fault its prose warned about, and the strengthened check caught it (2026-07-31) | `scripts/assert-design-obligation-gate.sh` |
| Design-critique loop with a materiality stop criterion that binds the critic; adjudicate every finding; verdict lines never govern; fix the generating cause once; stop when the loop turns self-sustaining | In the source repository a large design ran 17 critique rounds on prose findings before the criterion; with it, rounds sorted 6/5, 4/3, 6/2 material/non-material and the loop closed; one false approval came from trusting a verdict line over the findings body (2026-07-31) | Design-critique skill |
| Shared-machine coexistence: exact-PID ownership-proven kills only, never build/clean a foreign checkout, paid caches read-mostly, instance-tagged detached processes, untargeted release sweep | One week of incidents on the source machine: pattern kills destroyed three validation runs, foreign clean wiped a paid run twice, and a release note claimed a clean sweep while a tagged JVM still ran (2026-07-31) | Orchestration (Shared Machines) |
