...
- Make primary behavior changes as clean cutovers when feasible, and remove superseded internal paths in the same change. No feature flags, shadow modes, or dual primary paths for internal design corrections; when blast radius is high, make rollback explicit at the true external boundary instead.
