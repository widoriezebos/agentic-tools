...
1. Copy the harness contents into the new repository root, excluding `meta/` (harness maintenance, template-only) and the template's own ledger history (`plans/receipts.log` if present — every project starts its own). Merge shipped dotfiles such as `.gitattributes` into existing ones rather than overwriting them.
...
4. Register the skills themselves with the runtimes in use — without this, no runtime ever auto-triggers them and routing depends entirely on the model reading `wow.md`. For Claude Code, copy or symlink each `skills/<name>/` into `.claude/skills/<name>/` so the SKILL.md description drives auto-triggering; consult other runtimes' manuals for their skill locations. Then register the subagent profiles: copy each `skills/<name>/agents/claude-profile.md` to `.claude/agents/<name>.md`, each `skills/<name>/agents/devin/AGENT.md` to `.devin/agents/<name>/AGENT.md`, and each `agents/openai.yaml` where an OpenAI-style skill launcher consumes it. Skip runtimes the team does not use.
...
7. Put task state in `plans/`, generated evidence in the gitignored `artifacts/` directory at the repository root (create it and add it to the project's `.gitignore`), and durable decisions in code/docs—not in root prompt history.
8. Run `scripts/validate-harness.sh`, then a focused project build/test. Wire the hard checks into machinery, starting from the shipped examples: copy `scripts/enforcement/github-actions-harness.yml` to `.github/workflows/harness.yml` (it installs ripgrep, which the audit hard-requires) and merge `scripts/enforcement/claude-code-hooks.json` into `.claude/settings.json` so retro cadence surfaces at the end of every turn. Hard checks must not depend on anyone remembering to run them.
