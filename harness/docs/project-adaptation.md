...
The fast path is the adopter script, run from the template checkout: `scripts/adopt.sh <target> [--runtimes claude,devin,codex] [--enable debug-java]`. It performs steps 1, 3, 4, 8, and 9 below mechanically, refuses targets that need reconciliation instead of guessing at them, and prints the finish line: fill `docs/project-rules.md` (step 2), then run `scripts/validate-harness.sh`, which must pass with zero placeholders. A script can detect only file-shaped instruction assets; confirm by hand that no agent-directed prose, prompt directories under other names, or agent-encoding hooks and CI exist before treating a repository as fresh. The numbered steps below remain the specification and the manual fallback.

1. Copy the harness contents into the new repository root, excluding `meta/` (template maintenance), the template's own `README.md` and `LICENSE` (template documentation, not payload; copying them would clobber the project's own), and everything in `plans/` except the standing ledgers (`README.md`, `instruction-ledger.md`, `known-issues.md`); task-local plans and `receipts.log` are the template repository's own state, and every project starts its own. Merge shipped dotfiles such as `.gitattributes` into existing ones instead of overwriting them.
