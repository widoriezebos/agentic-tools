...
--session is HARNESS_SESSION_ID or session-<pid>, and --tag is
HARNESS_INSTANCE_TAG or harness-main-<runtime>-<sanitized-session>. If no
...
config=$harness_root/scripts/harness-config.sh
...
arm_repository() {
  ...
  session=${session:-${HARNESS_SESSION_ID:-session-$pid}}
  runtime=${runtime:-unknown}
  safe=$(sanitize "$session")
  tag=${tag:-${HARNESS_INSTANCE_TAG:-harness-main-$runtime-$safe}}
  ...
  owner_tag="harness-supervision-owner-$(sanitize "$(git -C "$repo" rev-parse --show-toplevel)")-$(date +%s)-$$"
