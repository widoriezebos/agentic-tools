#!/usr/bin/env python3
"""Check that this repository runs under the harness it ships.

`adopt.sh` installs the lifecycle hooks into an adopted repository. The template
never adopts itself, so for the whole of this harness's development its own
...
A harness whose own repository does not run under it is testing a claim it never
...
def main() -> int:
    ...
    if "supervision-hook.sh" not in flat:
        ...
        return 1
    # The shipped configuration assumes the harness is the project root, which
    ...
    if "$CLAUDE_PROJECT_DIR/harness" not in flat:
        print(
            "this repository's hooks do not enter the vendored harness directory",
            file=sys.stderr,
        )
        return 1
    return 0
