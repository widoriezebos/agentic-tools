...
def config_value(root: Path, key: str, default: str = "") -> str:
    try:
        lines = (root / "harness.conf").read_text(encoding="utf-8").splitlines()
    except OSError:
        return default
    values = []
    for raw in lines:
...
def started_at(pid: int) -> int:
    ...
    try:
        ...
        fixture = os.environ.get("HARNESS_FAKE_PROCESS_IDENTITY_FILE")
        if not fixture:
            raise
...
def enumerate_fixture(path: Path) -> list[Process]:
    if config_value(HARNESS_ROOT, "harness.runtimes") != "fake":
        raise CensusError("HARNESS_CENSUS_PROCESS_FILE is allowed only when harness.runtimes=fake")
    try:
...
def enumerate_processes(_repo: Path) -> list[Process]:
    fixture = os.environ.get("HARNESS_CENSUS_PROCESS_FILE")
...
def configured_signatures(runtimes: Iterable[str] | None = None) -> dict[str, tuple[list[str], list[str], str]]:
    selected = list(runtimes) if runtimes is not None else [
        item.strip() for item in config_value(HARNESS_ROOT, "harness.runtimes").split(",") if item.strip()
    ]
    if not selected:
        raise CensusError(
            f"{HARNESS_ROOT / 'harness.conf'} lists no harness.runtimes; "
            f"configure at least one runtime with an executable signature adapter under "
            f"{HARNESS_ROOT / 'scripts' / 'agents' / 'adapters'}"
        )
    result = {}
    for runtime in selected:
        adapter = HARNESS_ROOT / "scripts" / "agents" / "adapters" / f"{runtime}.sh"
        if not adapter.is_file() or not os.access(adapter, os.X_OK):
            raise CensusError(
                f"harness.runtimes names {runtime!r}, but its signature adapter is missing "
                f"or not executable: {adapter}; install or enable that adapter, or remove "
                f"{runtime!r} from {HARNESS_ROOT / 'harness.conf'}"
            )
...
def resolve_signature_cwds(
    ...
    if os.environ.get("HARNESS_CENSUS_PROCESS_FILE"):
        return processes
    resolved = []
...
def announcements(fixture_by_pid: dict[int, Process], errors: list[str]) -> list[dict[str, Any]]:
    directory = HARNESS_ROOT / "artifacts" / "agents" / "mains"
...
def run_census(repo: Path, fingerprint: str, interval: int, output: Path) -> int:
    ...
    try:
        ...
        declarations = {}
        resolved_processes = []
    fixture_by_pid = {process.pid: process for process in processes} if os.environ.get("HARNESS_CENSUS_PROCESS_FILE") else {}
...
def fingerprint(repo: Path) -> str:
    repo = Path(os.path.realpath(repo))
    state_path = HARNESS_ROOT / "artifacts" / "agents" / "supervision" / "state.json"
    try:
    ...
    declarations = configured_signatures()
    files = [
        HARNESS_ROOT / "scripts" / "agents" / "arm-supervision.sh",
        HARNESS_ROOT / "scripts" / "agents" / "dispatch.sh",
        HARNESS_ROOT / "scripts" / "agents" / "process-census.py",
        HARNESS_ROOT / "scripts" / "agents" / "adapters" / "runtime-common.sh",
        HARNESS_ROOT / "scripts" / "watch-background-jobs.sh",
    ]
    selected = [item.strip() for item in config_value(HARNESS_ROOT, "harness.runtimes").split(",") if item.strip()]
    files.extend(HARNESS_ROOT / "scripts" / "agents" / "adapters" / f"{runtime}.sh" for runtime in selected)
    file_hashes = {}
    for path in files:
        try:
            file_hashes[str(path.relative_to(HARNESS_ROOT))] = sha256(path.read_bytes()).hexdigest()
        except OSError as error:
    ...
    relevant_config = {
        key: config_value(HARNESS_ROOT, key, default)
        for key, default in {
            "harness.runtimes": "",
...
def find_ancestor(_repo: Path, pid: int, runtime: str | None) -> dict[str, Any]:
    fake_ancestor = os.environ.get("HARNESS_FAKE_AGENT_ANCESTOR_PID")
    if fake_ancestor and runtime == "fake":
        candidate = int(fake_ancestor)
        return {"pid": candidate, "pidStartedAt": started_at(candidate), "pgid": os.getpgid(candidate), "runtime": "fake", "argv": "harness-fake-agent"}
