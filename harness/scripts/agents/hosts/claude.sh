...
wait_for_start_gate() {
  local gate=${HARNESS_HOST_START_GATE:-} cap=${HARNESS_HOST_START_GATE_TIMEOUT_SEC:-10} started=$SECONDS
  [[ -z "$gate" ]] && return 0
...
max_budget=${HARNESS_CLAUDE_MAX_BUDGET_USD:-5.00}
max_turns=${HARNESS_CLAUDE_MAX_TURNS:-50}
