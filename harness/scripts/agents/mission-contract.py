...
def expand_paths(repo: Path, project_root: Path, ref: str, globs: Iterable[str], label: str) -> list[str]:
    ...
    try:
        ...
        fail("harness project root is outside its git repository")
...
def process_has_tag(project_root: Path, pid: int, started: int, tag: str) -> bool:
    ...
    try:
        ...
        fixture = os.environ.get("HARNESS_MISSION_PROCESS_IDENTITY_FILE")
        if not fixture:
            return False
        try:
...
def project_root_for(contract: Path, repo: Path) -> Path:
    try:
        contract.relative_to(HARNESS_ROOT)
        HARNESS_ROOT.relative_to(repo)
    except ValueError:
        return repo
    return HARNESS_ROOT
