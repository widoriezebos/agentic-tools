...
registered_runtime() { # runtime
  local configured
  configured=$(config_get --key harness.runtimes --default '')
...
model_tier() { # runtime, model; prints 999999 if absent
  python3 - "$root/harness.conf" "$1:$2" <<'PY'
import re, sys
from pathlib import Path
wanted = sys.argv[2]
tiers = []
...
try:
    ...
    fixture_path = os.environ.get("HARNESS_FAKE_PROCESS_IDENTITY_FILE")
    configured = ""
    for raw in (root / "harness.conf").read_text().splitlines():
        if raw.startswith("harness.runtimes="):
...
dispatch_job() {
  ...
  registered_runtime "$runtime" || die 1 "runtime $runtime is outside harness.runtimes"
