...
classifications=$(sed -n 's/^- Classification:[[:space:]]*//p' "$file" \
  | grep -oE 'contract-improved|falsified-continue|falsified-dead-end|no-progress|invalid-run' || true)
...
echo "stop-loss not triggered: $total_cycles cycles, $no_progress no-progress, budget ${budget:-none}"
