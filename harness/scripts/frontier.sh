...
Defaults: --file plans/frontier; --min-delta 0, override with
HARNESS_FRONTIER_MIN_DELTA or set the project noise floor per run.
Exit codes: 0 ok / beats frontier; 1 blocked / does not beat; 2 usage error.
USAGE
}

file=plans/frontier
min_delta=${HARNESS_FRONTIER_MIN_DELTA:-0}
...
numeric='^-?[0-9]+([.][0-9]+)?$'
[[ "$min_delta" =~ $numeric ]] || { echo "invalid --min-delta: $min_delta" >&2; exit 2; }
