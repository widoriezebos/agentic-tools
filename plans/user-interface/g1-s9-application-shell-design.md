...
- Gate 1, state `designed`, **revision 2**, author Claude on Fable as a D23 delegate, 2026-09-21. Revision 2 applies Sol's nine material findings and F11 ([review](g1-s9-application-shell-design-sol-review.md)): adoption reader, layout arithmetic, explicit cuts, dock persistence per the 4.13.1 API, keyboard contract, contrast tokens, component states, honest empty states, reserved paths, separator keys.
- Refines the [master](../user-interface-design.md) at `ui-development` commit `479c1ba54`: Agreed interface structure (`:19` to `:43`), The embedded agent workspace (`:84` to `:108`), The browser workspace (`:419` to `:449`), Workspace identity (`:451` to `:461`), the sections (`:463` to `:689`).
...
Paths are relative to `metasystem/` unless they start with `plans/`. Code claims were verified in source at `479c1ba54`, library claims in the `react-resizable-panels` `4.13.1` tag; nothing was executed; registry read 2026-09-21. `g1-s8` is under revision, so it is cited by section.
...
D28 settled revision 1's open questions; nothing is conditional. Two cuts (plan `:184`). **First cut**, this build: the shell at three widths; rail, header, panes, dock, focused view; routing and the resolver; the thin workspace resource with its adoption reader under `g1-s7`'s name; identity chip and title; empty states; tokens, type, motion, focus; the components; theme; four items of client state. **Second cut**, after `g1-s7`: the connection indicator, `/-/health` refetched on window `focus` and `online`, the rebuilt-executable notice, refetch on invalidation; specified here, marked *second cut*, absent from the first cut, which fetches only `/api/workspace`. Out of scope: other `/api/` routes; conversation and composer behaviour (gate 3); the event stream and automatic reload; the command palette; sign-in; mutation.
...
| Dock | 400 default, 320 minimum, 640 maximum; pointer and keyboard resizable; width remembered; header 48, 12px side padding |
...
Every pane is `EmptyState` (Components). None says nothing exists: the records exist (plan `:149`: 156 live and 430 archived goals here); what is absent is the projection, so every body says so and names what belongs there. Nothing in this build reads the records, so the action is one this build honours, or none. No pane renders an empty table, list, or endless skeleton.

| Pane | Heading; body; arrives with; action |
| --- | --- |
| Overview | Overview is not projected yet; This build does not read the records. Overview will show what needs you and what changed since your last visit, linked to its records; `g1-s14`; none |
| Project | Project is not projected yet; Intent, architecture, designs, constraints and assurance, open questions, and sittings are in the repository and will be read here; gate 5; none |
| Backlog | The backlog is not projected yet; Goals exist at the accepted tip. Board, outline, dependencies, list, and one workspace per goal will read them here; `g1-s10`, detail `g1-s11`; none |
| Fleet | Fleet is not projected yet; This machine's sessions, jobs, census, and health are recorded and will be shown with observation times and gaps; `g1-s13`; none |
| Decisions | Decisions are not projected yet; Questions from every seat, approvals, delegations, and rulings are in the ledger and will be read here; `g1-s12`; none |
| Application | Application is not projected yet; Implemented and released behaviour and its evidence are recorded and will be shown here; gate 6; none |
| Settings | Live: "About this workspace" (subject, mode, checkout, installation, state root, engine build, started at, the notices link, "source at HEAD: arrives with `g1-s7`", "adopted from": the SHA, "not recorded" for `placeholder` or `absent`, else "unreadable") and "Appearance" (the theme control); beneath, `EmptyState`: Settings pages are not built yet; Runtimes and models, connections and channels, identity and authority, execution defaults, storage and retention; gate 7; none |
| Brain | The Brain is not connected in this build; Conversation, shared context, activity, and actions; gate 3; none (the disabled composer beneath, with its reason) |
| Subject (focused) | No subject selected; The subject panel will show the artifact under discussion; gate 3; none |
| Not found | No page at this address; The address matches no section of this workspace; the address bar; "Go to Overview" |
...
**Keyboard.** Sequential order: skip link; rail toggle (from 1,128), Brain, the six sections, Settings, System, Light, Dark; menu (phone); Retry (when shown); dock toggle; *second cut* Reload; the pane's focusables in DOM order (Go to Overview; on Settings the theme control and notices link; nothing in the six empty panes); the separator (wide); Expand; Close. `main`, the disabled textarea, and the disabled Send are not in the order. A sheet traps focus while open. Every reason is reachable: the `aria-disabled` toggle's tooltip opens on focus; the composer's reason and the conflict text are visible.
...
- `EmptyState`: centred, 96px from the pane's top; icon 24 `text-3`; 12; heading 16/600 `text`; 8; body 14/20 `text-2`, max 480; 12; slice line 12/16 `text-3`; 16; action `Button`.
...
**Second cut**, after `g1-s7`: `src/shell/ConnectionIndicator.tsx`, `Notice.tsx`, `health.ts`, their tests; the event-stream subscription; no Go change.
...
- **O2, honest empties.** Heading, body, slice line, and action verbatim; none asserts absence; no empty list, table, or endless skeleton; `empties.test.ts` passes.
...
- **O7, bounded state.** `storage.test.ts` passes; every access wrapped; only the four keys written; the width written only under `isUserInteraction`, in pixels from `getSize().inPixels`; panel ids `work` and `dock`.
...
- **O12, geometry.** The five measured widths match the arithmetic; the rail collapses between 960 and 1,127; the focused switch below 960, the subject panel from 960.
- **O13, visual states.** Every bullet above matches in the browser in both themes; a deviation is a finding.
- **O14, cuts.** `cuts.test.ts` passes: no second-cut component or health fetch in the first cut.
