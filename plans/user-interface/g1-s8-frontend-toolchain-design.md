...
- Gate 1, state `designed`, revision 3 after Sol's round-2 design review (D12), author Claude on Fable as a D23 delegate, 2026-09-21.
...
Paths are relative to `metasystem/` unless they start with `plans/`. Code claims were verified by reading the source at `762446dfe` and the Go 1.27.1 tree, revision 3's additions at the working tree of `e0d2beefe`; nothing was executed. Registry, npm, Git, and upstream facts were read on 2026-09-21.
...
Lands before this slice, by its own design, critique, and build; the planner numbers it. It edits `internal/behaviorsurface`, `internal/pathpattern`, `internal/proofrun`, `internal/parallelratchet`, `internal/audit`, `scripts/agents/go-gate.sh`, and `scripts/agents/brain-fixtures.sh`, none of which `g1-s8` touches. It must deliver:
...
**Audit** (G1). After `npm ci --ignore-scripts`, the bundle script runs `npm audit --json --include=dev` in `_app`, captures stdout, and continues only when all four hold: the exit status is 0; stdout parses as a JSON object; `metadata.vulnerabilities.total` is the number 0; and `vulnerabilities` is an object with no keys. Anything else refuses and prints whatever npm wrote, so an unreachable registry, a non-JSON answer, or an npm that emits another shape refuses rather than skips. The count is the rule because a threshold was not: npm's `audit-level` is "the minimum level of vulnerability for `npm audit` to exit with a non-zero exit code", its values are `info`, `low`, `moderate`, `high`, `critical`, `none` (npm docs, `audit-level`), and the exit code ranks `info` below `low` (`npm-audit-report/lib/exit-code.js` in `npm/cli` at `latest`: the list `['info', 'low', 'moderate', 'high', 'critical', 'none']`, non-zero when some count at or above the level is positive), so revision 2's `--audit-level=low` let an informational advisory through. The JSON is Arborist's `AuditReport.toJSON` (`workspaces/arborist/lib/audit-report.js`, `npm/cli` at `latest`): `metadata.vulnerabilities` carries `info`, `low`, `moderate`, `high`, `critical`, and `total`, the number of vulnerable packages, and `vulnerabilities` holds one entry per such package whatever its severity, so an advisory of any level makes both non-zero. `--include=dev` keeps development dependencies in the audit even where `NODE_ENV=production` would otherwise omit them (npm docs, `omit` and `include`). No `--audit-level` is passed; a non-zero exit refuses whatever npm's default threshold is, and the count refuses the rest.
...
| --- | --- |
| Node differs from `.nvmrc`; the audit's JSON counts any vulnerable package, of any severity including `info`; npm exits non-zero or prints no JSON object | Bundle script refuses, naming both versions, the vulnerable packages with their severities, or npm's output |
...
- **O1, exclusion.** The exclusion slice is on the branch before the first install. With `node_modules` installed and absent, the same binary prints the same ENGINE, LANDING, and PAYLOAD digests, `audit parallel-ratchet` and `audit stop-decision-surface` agree, `section/brain-fixtures` passes, and the gate's gofmt, vet, and staticcheck pass with the tree installed, the gofmt also with a tracked `.go` file deleted only in the working tree; the slice's fixtures, the seam-scan prune among them, are present.
...
- **O9, supply chain.** `package.json` versions equal the tables; `.npmrc` as written; no `postinstall`; the lockfile committed; the bundle script runs the Node check and `npm audit --json --include=dev`, and refuses unless the exit status is 0, stdout parses, `metadata.vulnerabilities.total` is 0, and `vulnerabilities` is empty; no `--audit-level`, no exceptions file, no bypass of any kind.
