...
Every fleet has a brain whether or not the interface runs, and the interface is optional, so the brain does not live inside the interface. A fleet has one brain seat, at one canonical brain home that a terminal occupant and the hosted brain both validate at startup. The existing declaration enforces only one brain per host, so fleet-wide uniqueness is an explicit contract to add; relocation and automatic failover are deferred. The brain's live session is hosted by a brain process at that home, with its own start and stop. That process is the single occupant of the checkout lease, provider sessions are subordinate to it, and it owns the agent session, the application tools, the sitting store, and the budgets, so the brain works with the interface stopped. Before it enables any tool it verifies that it actually holds the lease, because an announcement can succeed while a live competing holder keeps the lease. The interface is a window onto that brain: its dock connects to the brain process, the interface owns browser sign-in and the human's session, and human credentials never become tool credentials. Conversations and windows may be many; the seat is one. Whether a brain must also be alive and acting on fleet events with nobody present is not decided; this design assumes an explicitly started occupant, and unattended turns would need a wake-up source and their own budget model.
