...
5. **Journal and recovery.** An open's journal intent carries both lists and recovery rebuilds the same mutation; `block` and `unblock` journal their edge and are replayed the same way. A replay carries no proof, so it is a seat's hand whatever name the journal recorded, and the seat rule is judged again at the replay. That is the whole boundary: an edge act a person made journals their name, a stored name cannot restore the proof that made it theirs, and recovery refuses it rather than running a person's act as a seat — the early unblock because no seat performs one, and a `block` or a satisfied `unblock` because the goal may not be this seat's to touch. A seat's own edge acts journal no name and replay.
