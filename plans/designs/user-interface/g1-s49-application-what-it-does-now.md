...
3. **The truthful account of what landed is the ledger's.** Every one of
   ...
   label `robustness`, most carry none; arcs are rare. Each conclusion
   names what landed and often the commit and the machine.
4. **The record of what is wrong is a register.** `memory/known-issues.md`
   is a table, one row per defect or limitation, 46 rows here: `| Id |
   Date | Symptom and evidence | Cost when it bites | Fix direction or
   lever | Status |`, the status column prose that begins FIXED, ACCEPTED
   or a date when concluded, and an open question (Q-…KGS7) records that
...
What the kit can truthfully answer today: J2 and J6 in full; J1 as the
sum of J2, because the conclusions of landed goals are the only
implemented-behaviour record the engine keeps; J3 from the register; J4
...
- D1. **The header names the subject and the mode**, as the shell does,
  ...
  Beside it, the engine this seat runs, from its own presence record,
  "engine 5b9d958 · generation 4", or "this seat has published no engine
  build"; the source at HEAD and whether they differ is step 2, named.
- D2. **"What landed" is the answer to J1 and J2.** The done goals with a
  conclusion, newest first, grouped by week under a line that says the
  ...
  from the rows shown. The header's count line: "429 landed · 320 this
  month · 7 since your last visit".
- D3. **"Known problems" is the answer to J3.** A reader for the register,
  ...
  names of its six columns and reads either column set by position; a
  row whose status begins with FIXED, ACCEPTED or CLOSED is concluded,
  any other is open; a row with the wrong column count is a defect listed
  in one quiet line, never hidden. The block lists open rows first, one
...
{ "schemaVersion": 1, "readAt": "…",
  ...
  "engine": { "build": "5b9d958", "generation": 4 } | null,
  ...
  "problems": { "columns": ["Id","Date","Symptom and evidence",…], "open": [ {row} ], "concluded": [ {row} ],
                "defects": [ "row=12: wrong column count: got 5, want 6" ] },
...
sentence rewritten to what it shows. Header, then three blocks in the
order of the jobs: What landed, Known problems, What it is. Rows and open
rows in the inbox's idiom; Find and chips as the queue's. Help terms:
`application-section` rewritten, `what-landed`, `known-problems`,
`landed-new`. The Partner's page capture carries the block open, the
...
Go: the known-issues reader over a fixture with both column sets, an open
and a concluded row of each status word, a wrong-column row; the
...
board observation every page shares. Medium on D3: a new reader over a
prose register with two known shapes; the defects line is its honesty.
Medium on D1: the engine build is this seat's own record and may be
absent. Weakest: J1 answered as a changelog rather than a capability map,
