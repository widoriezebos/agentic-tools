...
Disposition at the time, **since withdrawn, see Sol's review below**: partly agreed. The ambiguity was real and is removed in revision 5 of the design and in the master. The direction of the fix is not the one Astra suggested. The engine keeps two conventions in the self-hosted layout, checked in a template clone in active use: process families, the steward and the supervision, keep lifecycle state under the Git checkout (`cmd/metasystem/up.go:45`, `:169`, `process_verbs.go:52`), while the goal journal, the channel, and jobs keep domain state beneath the installation. The interface server is a process family and was modelled on the steward runner, so its state stays where it was designed, and D20's sentence was narrowed to the subject's state. No behaviour changed, so the design stands as implementable and the existing build still conforms. The human has the last word.

## Sol's design review under D12, 2026-09-21

Codex on `gpt-5.6-sol`, read-only, on revision 5; the report is [here](g1-s1-server-lifecycle-design-sol-review.md). Verdict: build after the listed changes. Three material findings, all accepted after the planner re-read the cited source.

| Finding | Problem | Disposition |
| --- | --- | --- |
| D1 | The checkout placement rests on a false premise. Existing process control uses the installation as its lifecycle root (`cmd/metasystem/process_verbs.go:50` to `:75`, `up.go:201` to `:205`), and the state-root owner has a steward kind (`internal/stateroot/stateroot.go:257`). Keying on the Git checkout puts the self-hosted lock in the wrong tree; using the literal installation would be wrong for a vendored adopted installation | **Accept; the planner's position on Astra's B8 is withdrawn.** State is located through `stateroot.RootForInstallation(installation)`. Obligation O3 |
| D2 | `--repo` and `--metasystem-root` were resolved independently, so a builder would accept checkout A with installation B, and later slices would read B's state under A's record | **Accept.** `lifecycle.ResolveRoots` derives and checks the three roots once; the checkout defaults to the installation's Git top; a mismatched pair is refused; `serve` derives them again with the same function. Obligation O4 |
| D3 | `UIListen` gave no seam for a hermetic default-value test: `config.Get` reads the ambient environment, the test environment does not clear a listen variable, and tests may not set one | **Accept.** An unexported resolver takes the environment lookup; the exported function passes `os.LookupEnv`. Obligation O5 |

Sol also marked three of the planner's supporting claims wrong: that process families receive the Git checkout; that in an adopted project the checkout and the installation are always the same place, which holds only for an installation at the repository root; and that the goal journal and the channel resolve an installation themselves, when they join whatever root their caller supplies. All three are corrected in revision 6 and in the master.

**Loop closed.** The three findings are narrow and each is expressible as a test, so they are obligations O3 to O5 for the code critique, beside O1 and O2. The design is ready to build on the human's explicit go. The existing build on `ui/g1-s1` predates revision 6 and does not conform to it in these three respects.
