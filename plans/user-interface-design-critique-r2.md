...
| B8. `g1-s1` conflates Git scope and state scope | **Partly agree** | The ambiguity is real and is removed. The direction is not Astra's: see below | Master: workspace identity. `g1-s1` design revision 5 and its critique record. Plan: D20, `g1-s1` |
...
**Not agreed with: the direction of B8's fix.** Astra assumed that D20's "every home of state" covers the interface server's lifecycle state, and concluded that the state must move beneath the installation before `g1-s1` can be finished. The planner checked what the engine does in a template clone in active use. It keeps two conventions in the self-hosted layout. Its process families, the steward and the supervision, keep lifecycle state under the Git checkout, resolved with `upRepositoryScope` (`cmd/metasystem/up.go:45`, `:169`; `process_verbs.go:52`). Its domain owners, the goal journal, the channel, jobs, keep theirs beneath the installation. The interface server is a process family and was modelled on the steward runner, so its state belongs with theirs; moving it would make it the one process family that differs. In an adopted project the two roots are the same directory, so the question arises only in the self-hosted layout. What was wrong was the planner's sentence in D20, which was too broad; it now names the three roots and says which state follows which. `g1-s1` gained a paragraph and lost no behaviour, so the planner's view is that it can be built as designed and that the existing build still conforms. The human has the last word.
