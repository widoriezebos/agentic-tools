...
**Revised later the same day (D18, D19).** The human settled the open question below and widened the model: nothing coordinates the seats, the brain is a sitting of a human and an agent with no unattended turns, and many humans may take part, each at the interface of their own seat. That reverses the direction taken for A5: one brain per host is the intended scope, not a gap, so fleet-wide uniqueness is not added. Astra's observation stands, that the code enforces per-host scope (`internal/brain/brain.go:283`); only the conclusion drawn from it changed. The rest of the S1 resolution is unchanged: the brain's session is its own process, the interface is a window, sign-in stays with the interface, and startup verifies the lease.

**Was open for the human, now settled by D18.** Whether "every fleet must have a brain" also means a brain that is alive and acts on fleet events with nobody present. Astra and the planner both assumed an explicitly started occupant. Unattended turns would need a wake-up source and a budget model of their own.

## Dispositions after Astra's second review

[Astra's second review](user-interface-design-critique-astra-r2.md) covered how its first findings were applied and the decisions taken after it (D18 to D21). The human's instruction was the same: apply what the planner agrees with, list what it does not, and give the reason. Every code claim was re-read in source by the planner before its finding was disposed.

Two of these findings correct the planner, not the design. The dispositions table above said, under A2, that human handoff cancellation "does the same" as session stop and requires the attending process to stay alive. That was wrong: the planner had read nine lines of the cancellation path and generalised from session stop. And under A3 it added that on a single-machine fleet the reader rollout "is the one engine build". That was wrong too, and the planner's own `g1-s1` design shows why: a detached interface server keeps running the executable it started with.

| Finding | Disposition | Reason | Applied in |
| --- | --- | --- | --- |
| B1. Attendance has no end condition | Agree | D18 said the occupant exists only while a human sits but never said when sitting ends, so an implementer could keep starting turns after the human left, or kill the brain when one of several tabs closed. No code claim involved | Master: the brain, conversation lifecycle, gate 3. Plan: gate 3, vocabulary |
| B2. UID-R1-11 was weakened below the isolation paragraph | Agree | The planner's S8 edit let an examiner with a checkout-wide read root pass while reading exploratory notes. The right fix for S8's original complaint is a grant by file, not by directory. Astra also withdrew its own earlier assent to S8 | Master: UID-R1-11 |
| B3. History does not identify arbitrary humans reliably | Agree | The actor is `"human:" + name` (`internal/goal/verbs.go:194`), the validator allows spaces (`sessionstop.go:111`), and a History line is split on whitespace (`file.go:1957`), so "Jane Doe" publishes a line readers reject; channel answers hard-code `human:wido` (`verbs.go:172`) | Master: human acts. Plan: `g2-s8` |
| B4. The named question path is local, and phone authority is singular | Agree | `goal.ReadChannelTree` reads questions, inbox, and listeners at the ledger tip and the inbound path checks state inside the transaction (`internal/goal/channel_inbox.go:20`, `:209`); `channel.WalkOpenQuestions` reads this checkout's files only. The planner's own first sweep had noted `ReadChannelTree` and the plan still named the local reader. The single-responder detail rests on Astra's citations and on the hard-coded actor the planner did verify | Master: Decisions. Plan: facts, `g1-s5`, open decision D22 |
| B5. Boot still supplies the old role contract | Agree | The role packet says "One seat for a fleet", reserves approval to one named human, and says the brain "does not write ... designs or briefs" (`records/misc/fleet-coordinator-brain-role-packet.md:10`, `:19`), which contradicts both D19 and what this design asks of the brain | Master: the brain, gate 3. Plan: gate 3 |
| B6. One build is not a reader rollout | Agree; the planner's addition was wrong | See above | Master: human acts. Plan: `g2-s8` |
| B7. Layout detection is a heuristic, and discovery needs an installation path | Agree | `templateMode` is a directory named `metasystem` beside `development/metasystem-design.md` (`internal/stateroot/stateroot.go:293`); `ResolveLayout` finds an installation only by walking up from the path, or a template-shaped nested one (`:158` to `:171`), so an application's root does not discover an ordinary nested adopted installation. "The engine already knows" was too strong. Placement is the planner's: hardening `templateMode` changes state resolution for the whole engine, so it is its own slice, `g1-s19`, and is not folded silently into an interface slice | Master: workspace identity. Plan: D20, `g1-s2`, `g1-s19` |
| B8. `g1-s1` conflates Git scope and state scope | **Agree. The planner first disagreed with the direction of the fix, and was wrong** | See the correction below | Master: workspace identity. `g1-s1` design revision 5 and its critique record. Plan: D20, `g1-s1` |
| B9. Ownership is not an upgrade-overwrite oracle | Agree | At a root installation `shippedInventoryPath` classes `plans/`, `memory/`, `records/`, `docs/project-rules.md`, and `AGENTS.md` as the MetaSystem's (`internal/stateroot/owner.go:142`), while the upgrade procedure never overwrites `plans/` or `docs/project-rules.md` (`docs/metasystem-reconciliation.md:78`). The planner had copied the enum's own comment, "replaced wholesale by an upgrade", into the design as if it were upgrade behaviour | Master: workspace identity, scenario 46. Plan: vocabulary, coverage |
| B10. Handoff cancellation was given a continuing attendance requirement it does not have | Agree; the planner's source claim was wrong | The cancellation formats the process identity into a reason and cancels under the lock, with no later liveness check (`internal/steward/handoff_capture.go:1249` to `:1282`) | Master: human acts. Plan: `g2-s8` |
| B11. The light path dropped Sol's design review | Agree | The human said of D12, "make the change and stick to it"; the planner's proposal contradicted it. A review of a half-page note is cheap | Plan: delivery order |
| B12 to B14 | Noted | They confirm D18's premise in source, that the narrator cursor is local to a checkout, and that an interface on a delivery seat coexists with its lease | No change |
| B15. The read path is satisfiable | Agree | `RootForInstallation(installation)` handed to `ResolveEndpoint`; neither `Layout.RepositoryRoot` nor `StateRoot` substituted blindly | Plan: `g1-s2` |
| B16. The candidate flag needs a known base and candidate | Agree | Missing linkage means unavailable, never "changes none" | Master: workspace identity, scenario 46 |
| B17. The first three steps keep their order | Agree | The header needs real workspace metadata before goal data, and the toolchain and policy work get the full pipeline | Plan: delivery order |
| Consequence: a delegation has no recipient and any full-standing human can revoke it | Agree | The view must show the owner's actual scope | Master: Decisions |
| Consequence: the uniqueness boundary is a registry home, not a host | Agree; the planner's "exactly per host" was overstated | `RegistryHome` defaults to the account's home and can be overridden (`internal/brain/brain.go:228`) | Master: the brain. Plan: D19 |
| Consequence: frontend placement works with a concrete Go boundary | Agree | `internal/` is already shipped and already covered by ownership, so the planner's "joins the shipped inventory" was unnecessary; an embed pattern cannot cross a module boundary | Master: workspace identity. Plan: `g1-s8` |
| Consequence: redistribution needs the bundled code's notices | Agree | MIT and Apache-2.0 notices accompany what is actually bundled; a compiler that is only used is not shipped | Plan: `g1-s8` |
| Consequence: a Radix modal injects a stylesheet at run time | Agree, as a requirement to exercise | Astra's chain rests on upstream sources the planner did not re-read; the requirement it leads to, a nonce in the policy and a modal exercised in the embedded production build, is cheap and right whether or not every link holds | Plan: `g1-s8`, delivery step 2 |

**Not agreed with: nothing. The planner's disagreement with B8 is withdrawn.** It had argued that the engine's process families keep lifecycle state under the Git checkout, so the interface server should too. Sol's design review of `g1-s1` showed the premise false, and the planner then confirmed it: `resolveProcessScope` returns `Root: installation` (`metasystem/cmd/metasystem/process_verbs.go:50` to `:75`); `up` resolves the checkout and then replaces it with the installation before arming, under the comment "Supervision control, enrollment and accounting belong to the authenticated installation" (`up.go:201` to `:205`); and the state-root owner has a kind for the steward's directory (`internal/stateroot/stateroot.go:257`). In the template clone in active use, the steward's and the supervision's state beneath `metasystem/` was written that day, and the copies under the Git top were five days stale.

How the planner got it wrong, recorded so that it is not repeated: it cited `up.go:169`, where the checkout is resolved, without reading the thirty-five lines below it, where the value is overwritten; and it took a directory listing as ground truth after cutting it to its first fourteen entries, which hid `steward` and `supervision` beneath the installation. Evidence used to overrule a reviewer has to meet a higher standard than evidence used to agree with one, and this met a lower one.

Astra's direction stands, with Sol's precision: the state is located through `stateroot.RootForInstallation(installation)`, which is the installation in the self-hosted layout and the application's repository root for an adopted installation, vendored or not. Revision 6 of the `g1-s1` design applies it.
