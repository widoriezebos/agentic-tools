...
| D20 | The interface distinguishes clearly between building the MetaSystem itself with the MetaSystem and building another application with it. There is no switch: it is a fact of the repository that the engine's layout owner resolves, by a heuristic today, so the interface resolves against the installation it was started from and refuses visibly when the marker and the adoption record disagree (`internal/stateroot`: the template's self-hosted layout, or an adopted installation vendored beneath, or placed at, the application's repository root). One workspace is one checkout with one subject; the header always says which, and the self-hosted case looks different. Inside a workspace the subject (Project, Backlog, Decisions, Application) and the machinery instance (Fleet, Settings) never blur. Paths carry the engine's ownership answer, which says nothing about what an upgrade overwrites. A checkout has three roots: the Git checkout, the installation, and the state root. The subject's state is located through the state-root owner from the known installation, never a fixed prefix; a process family's lifecycle state stays under the Git checkout, as the steward's and the supervision's does. Master: Workspace identity. | 2026-09-21 |
...
| --- | --- | --- | --- | --- |
| `g1-s1` Server lifecycle | `metasystem ui start` launches a background loopback server for one configured project and brain checkout and reports its address; `ui status` reports it; `ui stop` ends it cleanly after confirming the process is the one it started (pid and start time); `ui restart` replaces it with the build on disk at the same address. A second start is refused. Foreign Host and Origin are refused, and only GET and HEAD are served. No engine, brain declaration, or claim is started, and the engine's stop fence is deliberately not consulted. New: a `ui` family entry, `cmd/metasystem/ui.go`, packages under `internal/ui/`, `internal/config/ui.go` | none | [design](user-interface/g1-s1-server-lifecycle-design.md), [critique](user-interface/g1-s1-server-lifecycle-design-critique.md) | `designed`; **Sol's design review is still owed under D12 before anything is built.** Three Opus critique rounds predate the roster, and Astra's two reviews were extra reads at the human's request; neither fills Sol's lane. Astra's first review said implement as written; its second said not until the design tells the Git checkout, the installation, and the state root apart (B8). Revision 5 of the design now does, without changing behaviour: lifecycle state stays under the Git checkout, where the steward and the supervision keep theirs. The Codex build on `ui/g1-s1` (`3f1aadbd1`, worktree `.claude/worktrees/g1-s1`) was dispatched without the human's go, is unreviewed, passes build, vet, and tests, and conforms to revision 5 as it did to revision 4. The human decides whether it is kept, rebuilt under D12, or discarded |
