...
- The one exception, by D25: slice `g1-s20` edits `internal/behaviorsurface`, `internal/pathpattern`, `internal/parallelratchet`, `internal/audit`, `internal/proofrun` and `scripts/agents/go-gate.sh`, because the exclusion has to live where the walkers are. It lands while the frontend's dependency tree is absent, so it changes no digest and cannot disturb work in flight. Every other slice obeys the rule below.
