...
| `g1-s9` Application shell | Carries the design language folded in from `g1-s18`, in two cuts. First cut: a header that always shows the workspace's subject and mode, with a distinct treatment for the self-hosted case and the same in the tab title (D20). Navigation rail with the agreed sections, empty states, stable links, the resolver from reference to route, and a Brain entry that states it is unavailable until gate 3 Second cut, after `g1-s7`: connection and freshness indicators, refetch on invalidation, and the rebuilt-executable notice | `g1-s8`, D28 | none yet | `defined` |
...
| `g1-s20` The engine excludes a folder inside `internal` | Engine owner work. The Engine projection gains the exclusion the other projections already have: `internal/behaviorsurface/policy.v2.json` carries `nonRepositoryPaths` and `repositoryOperationalDataPaths` for generated and installed content, but `Policy.Includes` consults neither for Engine, whose rule is a bare match against `enginePaths`. Every walker that would meet the frontend's dependency tree honours the same exclusion: the behaviour-surface digest, the proof-input expansion through `testing.json` and `internal/pathpattern`, the parallel ratchet, and the stop-surface audit. It lands while the tree is absent, so it changes no digest, and the verification proves the ENGINE digest is identical before and after. No dependency may be installed in this repository until it has landed | D25 | none yet | `defined` |
