...
- Codex is dispatched through the Codex plugin. Opus 5 is dispatched as a fresh-context subagent. Both critique steps use the materiality test and written dispositions from `metasystem/skills/design-critique` and `code-critique` as a method; the engine's round limits and registers do not bind them.
...
| 3. Design critique | Opus 5, fresh context, not the author | The slice design and the master sections it cites | `user-interface/<slice-id>-design-critique.md` with findings and a disposition for each; the loop stops at the first round with no material finding | `design-critiqued` |
| 4. Implement | Codex | The slice design at a pinned commit; nothing else is authority | Branch `ui/<slice-id>` | `implementing` |
| 5. Code critique | Opus 5, fresh context | `git diff ui-development...ui/<slice-id>` against the slice design: conformance first, then defects | `user-interface/<slice-id>-code-critique.md`; corrections return to Codex | `code-critiqued` |
...
- Work that must edit existing CLI handlers or their tests is split into its own slice and held `blocked` until the other agent is done. `g1-s16` is the first of these. `g1-s17` edits engine source but no existing test, under the stop-and-report rule in its row.
...
| --- | --- | --- | --- |
| D3 | Approved by the human on 2026-09-21 and running: independent second review of the round-one amendments. The master's status line says they have had none, and gates 2 and 3 rest on them (command edge, authority, preparation and basis). | Gate 2 slice designs | Run it with Opus 5, scoped to the sections gates 2 and 3 cite, while gate 1 is being designed and built. |
...
Slices `g1-s2` to `g1-s6` are plain Go packages with no dependency on the server, and `g1-s8` needs only `g1-s1`, so the read models and the frontend toolchain can proceed side by side. The shortest path to a board in a browser is `s1`, `s17`, `s2`, `s4`, `s7`, `s8`, `s9`, `s10`. Design `g1-s1` first: it fixes package layout, naming, configuration, and lifecycle for everything after it.

One interaction with the engine is known and must be fixed early. The process census scopes by working directory or arguments below the checkout and classes the interface server UNTRACKED (`internal/census/run.go:208` to `:335`). The engine never signals such a process: the top-level `stop` lists it as "not the metasystem's, not touched" (`internal/stoptransition/families.go:841` to `:900`). But the steward counts it (`internal/steward/census.go:74`) and, when no worker is alive, notifies instead of reviving (`verdict.go:130`); handoffs are held (`handoff.go:101`); and the end-of-turn watchdog lists it until acknowledged (`internal/supervise/watchdog.go:135`). So a running interface suppresses the engine's automatic revival in that checkout. Critique round 1 of `g1-s1` found this; the first draft of this plan had called it a wording problem. `g1-s17` fixes it and is the second slice to build, directly after `g1-s1`. Until it lands, the interface should not be left running beside an engine that is relied on for revival.

| Slice | Outcome a human can observe | Depends on | Design | State |
| --- | --- | --- | --- | --- |
| `g1-s1` Server lifecycle | `metasystem ui start` launches a background loopback server for one configured project and brain checkout and reports its address; `ui status` reports it; `ui stop` ends it cleanly after confirming the process is the one it started (pid and start time); `ui restart` replaces it with the build on disk at the same address. A second start is refused. Foreign Host and Origin are refused, and only GET and HEAD are served. No engine, brain declaration, or claim is started, and the engine's stop fence is deliberately not consulted. New: a `ui` family entry, `cmd/metasystem/ui.go`, packages under `internal/ui/`, `internal/config/ui.go` | none | [design](user-interface/g1-s1-server-lifecycle-design.md), [critique](user-interface/g1-s1-server-lifecycle-design-critique.md) | `designed`; critique round 1 folded, round 2 running |
| `g1-s2` Accepted-tip snapshot | A package holds a parsed snapshot read through `goal.Project` from the accepted ref, never from worktree bytes, with tip identity, observation time, and the owner's staleness banners. Composition of exported functions; no extraction | none | none yet | `defined` |
...
| `g1-s9` Application shell | Navigation rail with the agreed sections, empty states, stable links, the resolver from reference to route, connection and freshness indicators, refetch on invalidation, a reload when the server's build changes after a reconnect, and a Brain entry that states it is unavailable until gate 3 | `g1-s7`, `g1-s8` | none yet | `defined` |
...
| `g1-s17` The engine accounts for the interface server | The census gives the process recorded in `artifacts/agents/ui/server.json` its own class. The steward neither counts it as a worker nor treats it as doubt, so revival and handoffs behave as if it were absent; the watchdog does not list it; the engine's `stop` reports it as the interface, left running. Edits `internal/census`, `internal/steward`, `internal/supervise`, and `internal/stoptransition`, adding new test files only. If an existing test must change, the slice stops and reports, because those files are the other agent's. Built second, directly after `g1-s1` | `g1-s1` | none yet | `defined` |
