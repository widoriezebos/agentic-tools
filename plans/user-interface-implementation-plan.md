...
Status: the Astra review is in and applied (2026-09-21). **Building remains on hold until the human's explicit go (D13).** Draft for human correction, 2026-09-21. Gate 1 is sliced against a source reading of the engine at commit `94181f05a`. Gate 2 is provisional and gates 3 to 7 are deliberately coarse.
...
| D20 | The interface distinguishes clearly between building the MetaSystem itself with the MetaSystem and building another application with it. There is no switch: it is a fact of the repository that the engine already determines (`internal/stateroot`: the template's self-hosted layout, or an adopted installation vendored beneath, or placed at, the application's repository root). One workspace is one checkout with one subject; the header always says which, and the self-hosted case looks different. Inside a workspace the subject (Project, Backlog, Decisions, Application) and the machinery instance (Fleet, Settings) never blur. Paths carry the engine's ownership answer. State is located through the engine's layout owners, never a fixed prefix. Master: Workspace identity. | 2026-09-21 |
...
| Ownership | The engine's answer for any path: the MetaSystem's, replaced wholesale by an upgrade; the application's, untouched by any upgrade; or runtime state |
...
Also proposed: ceremony scaled to risk. A slice that touches the engine, processes, security, or a data contract gets the full pipeline. A presentation slice gets a half-page note the human agrees to, the build, and the code review, with the human's eyes in the browser as the design review.
...
| `g2-s8` Session evidence in History | A compatible History extension for the session source, the user, and the sign-in issuer: writers, validators, and consumers first, then the reader rollout, and only then the first published line, because an unknown History key makes older readers reject the whole tree at fetch. The owners that consume terminal evidence directly, carry, session stop, and human handoff cancellation, are extended to session evidence or an authenticated-session presence binding when their verbs are exposed | `defined` |
...
| 6 | 46 ownership labels on paths and the flag on a candidate that changes MetaSystem-owned paths (documents at gate 5, changed files here), 27 intended versus evidenced behavior, 33 completed item to retained evidence, 35 inactive and unreachable seats |
