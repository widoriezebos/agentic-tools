...
- Proposed, and a change from "as few npm dependencies as the work allows": a small curated set, all build-time only, because the borrowed look and the master's keyboard-operation requirement are not worth hand-rolling: Tailwind, Radix primitives for menus, dialogs, and tooltips, a resizable-panels library for the dock, and an open icon set. Each is still named and justified in `g1-s8`.
