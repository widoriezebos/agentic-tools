...
- Use focused canaries while changing and reviewing code. Run the final full battery once the work is settled and those canaries are green. A commit is gated on that suite run's verdict, in one shell chain: `scripts/validate-metasystem.sh && ... && git commit && git push`. Never read a verdict from a log tail, a previous shell, or a check whose failure does not stop the chain. Two pushes on a red suite happened in one day, both that way (IL-17).
